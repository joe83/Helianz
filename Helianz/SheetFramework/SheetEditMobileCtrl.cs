	...
	///<summary>Creates an internal deep copy of the SheetDef and it's SheetFieldDefs. The original object will not be modified.</summary>
		public void SetSheetDef(SheetDef sheetDef){
			...
			var mobileFields=_sheetDef.SheetFieldDefs.Where(x =>
					HelianzBusiness.SheetFieldDefs.IsMobileFieldType(x.FieldType,x.TabOrderMobile,x.FieldName) && x.Language==_languageCur
				).ToList();
			...
			foreach(List<SheetFieldDef> sheetFields in finalFields) {
				...
				if(sheetFields.Count > 1 && sheetField.FieldType==SheetFieldType.CheckBox && sheetField.FieldName=="misc" && sheetFields.GroupBy(x=>x.UiLabelMobile).Count()==1 
					...
					Action<SheetFieldDef> addRadioOption=new Action<SheetFieldDef>((sheetFieldRadioItem) => {
						...
						sheetFieldRadioItem.UiLabelMobileRadioButton=HelianzBusiness.SheetFieldDefs.GetUiLabelMobileRadioButton(sheetFieldRadioItem);
		...
		public bool MergeMobileSheetFieldDefs(SheetDef sheetDef,bool requireValidation,Action<string> err=null) {
			try {
				...
				foreach(SheetFieldDef sheetField in listMobileSheetDefs.Where(x=>string.IsNullOrWhiteSpace(x.Language))) {
					if(listMobileSheetDefsAlreadyOrdered.Any(x=>HelianzBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(x,sheetField))) {
						continue;
					}
					...
					if(sheetField.FieldType==SheetFieldType.CheckBox) {
						List<SheetFieldDef> listSheetFieldDefNew=HelianzBusiness.SheetFieldDefs.GetRadioGroupForSheetFieldDef(sheetField,sheetDef.SheetFieldDefs.FindAll(x=>string.IsNullOrWhiteSpace(x.Language)));
						...
						foreach(SheetFieldDef newSFDCheckboxInGroup in listSheetFieldDefNew ) {
							...
							if(HelianzBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(newSFDCheckboxInGroup,sheetField)) {
				...
				if(SyncSheetFieldsWithDefualt) {
					foreach(SheetFieldDef sheetFieldNonDefLang in sheetDef.SheetFieldDefs.Where(x=>!string.IsNullOrWhiteSpace(x.Language))) {
						...
						SheetFieldDef sheetFieldDef=listMobileSheetDefs.FirstOrDefault(x=>HelianzBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(x,sheetFieldNonDefLang,true) && string.IsNullOrEmpty(x.Language));
					...
					foreach(List<SheetFieldDef> sheetFieldDefLanguageGrouping in listSheetFieldDefsByLanguage) {
						...
						foreach(SheetFieldDef sheetFieldDef in sheetFieldDefLanguageGrouping) {
							if(listMobileSheetDefsAlreadyOrdered.Any(x=>HelianzBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(x,sheetFieldDef))) {
								continue;
							}
							sheetFieldDef.TabOrderMobile=tabOrderMobile++;
							//We need to ensure any new SFD checkboxs in a group have the mobile tab order set. Even if they are 'new'.
							if(sheetFieldDef.FieldType==SheetFieldType.CheckBox) {
								List<SheetFieldDef> listSheetFieldDefNew=HelianzBusiness.SheetFieldDefs.GetRadioGroupForSheetFieldDef(sheetFieldDef,sheetDef.SheetFieldDefs.FindAll(x=>sheetFieldDef.Language==x.Language));
								...
								foreach(SheetFieldDef newSFDCheckboxInGroup in listSheetFieldDefNew ) {
									//Checks for the first element
									if(HelianzBusiness.SheetFieldDefs.CompareSheetFieldDefsByValueForMobileLayout(newSFDCheckboxInGroup,sheetFieldDef)) {
