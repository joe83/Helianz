...
namespace Helianz {
	...
	public partial class FormTaskEdit:FormODBase {
		...
		///If FormHelianz is not minimized, the first Task will instead appear in the center of wherever it is.
		...
		private void SetTaskStartingLocation() { 
			...
			if(listFormTaskEdits.Count<=1) {
				FormHelianz formHelianz=Application.OpenForms.OfType<FormHelianz>().ToList().FirstOrDefault();//Should be exactly 1.
				if(formHelianz!=null) { //Should never be null.
					pointStart=formHelianz.Location;
					pointStart.X+=(formHelianz.Width/2)-(this.Width/2);
					pointStart.Y+=(formHelianz.Height/2)-(this.Height/2);
		...
		private void comboJobs_SelectedIndexChanged(object sender,EventArgs e) {
			...
			if(comboJobs.SelectedIndex!=comboJobs.Items.Count-1 || checkIsReadOnly.Checked) {
				FormHelianz.S_GoToJob(_listJobs[comboJobs.SelectedIndex].JobNum);
				return;
			}
		...
		private void butCreateJob_Click(object sender,EventArgs e) {
			...
			FillComboJobs();
			FormHelianz.S_GoToJob(jobNew.JobNum);
			long signalNum=Signalods.SetInvalid(InvalidType.Task,KeyType.Task,TaskCur.TaskNum);
			UserControlTasks.RefillLocalTaskGrids(TaskCur,_listTaskNotes,new List<long>() { signalNum });
		}
		...
		public bool SaveCopy(List<long> listTaskListNums) {
			for(int i=0;i< listTaskListNums.Count;i++) {
				...
				if(taskCopy.TaskListNum==Userods.GetInbox(Security.CurUser.UserNum)) {//My inbox.
					FormTaskEdit formTaskEdit=new Helianz.FormTaskEdit(taskCopy);//Maintain previous behavior. If I send to myself, should popup.
