	...
	public partial class FormClaimEdit : FormODBase {
		...
		private void FormClaimEdit_Load(object sender, System.EventArgs e) {
			...
			if(insPlan.PlanType=="p"){//ppo
				...
				butPayProc.Enabled=false;//Only By Total payments should be allowed => https://www.helianz.com/manual/plancapitation.html
			}
		...
		private void comboClaimStatus_SelectionChangeCommitted(object sender,EventArgs e) {
			...
			if(_claim.ClaimType=="PreAuth") {//we're in a PreAuth and trying to change the status from the dropdown
				//https://helianz.com/manual/preauth.html - See the "Receive a preauthorization" section.
...
//fix: need to debug the recalculation feature to take this status into account.
		...
		private void SendClaim() {
			...
			if(ODEnvironment.IsCloudServer && Clearinghouses.IsDisabledForWeb(clearinghouseClin)) {
				MessageBox.Show(Lans.g("Eclaims","This clearinghouse is not available while using Helianz Cloud."));
				return;
			}
			//string warnings;
			//string missingData=
		...
		private bool SendClaimCanadian(Clearinghouse clearinghouseClin,ClaimSendQueueItem claimSendQueueItem) {
			...
			//However, Application.DoEvents() gets invoked every 2/10ths of a second while the synchronous method is waiting in order to not let Helianz go 'Not Responding'.
		...
		private void OpenAttachmentForm(FormClaimAttachment formClaimAttachment) {
			...
			if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.WaitingToSend)) {//Waiting to send claim
				...
				//There is no benefit to the user by attaching and sending files to DXC if the carrier cannot send an electronic claim within Helianz.
		...
		private void SaveCleanup(){
			...
			if(comboClaimStatus.SelectedIndex==_listClaimStatuses.IndexOf(ClaimStatus.Received) && _isPaymentEntered) {
				...
				//Try to transfer any patient payment around on the procedures associated to this claim if necessary (e.g. if Helianz guessed deductible incorrectly).
		...
		private void FormClaimEdit_FormClosing(object sender, CancelEventArgs e) {
			...
			//Check to see if the claim has already been deleted by another instance of Helianz.
