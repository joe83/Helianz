...
namespace Helianz{
	...
	public partial class FormScreenGroupEdit:FormODBase {
		...
		private List<HelianzBusiness.Screen> _listScreens;
		...
		private void FillGrid() {
			...
			GridRow row;
			List<HelianzBusiness.Screen> listScreensToDelete=new List<HelianzBusiness.Screen>();
		...
		private void FillScreenPats() {
			...
			for(int i=0;i<_listScreenPats.Count();i++) {
				...
				HelianzBusiness.Screen screen=_listScreens.FirstOrDefault(x => x.ScreenPatNum==_listScreenPats[i].ScreenPatNum);
		...
		private void AddAnonymousScreens() {
			while(true) {
				...
				if(_listScreens.Count==0) {
					formScreenEdit.ScreenCur=new HelianzBusiness.Screen();
		...
		private void AddAnonymousScreensForSheets() {
			...
			HelianzBusiness.Screen screen=new HelianzBusiness.Screen();
		...
		private void ViewScreenForPat(HelianzBusiness.Screen screenCur) {
			using FormScreenEdit formScreenEdit=new FormScreenEdit();
			formScreenEdit.ScreenGroupCur=_screenGroup;
		...
		private void ViewScreenForPatWithSheets(HelianzBusiness.Screen screenCur) {
