...
namespace Helianz {
	...
	public partial class FormEServicesEConnector:FormODBase {
		///<summary>The background color used when the HelianzCustListener service is down.  Using Red was deemed too harsh.</summary>
		private Color _colorCritBackground=Color.OrangeRed;
		///<summary>The text color used when the HelianzCustListener service is down.</summary>
		private Color _colorCritText=Color.Yellow;
		///<summary>The background color used when the HelianzCustListener service has an error that has not be processed.</summary>
		private Color _colorErrBackground=Color.LightGoldenrodYellow;
		///<summary>The text color used when the HelianzCustListener service has an error that has not be processed.</summary>
		...
		private void FormEServicesEConnector_Load(object sender,EventArgs e) {
			...
			ODException.SwallowAnyException(() => {
				if(ServicesHelper.GetServicesByExe("HelianzEConnector.exe").Count>0) {
		...
		private void butStartListenerService_Click(object sender,EventArgs e) {
			...
			try {
				listServiceControllers=ServicesHelper.GetServicesByRegistryImagePath("HelianzEConnector.exe");
			}
			catch(Exception ex) {
