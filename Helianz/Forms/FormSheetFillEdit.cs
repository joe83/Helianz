	...
	public partial class FormSheetFillEdit:FormODBase {
		...
		private void butUnlock_Click(object sender,EventArgs e) {
			...
			if(SheetCur.SheetType==SheetTypeEnum.PaymentPlan) {
				...
				for(int i=0;i<panelMain.Controls.Count;i++){
					...
					Helianz.UI.SignatureBoxWrapper sigBox=(Helianz.UI.SignatureBoxWrapper)panelMain.Controls[i];
					sigBox.Enabled=true;
				}
			...
			if(SheetCur.SheetType==SheetTypeEnum.PaymentPlan) {
				...
				if(payPlan.Signature!="" && (payPlan.Signature!=null || !payPlan.IsNew)) {
					for(int i=0;i<panelMain.Controls.Count;i++){
						...
						Helianz.UI.SignatureBoxWrapper signatureBoxWrapper=(Helianz.UI.SignatureBoxWrapper)panelMain.Controls[i];
		...
		private void panelMain_Paint(object sender,PaintEventArgs e) {
			...
			for(int i=0;i<SheetCur.SheetFields.Count;i++){
				...
				HelianzBusiness.SheetPrinting.DrawFieldSpecial(SheetCur,SheetCur.SheetFields[i],g,null,0);
			}
			//Draw pagebreak
		...
		private string LayoutFields() {
			...
			for(int i=0;i<SheetCur.SheetFields.Count;i++){
				...
				Helianz.UI.SignatureBoxWrapper signatureBoxWrapper=new Helianz.UI.SignatureBoxWrapper();
		...
		private void LoadImageOne(SheetField sheetField){
			...
			if(sheetField.FieldName=="Patient Info.gif") {
				sheetField.BitmapLoaded=HelianzBusiness.Properties.Resources.Patient_Info;
				return;
			}
		...
		private bool TryToSaveData() {
			...
			for(int i=0;i<panelMain.Controls.Count;i++){
				if(panelMain.Controls[i].GetType()!=typeof(Helianz.UI.SignatureBoxWrapper)) {
					continue;
				}
				if(panelMain.Controls[i].Tag==null) {
					continue;
		...
		private bool VerifyRequiredFields(){
			...
			for(int i=0;i<_listSignatureBoxWrappers.Count;i++) {
				...
				Helianz.UI.SignatureBoxWrapper sigBox= _listSignatureBoxWrappers[i];
		...
		private void SaveSignaturePayPlan() {
			...
			if((payPlan.IsNew || hasSigChanged) && isValidSig) {
				...
				//create doc--------------------------------------------------------------------------------------
				HelianzBusiness.Document document=null;
				try {
