...
namespace Helianz {
	...
	public partial class FormEhrTimeSynch:FormODBase {
		...
		///<summary>Set true when launched while Helianz starts.  Will automatically check times and close form silently if server time is in synch with local time.</summary>
		...
		///<summary>Called from FormHelianz.Load.  Updates local time and checks to see if server time is in synch, with a fast db call (only acurate to seconds, not miliseconds).</summary>
		...
		private void refreshDisplays() {
			...
			if(!LocalInSynch()) { //This should not happen, time is updated automatically. If you get to this point, you should have already had a message box pop up saying there was an error updating your local time.
				textMessage.Text="Your local machine time is out of sync.  Please ensure Helianz is running with Administrator Windows privileges.  If you have done this and you still see this message, please call Helianz support.";
			}
			else if(!ServerInSynch()) {
				textMessage.Text="Your database time is out of sync with your local machine.  Please start the Helianz application on your database server and leave it running to keep times synchronized as required for EHR compliance.  If you have done this and you still see this message, please call Helianz support.";
			}
