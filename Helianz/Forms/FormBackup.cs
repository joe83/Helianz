...
namespace Helianz {
	public partial class FormBackup : FormODBase {
		...
		private void FormBackup_Load(object sender, System.EventArgs e) {
			...
			#region Backup Tab
			//usesInternalImages=(PrefC.GetString(PrefName.ImageStore)=="Helianz.Imaging.SqlStore");
      ...
      CDT.Class1.Encrypt(textArchivePass.Text,out encryptedPassword);
		...
		private void butBackup_Click(object sender, System.EventArgs e) {
			...
			if(!Directory.Exists(ODFileUtils.CombinePaths(textBackupFromPath.Text,dbName))){// C:\mysql\data\helianz
		...
		private void InstanceMethodDatabaseBackup(string databaseName,string backupFromPath,string backupToPath,bool isInnoDb,out string msg) {
			...
			if(Directory.Exists(dbBackupToPath)) {// D:\helianz
				int loopCount=1;
		...
		private void InstanceMethodAtoZBackup(string backupToPath, string aToZDirectory,string aToZFullPath, double aToZSize) {
			if(!Directory.Exists(ODFileUtils.CombinePaths(backupToPath,aToZDirectory))) {// ex. D:\HelianzData
				try {
					Directory.CreateDirectory(ODFileUtils.CombinePaths(textBackupToPath.Text,aToZDirectory));// ex. D:\HelianzData
				}
				catch {
			...
			_amtCopied = 0;
			CopyDirectoryIncremental(ODFileUtils.CombinePaths(aToZFullPath,""),// C:\HelianzData\
				ODFileUtils.CombinePaths(new string[] { backupToPath,aToZDirectory,"" }),// D:\HelianzData\
				aToZSize);
		}
		...
		private void InstanceMethodRestore(string aToZFullPath, string restoreFromPath){
			string aToZDirectory=aToZFullPath.Substring(aToZFullPath.LastIndexOf(Path.DirectorySeparatorChar)+1);// HelianzData
			long aToZSize=GetFileSizes(ODFileUtils.CombinePaths(new string[] {restoreFromPath,aToZDirectory,""}),
				ODFileUtils.CombinePaths(aToZFullPath,""))/1024;// C:\HelianzData\
			if(!Directory.Exists(aToZFullPath)){// C:\HelianzData\
				Directory.CreateDirectory(aToZFullPath);// C:\HelianzData\
			}
			_amtCopied = 0;
			CopyDirectoryIncremental(ODFileUtils.CombinePaths(new string[] {restoreFromPath,aToZDirectory,""}),
				ODFileUtils.CombinePaths(aToZFullPath,""),// C:\HelianzData\
				aToZSize);
		}
		...
		private void butRestore_Click(object sender, System.EventArgs e) {			
			...
			if(!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreFromPath.Text,dbName))){// D:\helianz
				MessageBox.Show(Lan.g(this,"Restore FROM path is invalid.  Unable to find folder named ")+dbName);
				return;
			}
			if(!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreToPath.Text,dbName))) {// C:\mysql\data\helianz
				MessageBox.Show(Lan.g(this,"Restore TO path is invalid.  Unable to find folder named ")+dbName);
				return;
			}
			if(ShouldUseAtoZFolder()) {
				if(!Directory.Exists(textBackupRestoreAtoZToPath.Text)) {// C:\HelianzData\
					MsgBox.Show(this,"Restore A-Z images TO path is invalid.");
					return;
				}
				string aToZFullPath=textBackupRestoreAtoZToPath.Text;// C:\HelianzData\
				//remove the trailing \
				aToZFullPath=aToZFullPath.Substring(0,aToZFullPath.Length-1);// C:\HelianzData
				string aToZDirectory=aToZFullPath.Substring(aToZFullPath.LastIndexOf(Path.DirectorySeparatorChar)+1);// HelianzData
				if(!Directory.Exists(ODFileUtils.CombinePaths(textBackupRestoreFromPath.Text,aToZDirectory))){// D:\HelianzData
					MsgBox.Show(this,"Restore A-Z images FROM path is invalid.");
					return;
				}
			}
			string fromPath=ODFileUtils.CombinePaths(new string[] {textBackupRestoreFromPath.Text,dbName,""});// D:\helianz\
		...
		private void butArchive_Click(object sender,EventArgs e) {
			...
			if(checkArchiveDoBackupFirst.Checked) { //We only need to validate the backup settings if the user wants to make a backup first
				if(!MsgBox.Show(MsgBoxButtons.YesNo,"To make a backup of the database, ensure no other machines are currently using Helianz. Proceed?")) {
					return;
				}
				//Validation
			...
			try{
				...
				FriendlyException.Show("An error occurred backing up the old database. Old data was not removed from the database. "+
					"Ensure no other machines are currently using Helianz and try again.",ex);
				return;
			}
