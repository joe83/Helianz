...
namespace Helianz{
	...
	public partial class FormApptEdit:FormODBase {
		...
		private void CalculatePatternFromProcs(bool ignoreTimeLocked = false) {
			...
			if(!IsNew && !HelianzBusiness.Security.IsAuthorized(HelianzBusiness.EnumPermType.AppointmentResize,suppressMessage:true)) {
				timeLocked=true;
		...
		private bool UpdateListAndDB(bool isClosing = true,bool doCreateSecLog = false,bool doInsertHL7 = false) {
			...
			//Also need to see if there is a generic HL7 def enabled where Helianz is not the filler application.
			//Helianz is the filler application if appointments, schedules, and operatories are maintained by Helianz and messages are sent out
			//to inform another software of any changes made.  If Helianz is an auxiliary application, appointments are created from inbound SIU
			//messages and Helianz no longer has control over whether the appointments overlap or which operatory/provider's schedule the appointment
