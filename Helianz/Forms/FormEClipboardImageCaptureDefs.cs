		...
		private void butRight_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<listDefsSelected.Count;i++) {
				...
				if(listDefsSelected[i].DefNum==0) {
					...
					listDefsSelected[i].DefNum=HelianzBusiness.Defs.GetDefsForCategory(HelianzBusiness.DefCat.ImageCats,true).FirstOrDefault(x => x.ItemValue.Contains("P"))?.DefNum??0;
				}
				EClipboardImageCaptureDef eClipboardImageCaptureDef=new EClipboardImageCaptureDef();
