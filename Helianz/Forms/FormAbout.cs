	...
	public partial class FormAbout : FormODBase{
		...
		private void FormAbout_Load(object sender, System.EventArgs e) {
			...
			if(Programs.GetCur(ProgramName.BencoPracticeManagement).Enabled) {
				pictureHelianz.Image=Properties.Resources.bencoLogo;
			}
			if(softwareName!="Helianz Software" && !Programs.GetCur(ProgramName.BencoPracticeManagement).Enabled) {
				pictureHelianz.Visible=false;
			}
		...
		private void butDiagnostics_Click(object sender,EventArgs e) {
			string diagnostics = BugSubmissions.GetDiagnostics(FormHelianz.PatNumCur);
