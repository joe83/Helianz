...
namespace Helianz {
	public partial class FormOIDRegistryInternal:FormODBase {
		...
		private void butRetrieveOIDs_Click(object sender,EventArgs e) {
			...
			xmlWriter.Close();
#if DEBUG
			Helianz.localhost.Service1 OIDService=new Helianz.localhost.Service1();
#else
			Helianz.customerUpdates.Service1 OIDService=new Helianz.customerUpdates.Service1();
			...
			if(existsCustomOIDs && !MsgBox.Show(this,MsgBoxButtons.YesNo,"Would you like to update all OIDs using the root provided by Helianz?")) {
				FillGrid();
				return;
			}
			for(int i=1;i<_listOIDInternals.Count;i++) {//start with 1, root is 0 and is updated above
