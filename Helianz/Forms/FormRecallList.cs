...
namespace Helianz {
	...
	public partial class FormRecallList:FormODBase {
		...
		///<summary>Indicates whether the Reg Key is currently on Helianz support.</summary>
		...
		private void grid_CellClick(object sender,Helianz.UI.ODGridClickEventArgs e) {
			SetFamilyColors();
		}
		...
		private void butEmail_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<_tableAddress.Rows.Count;i++){
				...
				Helianz.UI.ComboBox cbEmail=isRecallGridSelected?comboEmailFromRecalls:comboEmailFromReact;
		...
		private void pdLabels_PrintPage(object sender, PrintPageEventArgs ev){
			...
			while(yPos<1000 && patientsPrinted<_tableAddress.Rows.Count){
				...
				Helianz.InternalTools.Phones.MapPanel.FitTextOld(text,new Font(FontFamily.GenericSansSerif,11),Brushes.Black,rectangle,new StringFormat(),g);
		...
		private void butGotoFamily_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.GoToModule(EnumModuleType.Family, patNum:_patNumCur);
		}
		...
		private void butGotoAccount_Click(object sender,EventArgs e) {
			...
			WindowState=FormWindowState.Minimized;
			GlobalFormHelianz.GoToModule(EnumModuleType.Account,patNum:_patNumCur);
		}
...
#region printHeading
		...
		private void butSched_Click(object sender,EventArgs e) {
			...
			if(isRecall && listAppointmentsPinned[listAppointmentsPinned.Count-1].AptDateTime>=DateTime.Today) { //we're dealing with a future recall(s), so pin it and jump to it's date
				GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:listAppointmentsPinned.Select(x=>x.AptNum).ToList(), patNum:_patNumCur,dateSelected:listAppointmentsPinned[listAppointmentsPinned.Count-1].AptDateTime);
			}
			else { //it's a reactivation / it's a recall that is overdue or unsafe to jump to, so just pin it and jump to today's date
				GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:listAppointmentsPinned.Select(x=>x.AptNum).ToList(), patNum:_patNumCur, dateSelected:DateTime.Today);
			}
