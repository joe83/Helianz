		...
		private void butMerge_Click(object sender,EventArgs e) {
			...
			{//mismatch
				...
				if(Programs.UsingEcwTightOrFullMode()) {
					msgText+="\r\n"+Lan.g(this,"The patients must first be merged from within eCW, then immediately merged in the same order in Helianz.  "
						+"If the patients are not merged in this manner, some information may not properly bridge between eCW and Helianz.");
				}
				msgText+="\r\n\r\n"
			...
			if(isSuccessfulMerge) {
				...
				FormHelianz.S_HandleRefreshedTasks(listSignals,listTasksPatient.Select(x => x.TaskNum).ToList(),listTasksPatient,new List<TaskNote>()
					,new List<UserOdPref>());
				#endregion
				...
				if(FormHelianz.PatNumCur==_patientFrom.PatNum) {
					GlobalFormHelianz.PatientSelected(_patientTo,true);
				}
