...
///<summary></summary>
	public partial class FormConfirmList : FormODBase {
		...
		private void SelectPatient_Click() {
			...
			GlobalFormHelianz.PatientSelected(patient,isRefreshCurModule:true);
		}
		...
		private void SeeChart_Click() {
			...
			long patNum=PIn.Long(_tableAppointments.Rows[gridMain.SelectedIndices[gridMain.SelectedIndices.Length-1]]["PatNum"].ToString());
			Patient patient=Patients.GetPat(patNum);
			GlobalFormHelianz.PatientSelected(patient,isRefreshCurModule:false);
			GlobalFormHelianz.GoToModule(EnumModuleType.Chart,patNum:patient.PatNum);
		}
		...
		private void SendPinboard_Click() {
			...
			GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:listAptNumsSelected,dateSelected:DateTime.Today);
		}
		...
		private void grid_CellDoubleClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
			Cursor=Cursors.WaitCursor;
		...
		private void pdLabels_PrintPage(object sender, PrintPageEventArgs ev){
			...
			while(yPos<1000 && _patientsPrinted<_tableAddresses.Rows.Count){
				...
				Helianz.InternalTools.Phones.MapPanel.FitTextOld(text,font,Brushes.Black,rectangle,new StringFormat(),g);
