	...
	public partial class FormQueryMonitor:FormODBase {
		...
		private void ButLog_Click(object sender,EventArgs e) {
			...
			if(ODEnvironment.IsCloudServer) {
				MsgBox.Show(this,"Logging not supported while using Helianz Cloud.");
				return;
			}
			...
			string logFileName="";
			if(!HelianzBusiness.FileIO.FileAtoZ.DirectoryExistsRelative(logFolderPath)) {
				try {
					//Create the query monitor log folder in the AtoZ image path.
					HelianzBusiness.FileIO.FileAtoZ.CreateDirectoryRelative(logFolderPath);
				}
			...
			try{
				while(HelianzBusiness.FileIO.FileAtoZ.ExistsRelative(logFolderPath,logFileName)) {
					logFileName=$"QueryMonitorLog_{DateTime.Now.ToString("yyyyMMdd_HHmmss")}.txt";
					Thread.Sleep(100);
				}
			}
			...
			try{
			HelianzBusiness.FileIO.FileAtoZ.WriteAllTextRelative(logFolderPath,logFileName,
