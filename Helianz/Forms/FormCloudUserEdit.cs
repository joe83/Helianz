		...
		private bool SyncGroups(List<CloudGroup> listNew,List<CloudGroup> listOld) {
			...
			for(int i=0;i<listRemove.Count;i++) {
				if(!CanRemoveUserFromGroup(listRemove[i].GroupId)) {
					MsgBox.Show(Lan.g(this,"This user is the last user in the group")+" "+listRemove[i].DisplayName+". "+Lan.g(this,"You cannot remove the last user from a group or you will lose all access to the Helianz Cloud application assigned to that group."));
					return false;
				}
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			if(listGroupIdsLastUser.Count>0) {
				...
				msgText=Lan.g(this,"This user is the last user in the group(s)")+":\r\n"+string.Join("\r\n",listGroupDisplayNames)+"\r\n"
					+Lan.g(this,"You cannot remove the last user from a group or you will lose all access to the Helianz Cloud application assigned to that group.");
				MsgBox.Show(msgText);
				return;
			}
			msgText="Deleting this user will remove their access to the Helianz Cloud application. This action cannot be undone. Continue?";
