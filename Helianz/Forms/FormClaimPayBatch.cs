...
///<summary></summary>
	public partial class FormClaimPayBatch:FormODBase {
		...
		private void menuItemGotoAccount_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.PatientSelected(pat,isRefreshCurModule:false);
			GlobalFormHelianz.GoToModule(EnumModuleType.Account, claimNum:GotoClaimNum);
		}
		...
		private bool IsAmountAndTotalEqual(bool isSilent=false) {
			List<ClaimPaySplit> listClaimPaySplitsAttached=(List<ClaimPaySplit>)gridAttached.Tag;
			// There are instances that the check amount when this form is launched gets modified by another instance of Helianz and the amount when the form launched
