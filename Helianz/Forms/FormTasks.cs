...
namespace Helianz{
	...
	public partial class FormTasks:FormODBase {
		...
		private void userControlTasks1_GoToChanged(object sender,EventArgs e) {
			...
			if(taskObjectTypeGoto==TaskObjectType.Patient){
				if(gotoKeyNum!=0){
					...
					if(_isTriage) {
						...
						GlobalFormHelianz.GoToModule(EnumModuleType.Account,patNum:patient.PatNum);
					}
				}
			}
			...
			//OnPatientSelected(pat);
			GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, dateSelected:dateTimeSelected,selectedAptNum:appointment.AptNum);
