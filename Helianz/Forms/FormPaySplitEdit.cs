...
namespace Helianz {
	...
	public partial class FormPaySplitEdit : FormODBase {
		...
		private void FillPatient(){
			...
			//this can happen if it is a new payment split or user unchecks the "Is From Other Fam" box. Need to reset and select the patient currently selected in Helianz if possible.
			if(PaySplitCur.PatNum==0){
				...
				for(int i=0;i<_family.ListPats.Length;i++) {//This is the same order added to listPatient.
					if(FormHelianz.PatNumCur==_family.ListPats[i].PatNum) {
						listPatient.SelectedIndex=i;
