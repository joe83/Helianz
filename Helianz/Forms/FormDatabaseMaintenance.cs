	...
	public partial class FormDatabaseMaintenance:FormODBase {
		...
		private void FamilyBalanceFix(bool isSingleFamilyMode=false) {
			...
			if(isSingleFamilyMode) {
				...
				formFamilyBalancer.PatNum=FormHelianz.PatNumCur;
			}
		...
		private void Run(GridOD grid,DbmMode dbmModeCur) {
			...
			if(PrefC.GetBool(PrefName.DatabaseMaintenanceSkipCheckTable)) {//Display informative message that table checks will not be run.
				...
				string warningMessage="CHECK TABLES will be run first.  This will take an estimated "+strTotalTime+" and may limit users from accessing Helianz while this action runs. Would you like to continue?";
