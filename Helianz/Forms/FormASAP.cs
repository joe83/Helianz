...
namespace Helianz {
	...
	public partial class FormASAP:FormODBase {
		...
		private void SendPinboard_Click() {
			...
			GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:listAptNums,dateSelected:DateTime.Today);//Pins all appointments to the pinboard that were in listAptNums.
		}
		...
		private void MakeAppointment_Click() {
			...
			for(int i=0;i<listRecallsSelected.Count;i++) {
				...
				GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:new List<long>() { appointment.AptNum }, patNum:patient.PatNum, dateSelected:DateTime.Today);
			}
			FillGridRecalls();
		}
		...
		private void SeeChart_Click(long patnum) {
			...
			GlobalFormHelianz.PatientSelected(patient,isRefreshCurModule: false); //Selects the patient in Helianz.
