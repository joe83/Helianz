...
namespace Helianz {
	public partial class FormSmsTextMessaging:FormODBase {
		...
		///<summary>Set from FormHelianz.  This can be null if the calling code does not wish to get dynamic unread message counts.
		///Allows FormSmsTextMessaging to update the unread SMS text message count in FormHelianz as the user reads their messages.</summary>
		...
		private void butPatCurrent_Click(object sender,EventArgs e) {
			_patNum=(FormHelianz.PatNumCur==0 ? -1 : FormHelianz.PatNumCur);
			if(_patNum==-1) {
				textPatient.Text="";
			}
			else {
		...
		private void menuItemSelectPatient_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.PatientSelected(Patients.GetPat(GetSelectedPatNum()),true);
		}
