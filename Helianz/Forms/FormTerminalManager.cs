...
namespace Helianz {
	...
	public partial class FormTerminalManager:FormODBase {
		...
		private void FormTerminalManager_Load(object sender,EventArgs e) {
			...
			butByod.Enabled=_appointment!=null && HelianzBusiness.AutoComm.Byod.IsEnabledForConfirmed(_appointment.Confirmed,_appointment.ClinicNum,out string error);
		}
				...
				void CellClick(object sender,EventArgs e) {
					...
					if(sheetDeviceCurrent.GetPatNum()==0) { //we are trying to load the patient
						if(FormHelianz.PatNumCur==0) {
							MsgBox.Show(this,"There is currently no patient selected to send to the device Select a patient in Helianz " +
						...
						if(sheetDeviceCurrent.IsKiosk()) { //kiosk only
							...
							if(MobileAppDevices.PatientIsAlreadyUsingDevice(FormHelianz.PatNumCur)) {
							...
							Appointment apptForToday=Appointments.GetAppointmentsForPat(FormHelianz.PatNumCur).FirstOrDefault(x => x.AptDateTime.Date==DateTime.Today.Date);
						...
						sheetDeviceCurrent.SetPatNum(FormHelianz.PatNumCur);
					}
		...
		private void FillPat() {
			...
			if(sheetDevice.GetPatNum()==0) {
				...
				if(FormHelianz.PatNumCur==0) {
					...
					labelPatient.Text=Patients.GetLim(FormHelianz.PatNumCur).GetNameLF();
					butPatForms.Enabled=true;
					Sheets.GetForTerminal(FormHelianz.PatNumCur).ForEach(x => listSheets.Items.Add(x.Description,x));
				}
			}
			else {
		...
		private void butPatForms_Click(object sender,EventArgs e) {
			...
			if(formPatientForms.PatNum==0){
				formPatientForms.PatNum=FormHelianz.PatNumCur;
			}
