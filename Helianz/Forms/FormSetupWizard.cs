...
namespace Helianz {
	public partial class FormSetupWizard:FormODBase {
		...
		private void butAll_Click(object sender,EventArgs e) {
			List<SetupWizard.SetupWizClass> listSetupWizClasses = new List<Helianz.SetupWizard.SetupWizClass>();
			for(int i=0;i<_listSetupWizClassesItems.Count();i++) {
		...
		private void butSelected_Click(object sender,EventArgs e) {
			...
			for(int i=0;i<gridMain.SelectedIndices.Count();i++) {
				...
				if(gridRowSelected.Tag.GetType().BaseType != typeof(Helianz.SetupWizard.SetupWizClass)) {
					continue;
				}
