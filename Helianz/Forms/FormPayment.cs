...
using PayConnectService = HelianzBusiness.PayConnectService;
using HelianzBusiness.WebTypes.Shared.XWeb;
using PdfSharp.Pdf;
using EdgeExpressProps = HelianzBusiness.ProgramProperties.PropertyDescs.EdgeExpress;
using Bridges;
using static HelianzBusiness.PayConnect2;
	...
	public partial class FormPayment:FormODBase {
		...
		private bool HasPayConnect() {
			...
			if(program.Enabled) {
				...
				ODException.SwallowAnyException(() =>
					Process.Start("http://www.helianz.com/resources/redirects/redirectpayconnect.html")
				);
				return false;
			}
			if(isSetupRequired) {
		...
		private bool HasPaySimple() {
			...
			if(program.Enabled) {
				...
				ODException.SwallowAnyException(() =>
					Process.Start("http://www.helianz.com/resources/redirects/redirectpaysimple.html")//
				);
				return false;
			}
			if(isSetupRequired) {
		...
		private bool HasXCharge() {
			...
			if(_programX.Enabled) {
				...
				ODException.SwallowAnyException(() =>
					Process.Start("http://www.helianz.com/resources/redirects/redirectopenedge.html")
				);
				return false;
			}
		...
		private void ToggleShowHideSplits() {
			...
			this.butShowHide.Image = global::Helianz.Properties.Resources.arrowUpTriangle;
		}
		...
		private void VoidXChargeTransaction(string transID,string amount,bool isDebit) {
			...
			Cursor=Cursors.Default;
			//Next, record the voided payment within Helianz.  We use to delete the payment but Nathan wants us to negate voids with another payment.
			string resultText="";
			string line="";
			bool showApprovedAmtNotice=false;
		...
		private void XWebVoid() {
			...
			Cursor=Cursors.WaitCursor;
			string payNote=Lan.g(this,"Void XWeb payment made from within Helianz");
			try {
		...
		public string MakeXChargeTransaction(double prepaidAmt = 0) {
			if(ODEnvironment.IsCloudServer) {
				MsgBox.Show(this,"XCharge is not available while using Helianz Cloud.");
				return null;
			}
