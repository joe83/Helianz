...
namespace Helianz {
	...
	public partial class FormBilling : FormODBase {
		...
		private void menuItemGoTo_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.PatientSelected(Patients.GetPat(patNum),false);
			GlobalFormHelianz.GoToModule(EnumModuleType.Account,patNum:0);
			SendToBack();
		}
		...
		private void butSend_Click(object sender,System.EventArgs e) {
				...
				)) 
			{
				MsgBox.Show(this,$"Electronic statements using {electronicBillingType.GetDescription()} "
					+"are not available while using Helianz Cloud.");
				return;
			}
			if(gridBill.SelectedIndices.Length==0){
			...
			sendStatementsIO.FuncSendEhgStatement=(xmlData,clincNum) => {
				HelianzBusiness.Bridges.EHG_statements.Send(xmlData,sendStatementsIO.CurStatementBatch.ClinicNum,out string alertMsg);
				return alertMsg;
			};
