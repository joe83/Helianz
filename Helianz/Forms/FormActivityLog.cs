...
namespace Helianz {
	public partial class FormActivityLog:FormODBase {
			...
			for(int i = 0;i<listEServiceLogs.Count;i++) {
				...
				if(PrefC.HasClinicsEnabled) {
					if(listEServiceLogs[i].ClinicNum==0) {
						...
						row.Cells.Add(HelianzBusiness.Clinics.GetClinic(listEServiceLogs[i].ClinicNum).Abbr);
					}
				}
		...
		private void butRefresh_Click(object sender,EventArgs e) {
			_listEServiceLogs=HelianzBusiness.EServiceLogs.GetEServiceLog(comboBoxClinicMulti.ClinicNumSelected,datePicker.GetDateTimeFrom(),datePicker.GetDateTimeTo());
			FillGrid();
		}
