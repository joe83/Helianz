...
namespace Helianz{
	public class AppointmentL {
		...
		public static void CopyAptToPinboardHelper(Appointment appointment) {
			GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:new List<long>(){ appointment.AptNum }, patNum:appointment.PatNum, dateSelected:DateTime.Today);
		}
		...
		public static PlannedApptStatus CreatePlannedAppt(Patient patient,int itemOrder,List<long> listPreSelectedProcNums=null) {
			...
			if(patientMerged!=null) {
				...
				GlobalFormHelianz.PatientSelected(patient,isRefreshCurModule: true,isApptRefreshDataPat: false);
			}
			...
			GlobalFormHelianz.RefreshCurrentModule(isClinicRefresh:false);//if procs were added in appt, then this will display them
		...
		public static void ShowKioskManagerIfNeeded(Appointment appointmentOld,long defNumConfirmedNew) {
			...
			HelianzBusiness.AutoComm.Arrivals.ProcessArrival(appointmentOld.PatNum,appointmentOld.ClinicNum,ListTools.FromSingle(appointmentOld));
		}
		...
		public static void SendByodLink(Appointment appointment) {
			void displayError(string error) {
				MsgBox.Show(nameof(HelianzBusiness.AutoComm.Byod),$"Unable to {MenuItemNames.SendEClipboardByod}.  "+error);
			}
			string message=null;
			UI.ProgressWin progressOD=new UI.ProgressWin();
			progressOD.ActionMain=() => {
				if(appointment is null) {
					MessageBox.Show(Lan.g(nameof(HelianzBusiness.AutoComm.Byod),"Unable to send for an invalid appointment."));
					return;
				}
				long clinicNum=0;
				if(PrefC.HasClinicsEnabled){
				...
				if(HelianzBusiness.AutoComm.Byod.IsEnabledForConfirmed(appointment.Confirmed,appointment.ClinicNum,out string err)) {
					...
					if(patComm==null){
						MessageBox.Show(Lan.g(nameof(HelianzBusiness.AutoComm.Byod),"Patient is not setup to receive text messages."));
						return;
					}
					...
					message=HelianzBusiness.AutoComm.Byod.GetCheckInMsg(new List<Appointment> { appointment },listPatComms);
				}
				else {
					MessageBox.Show(err);
					return;
			...
			progressOD.StartingMessage=Lans.g(nameof(HelianzBusiness.AutoComm.Byod),"Generating eClipboard links...");
			progressOD.ShowDialog();
			if(progressOD.IsCancelled){
				return;
			}
			if(string.IsNullOrWhiteSpace(message)) {
				MessageBox.Show(Lan.g(nameof(HelianzBusiness.AutoComm.Byod),"Unable to generate links for the appointment."));
				return;
			}
