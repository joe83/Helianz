...
namespace Helianz {
	public class PrintRemoteRequestL {
		...
		public static bool ProcessRemotePrintRequest(PrintRemoteRequest printRemoteRequest){
			...
			switch(printRemoteRequest.PrintRequestType){
				...
				case EnumPrintRequestType.Sheet:
					...
					HelianzBusiness.Statement statement=Statements.GetStatement(statementNum);
					if(statement==null){
						//Attempt to print.
		...
		#region Sheets
		public static void TryPrintSheet(long sheetNum,bool isStatement,bool IsRxControlled,HelianzBusiness.Statement StatementCur,MedLab MedLabCur,DataSet dataSet,long printerNumOverride=0){
