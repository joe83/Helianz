...
namespace Helianz {
	public class PrefL {
		...
		public static bool ConvertDB(bool isSilent,string toVersion,Form form,bool useDynamicMode) {
			...
			if(FormHelianz.ExitCode==0) {
				FormHelianz.ExitCode=200;//Convert Database has failed during execution (Unknown Error)
			}
			Environment.Exit(FormHelianz.ExitCode);
			return false;
		}
		...
		public static bool CopyFromHereToUpdateFiles(Version versionCurrent,bool isSilent,bool hasAtoZ,bool hasConcatFiles) {
			...
			if(hasAtoZ && PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
				...
				if(prefImagePath==null || !Directory.Exists(prefImagePath)) {//AtoZ folder not found
					if(isSilent) {
						FormHelianz.ExitCode=300;//AtoZ folder not found (Warning)
						return false;
					}
		...
		public static bool CopyFilesToDatabase(bool hasAtoZ,bool isSilent,Version versionCurrent,bool hasConcatFiles) {
			...
			if(!DeleteFolder(folderAtoZUpdateFiles) | !DeleteFolder(folderTempUpdateFiles)) {//Logical OR to prevent short circuit.
				FormHelianz.ExitCode=301;//UpdateFiles folder cannot be deleted (Warning)
				if(!isSilent) {
			...
			if(!CopyInstallFilesToPath(folderAtoZUpdateFiles,versionCurrent) | !CopyInstallFilesToPath(folderTempUpdateFiles,versionCurrent)) {
				FormHelianz.ExitCode=302;//Installation files could not be copied.
				if(!isSilent) {
				...
				if(maxAllowedPacket < maxDefaultAllowedPacketSize) {
					try {
						...
						ODException.SwallowAnyException(() => {
							EventLog.WriteEntry("Helianz","Error updating max_allowed_packet from "+maxAllowedPacket
			...
			try {
				...
				FormHelianz.ExitCode=304;//Error compressing UpdateFiles
				if(!isSilent) {
			...
			if(hasConcatFiles) {
				...
				try {
					...
					Thread.Sleep(300);
					ODException.SwallowAnyException(() => {
						EventLog.WriteEntry("Helianz","Error inserting new update files into database row:\r\n"+ex.Message,
			...
			try {
				...
				try {
					...
					ODException.SwallowAnyException(() => {
						EventLog.WriteEntry("Helianz","Error inserting UpdateFiles into database:"
				...
				FormHelianz.ExitCode=303;//Failed inserting update files into the database.
				if(isSilent) {
					return false;
				}
		...
		private static void CopyFilesToFolder(FileInfo[] fileInfoArrayApp,string folderPath) {
			for(int i=0;i<fileInfoArrayApp.Length;i++){
				...
				if(fileInfoArrayApp[i].Name=="HelianzServerConfig.xml") {
					continue;//skip also
				}
				...
				if(fileInfoArrayApp[i].Name=="HelianzReplicationServiceConfig.xml") {
					continue;
				}
		...
		///<summary>Called from FormHelianz.PrefsStartup. Compares the installed version to the version on the server being connected to.</summary>
		...
		public static bool CheckProgramVersion(Form formCurrent,bool isSilent,ChooseDatabaseInfo chooseDatabaseInfo,CommandLineArgs commandLineArgs) {
			...
			if(PrefC.GetBool(PrefName.UpdateWindowShowsClassicView)) {
				if(isSilent) {
					FormHelianz.ExitCode=399;//Classic View is not supported with Silent Update
					return false;
				}
			...
			if(versionStored>versionCurrent) {
				if(isSilent) {//This should never happen after a silent update.
					FormHelianz.ExitCode=312;//Stored version is higher that client version after an update was successful.
					return false;
				}
				...
				return false;//Regardless if the client update was successful or not we HAVE to restart Helianz so always return false.
			}
					...
					|| (updateServerName=="" && (chooseDatabaseInfo?.UseDynamicMode??false))))
			{
				if(isSilent) {//This should have already been thrown but we do it here anyway.
					FormHelianz.ExitCode=141;//Updates are only allowed from a designated web server
					return false;
				}
				...
				return false;//Regardless if the downgrade was successful or not we HAVE to restart Helianz so always return false.
			}
				...
				&& (updateServerName=="" || ODEnvironment.IdIsThisComputer(updateServerName.ToLower()))) 
			{
				if(ODBuild.IsTrial() && PrefC.GetString(PrefName.RegistrationKey)!="") {//Allow databases with no reg key to continue.  Needed by our conversion department.
					...
					FormHelianz.ExitCode=398;//Trial versions does not support updating databases.
				...
				Cache.Refresh(InvalidType.Prefs);
				//Try to install Helianz Service, since this is the computer that matches PrefName.WebServiceServerName.
				...
				//In addition to the service installing, there will also be a new HelianzServiceConfig.xml file that the service will use.  
				//The config data is defaulted to the current connection settings in DataConnection. 
				TryInstallHelianzService(isSilent);
				...
				TryInstallHelianzApiService(isSilent:true);
			}
			if(versionCurrent < versionDB) {
				...
				FormHelianz.ExitCode=(int)FormHelianzExitCodes.DbVersionHigherThanCurrent;//309
				return false;
			}
			return true;
		}

		///<summary>Launches the correct version of Helianz based on the parameters passed in.
		...
		///to the correct version of Helianz when it is launched.</param>
		...
		{
			//The full path to the official installation folder for Helianz.
			...
			if(isInDynamicModeFolder) {
				...
				//If this were to run on the main installation of Helianz, it would continue to try and open itself again and again due to the check
			...
			if(!File.Exists(serverVersionExePath) || FileVersionInfo.GetVersionInfo(serverVersionExePath).ProductVersion!=versionStored.ToString()) {
				...
				try {
					...
					ex.DoNothing();//Swallow silently and shut down Helianz.
					return;
				}
			}
		...
		///<summary>Model can be null, if so then return blank. Otherwise generate command line arguments based on the arguments passed from FormHelianz.FormOpenShown(). 
		...
		public static void DownloadInstallPatchFromURI(string downloadUri,string destinationPath,bool runSetupAfterDownload,bool showShutdownWindow,string destinationPath2) {
			...
			if(CloudStorage.IsCloudStorage) {
				HelianzCloud.Core.TaskStateUpload taskStateUpload=null;
			...
			#region Stop Helianz Services
			//If the update has been initiated from the designated update server then try and stop all "Helianz..." services.
			//They will be automatically restarted once Helianz has successfully upgraded.
			if(PrefC.GetString(PrefName.WebServiceServerName)!="" && ODEnvironment.IdIsThisComputer(PrefC.GetString(PrefName.WebServiceServerName))) {
				...
				List<ServiceController> listServiceControllersHelianz=ServicesHelper.GetAllHelianzServices();
				//Newer versions of Windows have heightened security measures for managing services.
				//We get lots of calls where users do not have the correct permissions to start and stop Helianz services.
				//Helianz services are not important enough to warrent "Admin" rights to manage so we want to allow "Everyone" to start and stop them.
				ServicesHelper.SetSecurityDescriptorToAllowEveryoneToManageServices(listServiceControllersHelianz);
				//Loop through all Helianz services and stop them if they have not stopped or are not pending a stop so that their binaries can be updated.
				string strServicesNotStopped=ServicesHelper.StopServices(listServiceControllersHelianz);
			...
			try {
				...
				FormHelianz.S_ProcessKillCommand();
			}
			catch{
				Prefs.UpdateString(PrefName.UpdateInProgressOnComputerName,"");//unlock workstations, since nothing was actually done.
		...
		public static bool CheckMySqlVersion(bool isSilent) {
			...
			if(versionMySQL < new Version(5,0)) {
				FormHelianz.ExitCode=110;//MySQL version lower than 5.0
				if(!isSilent) {
				...
				if(Prefs.UpdateString(PrefName.MySqlVersion,(thisVersion))) {
					if(!ODBuild.IsDebug()) {
						...
						if(!Shared.BackupRepairAndOptimize(isSilent,BackupLocation.ConvertScript,false)) {
							...
							Prefs.UpdateString(PrefName.MySqlVersion,mysqlVersionPrev);
							FormHelianz.ExitCode=101;//Database Backup failed
							return false;
						}
						hasBackup=true;
					}
		...
		///Uninstalls all HelianzCustListener services that are present. Installs the eConnector service after successfully removing all CustListener services.
			...
			bool isInvalidUpdateServerNameAllowed=false)
		{
			...
			if(ODEnvironment.IsCloudServer) {//We do not want to install in case this is a pre-test cloud database.
				if(!isSilent) {
					MessageBox.Show(Lans.g("ServicesHelper","Not allowed to install the HelianzEConnector service in cloud mode."));
				}
				return false;
			}
		...
		private static bool UninstallCustListenerServices() {
			...
			ODException.SwallowAnyException(() => listServiceControllersCustListener=ServicesHelper.GetServicesByExe("HelianzCustListener.exe"));
		...
		private static bool TryInstallEConnector(out string error) {
			...
			try {
				listServiceControllersEConnector=ServicesHelper.GetServicesByExe("HelianzEConnector.exe");
				pathEConnectorExe=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzEConnector","HelianzEConnector.exe");
			}
			catch(Exception ex) {
			...
			if(!fileInfoEConnectorExe.Exists) {
				error=Lans.g("ServicesHelper","Unable to install the HelianzEConnector service; Cannot find")+$" '{pathEConnectorExe}'";
				return false;
			}			
			if(!ServicesHelper.Install("HelianzEConnector",fileInfoEConnectorExe)) {
					error=Lans.g("ServicesHelper","Unable to install the HelianzEConnector service.");
					return false;
				}
			//Create a new HelianzWebConfig.xml file for the eConnector if one is not already present.
			if(!CreateConfigForEConnector()) {
				error=Lans.g("ServicesHelper","The config file for the HelianzEConnector service could not be created.");
				return false;
			}
			//Now that the service has finally installed we need to try and start it.
			listServiceControllersEConnector=ServicesHelper.GetServicesByExe("HelianzEConnector.exe");
			if(listServiceControllersEConnector.Count < 1) {
				error=Lans.g("ServicesHelper","HelianzEConnector service could not be found in order to automatically start it.");
				return false;
			}
		...
		public static bool TryInstallHelianzApiService(bool isSilent,string updateServerName=null) {
			if(updateServerName==null) {
				updateServerName=PrefC.GetString(PrefName.WebServiceServerName);
			}
			...
			if(ODEnvironment.IsCloudServer) {//We do not want to install in case this is a pre-test cloud database.
				actionDisplayError(Lans.g("ServicesHelper","Not allowed to install the HelianzAPIService in cloud mode."));
				return false;
			}
			if(string.IsNullOrWhiteSpace(updateServerName)) {
			...
			try {
				listServiceControllersAPI=ServicesHelper.GetServicesByExe("HelianzAPIService.exe");
				pathApiServiceExe=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzAPIService","HelianzAPIService.exe");
			}
			catch(Exception ex) {
				actionDisplayError(Lans.g("ServicesHelper","Failed installing the HelianzAPIService:")
					+"\r\n"+ex.Message
					+"\r\n\r\n"+Lans.g("ServicesHelper","Try running as Administrator."));
				return false;
			}
			...
			if(!fileInfoApiExe.Exists) {
				actionDisplayError(Lans.g("ServicesHelper","Unable to install the HelianzAPIService; Cannot find")+$" '{pathApiServiceExe}'");
				return false;
			}
			if(!ServicesHelper.Install("HelianzAPIService",fileInfoApiExe)) {
				actionDisplayError(Lans.g("ServicesHelper","Unable to install the HelianzAPIService."));
				return false;
			}
			//Create a new HelianzWebConfig.xml file for the API service if one is not already present.
			if(!CreateConfigForApi()) {
				actionDisplayError(Lans.g("ServicesHelper","The config file for the HelianzAPIService could not be created."));
				return false;
			}
			//Now that the service has finally installed we need to try and start it.
			listServiceControllersAPI=ServicesHelper.GetServicesByExe("HelianzAPIService.exe");
			if(listServiceControllersAPI.Count < 1) {
				actionDisplayError(Lans.g("ServicesHelper","HelianzAPIService could not be found in order to automatically start it."));
				return false;
			}
			string apiStartingErrors=ServicesHelper.StartServices(listServiceControllersAPI);
			if(!string.IsNullOrEmpty(apiStartingErrors)) {
				actionDisplayError(Lans.g("ServicesHelper","Unable to start the following HelianzAPIServices:")+"\r\n"+apiStartingErrors);
				return false;
			}
			return true;
		}

		///<summary>Tries to install the HelianzService if needed.  Returns false if failed.
		///Set isSilent to false to show meaningful error messages, otherwise fails silently.</summary>
		public static bool TryInstallHelianzService(bool isSilent) {
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				if(!isSilent) {
			...
			try {
				listServiceControllersOD=ServicesHelper.GetServicesByExe("HelianzService.exe");
				odServiceFilePath=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzService","HelianzService.exe");
			}
			catch(Exception e) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),Lans.g("ServicesHelper","Unknown exception:")+" "+e.Message);
				return false;
			}
			if(listServiceControllersOD.Count>0) {
					return true;//An Helianz Service is already installed.
			}				
			fileInfoOdServiceExe=new FileInfo(odServiceFilePath);
			if(!fileInfoOdServiceExe.Exists) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),(Lans.g("ServicesHelper","Unable to install the HelianzService; Cannot find")+$" '{odServiceFilePath}'"));
				return false;
				}
			if(!ServicesHelper.Install("HelianzService",fileInfoOdServiceExe)) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),Lans.g("ServicesHelper","Failed to install HelianzService, try running	as admin."));
				return false;
			}
			//Create a new HelianzServiceConfig.xml file for Helianz Service if one is not already present.
			if(!CreateConfigForHelianzService()) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),Lans.g("ServicesHelper","Failed to create HelianzServiceConfig.xml file."));
				return false;
			}
			//Now that the service has finally installed we need to try and start it.
			listServiceControllersOD=ServicesHelper.GetServicesByExe("HelianzService.exe");
			if(listServiceControllersOD.Count<1) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),Lans.g("ServicesHelper","Helianz Service could not be found."));
				return false;
			}
			...
			if(!string.IsNullOrEmpty(openDentalServiceStartingErrors)) {
				AlertItems.CreateGenericAlert(Lans.g("ServicesHelper","Helianz Service Error"),Lans.g("ServicesHelper","The following service(s) could not start:")+" "+openDentalServiceStartingErrors);
				return false;
			}
			return true;
		}

		///<summary>Creates a default HelianzWebConfig.xml file for the eConnector if one is not already present.
		...
		private static bool CreateConfigForEConnector() {
			string eConnectorConfigPath=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzEConnector","HelianzWebConfig.xml");
			string custListenerConfigPath=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzCustListener","HelianzWebConfig.xml");
		...
		///<summary>Creates a default HelianzServiceConfig.xml file for Helianz Service if one is not already present.
		///Uses the current connection settings in DataConnection.  This method does NOT work if called via middle tier.
		///Users should not be installing Helianz Service via the middle tier.</summary>
		public static bool CreateConfigForHelianzService() {
			string odServiceConfigPath=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzService","HelianzServiceConfig.xml");
			//Check to see if there is already a config file present.
			if(File.Exists(odServiceConfigPath)) {
				return true;//Nothing to do.
			}
			//At this point we know that Helianz Service does not have a config file present.
		...
		///<summary>Creates a default HelianzWebConfig.xml file for the API service if one is not already present.
		///Uses the current connection settings in DataConnection.  This method does NOT work if called via middle tier.
		///Users should not be installing the services via the middle tier.</summary>
		private static bool CreateConfigForApi() {
			string apiConfigPath=ODFileUtils.CombinePaths(Directory.GetCurrentDirectory(),"HelianzAPIService","HelianzWebConfig.xml");
			//Check to see if there is already a config file present.
			...
			bool useLocalUpdateFileCopier,bool openCopiedFiles) 
		{
			...
			//Manifest version matches. Show window if they are updating their main installation of Helianz.
		...
		private static bool DownloadUpdateFilesFromDatabase(string tempFolderUpdate) {
			if(Directory.Exists(tempFolderUpdate)) {
				try {
					...
					FormHelianz.ExitCode=301;//UpdateFiles folder cannot be deleted
					Environment.Exit(FormHelianz.ExitCode);
					return false;
				}
			}
		...
		///<param name="killServices">Will tell the file copier whether to kill all Helianz services or not.</param>
		...
		public static bool OpenFileCopier(string folderUpdate,string destDir,bool killServices,bool useLocalUpdateFileCopier,bool openCopiedFiles) {
			...
			string arguments="\""+folderUpdate+"\""//pass the source directory to the file copier.
				+" "+processId.ToString()//and the processId of Helianz.
			...
			try {
				...
				FormHelianz.ExitCode=305;//Unable to start the UpdateFileCopier.exe process.
				Environment.Exit(FormHelianz.ExitCode);
				return false;
			}
