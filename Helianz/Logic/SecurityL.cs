	...
	public class SecurityL {
		...
		public static bool ChangePassword(bool isForcedLogOff,bool willRefreshSecurityCache=true) {
			...
			if(Security.CurUser.UserNumCEMT!=0) {
				MsgBox.Show("FormHelianz","Use the CEMT tool to change your password.");
				return false;
			}
			...
			if(formUserPassword.DialogResult==DialogResult.Cancel) {
				if(isForcedLogOff) {
					FormHelianz formHelianz=Application.OpenForms.OfType<FormHelianz>().ToList()[0];//There always should be exactly 1.
					formHelianz.LogOffNow(true);
				}
				return false;
			}
