	...
	public class PatientL{
		...
		public static string GetMainTitle(Patient patient,long clinicNum) {
			...
			if(PrefC.HasClinicsEnabled && clinicNum>0) {
				if(retVal!="") {
					retVal+=" - "+Lan.g("FormHelianz","Clinic")+": ";
				}
			...
			if(patient==null || patient.PatNum==0 || patient.PatNum==-1) {
				...
				if(FormHelianz.IsRegKeyForTesting) {
					retVal+=" - "+Lan.g("FormHelianz","Developer Only License")+" - "+Lan.g("FormHelianz","Not for use with live patient data")+" - ";
				}
				return retVal;
			}
		...
		public static string MainTitleUpdateCountdown(string titleText = "") {
			...
			if(timeSpanLeft.Days>=1) {
				strTimeLeft=timeSpanLeft.Days+" "+Lan.g("FormHelianz","days")+", "+timeSpanLeft.Hours+" "+Lan.g("FormHelianz","hours");
			}
			else if(timeSpanLeft.Hours>=1) {
				strTimeLeft=timeSpanLeft.Hours+" "+Lan.g("FormHelianz","hours")+", "+timeSpanLeft.Minutes+" "+Lan.g("FormHelianz","minutes");
			}
			else if(timeSpanLeft.Minutes>=1) {
				strTimeLeft=timeSpanLeft.Minutes+" "+Lan.g("FormHelianz","minutes")+", "+timeSpanLeft.Seconds+" "+Lan.g("FormHelianz","seconds");
			}
			else if(timeSpanLeft.Seconds>=0) {
				strTimeLeft=timeSpanLeft.Seconds+" "+Lan.g("FormHelianz","seconds");
			}
			retVal=titleText+"        "+Lan.g("FormHelianz","Update In")+": "+strTimeLeft;
			return retVal;
		}
		...
		public static HelianzBusiness.Address VerifyPatientAddressInteraction(HelianzBusiness.Address address, object sender) {
		...
		private static HelianzBusiness.Address VerifyPatientAddressInteractionHelper(HelianzBusiness.Address address, object sender) {
			HelianzBusiness.Address addressResult=new HelianzBusiness.Address();
