...
namespace Helianz{
	...
	public partial class ClassConvertDatabase {
		...
		///<summary>Return false to indicate exit app.  Only called when program first starts up at the beginning of FormHelianz.PrefsStartup.</summary>
		public bool Convert(string fromVersion,string toVersion,bool isSilent,Form currentForm,bool useDynamicMode) {
			...
			if(FromVersion>=new Version("3.4.0") && PrefC.GetBool(PrefName.CorruptedDatabase)) {
				FormHelianz.ExitCode=201;//Database was corrupted due to an update failure
				if(!isSilent) {
			...
			if(FromVersion < new Version("2.8.0")) {
				FormHelianz.ExitCode=130;//Database must be upgraded to 2.8 to continue
				if(!isSilent) {
			...
			if(FromVersion < new Version("6.6.2")) {
				FormHelianz.ExitCode=131;//Database must be upgraded to 11.1 to continue
				if(!isSilent) {
			...
			if(FromVersion.ToString()=="2.9.0.0" || FromVersion.ToString()=="3.0.0.0" || FromVersion.ToString()=="4.7.0.0") {
				FormHelianz.ExitCode=190;//Cannot convert this database version which was only for development purposes
				if(!isSilent) {
			...
			if(ODBuild.IsTrial() && PrefC.GetString(PrefName.RegistrationKey)!="") {//Allow databases with no reg key to update.  Needed by our conversion department.
				FormHelianz.ExitCode=191;//Trial versions cannot connect to live databases
				if(!isSilent) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientMT) {
				FormHelianz.ExitCode=140;//Web client cannot convert database
				if(!isSilent) {
			...
			if(ReplicationServers.ServerIsBlocked()) {
				FormHelianz.ExitCode=150;//Replication server is blocked from performing updates
				if(!isSilent) {
			...
			if(DataConnection.DBtype==DatabaseType.MySql) {//not for Oracle
				...
				if(dictEngineTableNames.Keys.Count>1) {
					...
					if(!DatabaseMaintenances.ConvertToDefaultEngine()) {
						FormHelianz.ExitCode=102;//Failed to convert tables to the default storage engine format
						if(!isSilent) {
					...
					if(dictEngineTableNames.First().Key.ToUpper()!=defaultStorageEngine.ToUpper()) {
						FormHelianz.ExitCode=103;//Default database .ini setting is different than all tables
						if(!isSilent) {
			...
			if(isBlocked){
				...
				FormHelianz.ExitCode=142;//Update is already in progress from another computer
				if(!isSilent) {
			...
			if(FromVersion!=versionDatabase) {
				FormHelianz.ExitCode=143;//Database has already been updated from another computer
				if(!isSilent) {
			...
			try {
				...
				if(FromVersion < new Version("7.5.17")) {//Insurance Plan schema conversion
					if(isSilent) {
						FormHelianz.ExitCode=139;//Update must be done manually to fix Insurance Plan Schema
						Application.Exit();
						return false;
					}
					Cursor.Current=Cursors.Default;
				...
				{
					if(isSilent) {
						FormHelianz.ExitCode=138;//Update must be done manually in order to get data loss notification(s).
						Application.Exit();
						return false;
					}
			...
			return true;
				...
				FormHelianz.ExitCode=160;//File not found exception
				if(!isSilent) {
				...
				FormHelianz.ExitCode=160;//ConversionFiles folder could not be found
				if(!isSilent) {
