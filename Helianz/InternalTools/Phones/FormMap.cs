...
namespace Helianz.InternalTools.Phones {
	public partial class FormMap:FormODBase {
		...
		private void FormMapHQ2_FormClosed(object sender,FormClosedEventArgs e) {
			FormHelianz.RemoveMapFromList2(this);
		}
		...
		private void FormMap_Load(object sender,EventArgs e) {
			...
			FillTriageLabelColors();
			FormHelianz.AddMapToList2(this);
			//refresh immediately instead of waiting a second for a new signal.
			ODThread.WakeUpThreadsByGroupName(FormHelianz.FormODThreadNames.HqMetrics.GetDescription());
		...
		private void menuItemEscalation_Click(object sender,EventArgs e) {
			...
			List<PhoneEmpDefault> listPhoneEmpDefaults=PhoneEmpDefaults.GetDeepCopy();
			RefreshCurrentComboHelper(listPhoneEmpDefaults,HelianzBusiness.Phones.GetPhoneList(),PhoneEmpSubGroups.GetAll());
		...
		private void menuItemOpenNewMap_Click(object sender,EventArgs e) {
			//Open the map from FormHelianz so that it can register for the click events.
		...
		private bool DoAddToEscalationView(PhoneEmpDefault phoneEmpDefault,List<Phone> listPhones) {
			...
			Phone phone=HelianzBusiness.Phones.GetPhoneForEmployeeNum(listPhones,phoneEmpDefault.EmployeeNum);
		...
		private Bitmap GetEmployeePicture(long employeeNum){
			...
			string dir=@"\\helianz.od\SERVERFILES\Storage\OPEN DENTAL\Staff\Staff Photos";
			if(ODBuild.IsDebug()) {
				if(Environment.MachineName.ToLower()=="jordanhome"){
					dir=@"E:\Documents\Helianz\Photos Staff";
				}
		...
		private void comboEscalationView_SelectionChangeCommitted(object sender,EventArgs e) {
			List<PhoneEmpDefault> listPhoneEmpDefaults=PhoneEmpDefaults.GetDeepCopy();
			List<Phone> listPhones=HelianzBusiness.Phones.GetPhoneList();
