	...
	public partial class MapPanel:UserControl{
		...
		public void SetPhoneList(List<Phone> listPhones,List<ChatUser> listChatUsers,List<WebChatSession> listWebChatSessions, List<PeerInfo> listPeerInfosRemoteSupportSessions)
		{
			...
			for(int i=0;i<ListMapAreas.Count;i++){
				...
				string status=HelianzBusiness.Phones.ConvertClockStatusToString(phone.ClockStatus);
				...
				HelianzBusiness.Phones.GetPhoneColor(phone,phoneEmpDefault,isForDualColorScheme:true,out colorBorder,out colorBack,out colorFont,out isTriageOperatorOnTheClock);
		...
		private void MapPanel_MouseDown_Normal(MouseEventArgs e){
			...
			if(e.Button==MouseButtons.Left) {
				if(ListMapAreaMores[idx].ClockStatus==ClockStatusEnum.NeedsHelp){
					...
					HelianzBusiness.Phones.SetPhoneStatus(ClockStatusEnum.HelpOnTheWay,ListMapAreas[idx].Extension);
					ODThread.WakeUpThreadsByGroupName(FormHelianz.FormODThreadNames.HqMetrics.GetDescription());
					cubicleClickedDetail.Status=HelianzBusiness.Phones.ConvertClockStatusToString(ClockStatusEnum.HelpOnTheWay);
					CubicleClicked?.Invoke(this,cubicleClickedDetail);
					return;
				}
				if(ListMapAreaMores[idx].ClockStatus==ClockStatusEnum.HelpOnTheWay){
					...
					HelianzBusiness.Phones.SetPhoneStatus(ClockStatusEnum.Available,ListMapAreas[idx].Extension);
					ODThread.WakeUpThreadsByGroupName(FormHelianz.FormODThreadNames.HqMetrics.GetDescription());
					cubicleClickedDetail.Status=HelianzBusiness.Phones.ConvertClockStatusToString(ClockStatusEnum.Available);
					CubicleClicked?.Invoke(this,cubicleClickedDetail);
					return;
				}
				CubicleClicked?.Invoke(this,cubicleClickedDetail);
		...
		private void menuItemListenToCall_Click(object sender,EventArgs e) {
			...
			int curUserExtension=HelianzBusiness.Phones.GetExtensionForEmp(curUserEmployeeNum);
