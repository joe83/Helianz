...
using static HelianzBusiness.Backports;
	...
	public partial class FormBackport:FormODBase {
		...
		private List<string> _listComboOptions {
			get {
				return new List<string> { 
					...
					$@"C:\development\Shared Projects Subversion\HelianzWebApps",
					...
					$@"C:\development\Shared Projects Subversion\HelianzHelp",
					$@"C:\development\Shared Projects Subversion\HelianzService",
		...
		private void RefreshData() {
			...
			if(!Directory.Exists(comboPath.Text)) {
				...
				if(_pathOnRefresh.Contains("OPEN DENTAL SUBVERSION")) {//look for helianz first as its naming scheme does not match the rest.
					_currentProject=BackportProjects.Helianz;
				}
				else {
		...
		private void Compile(string backportVersion) {
			...
			switch(_currentProject.Name) {
				...
				case ProjectName.EHR:
					...
					break;
				case ProjectName.helianz:
					path+="Helianz"+"_"+versionCur.Major+"_"+versionCur.Minor+".sln";
					break;
				case ProjectName.HelianzHelp:
					path+="HelianzHelp\\HelianzHelp.csproj";
					break;
				case ProjectName.ODCrypt:
				case ProjectName.HelianzService:
				case ProjectName.HelianzWebApps:
