...
namespace Helianz {
	...
	public partial class FormJobManager:FormODBase {
		...
		///<summary>Information about engineer employee clock status. Kept current by FormHelianz typical clock status timer.</summary>
		...
		private void menuGoToChartQueries_Click(object sender,EventArgs e) {
			try {
				...
				GlobalFormHelianz.GoToModule(EnumModuleType.Chart,patNum:patNum);
			}
			catch(Exception ex) {
		...
		private void menuGoToAccountMarketing_Click(object sender,EventArgs e) {
			try {
				GlobalFormHelianz.GoToModule(EnumModuleType.Chart,patNum:((Job)gridMarketing.ListGridRows[gridMarketing.SelectedIndices[0]].Tag).ListJobQuotes[0].PatNum);
			}
			catch(Exception ex) {
				MsgBox.Show(this, "Please select a job.");
			}
		...
		private void FillMenu() {
			...
			if(JobPermissions.IsAuthorized(JobPerm.Engineer,true)) {
				//This menu will be kept current by FormHelianz on the typical HQ phone status update timer.
		...
		public void SetEmployeeStatus(Phone phone) {
			...
			//Only allow simple clock status switching if currently Available or Unavailable. Everything else should be done from FromHelianz.
