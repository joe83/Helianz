...
namespace Helianz.Reporting.Allocators.MyAllocator1 {
	class GuarantorLedgerItemsCollection {
		...
		///									Also used to determine if entries exist that are no longer a part of helianz. (meaning they have been deleted)
		...
		///		4.	Delete entries not used in htCurrentRecord (ie data that has been deleted from Helianz)
		...
		private bool Update2() {
			...
			#region Delete Items in allocation_provider table that data doesn't exist for in helianz tables
		...
		/// 
		///		1. Generate 2 Tables
		///			i.		Table1:		dt_ODItems			Contains all the items of Ledger Interest from Helianz Tables
		...
		///			i.		Build a Ledger Item from the dt_ODItems (ie from the helianz tables)
		...
		/// helianz data (ie dt_ODItems) // use Update2() to Clear these entries from data that 
