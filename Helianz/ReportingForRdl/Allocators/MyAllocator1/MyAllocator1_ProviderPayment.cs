...
using Helianz.Reporting.Allocators.MyAllocator1;
using HelianzBusiness;

namespace Helianz.Reporting.Allocators {
	class MyAllocator1_ProviderPayment:Allocator,IAllocate {
		...
		/// various helianz tables to determine what needs to be changed.
		/// </summary>
		...
		private static void _AllocateExecute(long iGuarantor) {
			Helianz.Reporting.Allocators.MyAllocator1.GuarantorLedgerItemsCollection Ledger =
								new Helianz.Reporting.Allocators.MyAllocator1.GuarantorLedgerItemsCollection(iGuarantor);
			Ledger.Fill(false);
		...
		public static string CreatTableString(DatabaseType type1) {
			...
			//else if (type1 == HelianzBusiness.DatabaseType.Oracle)
			//;// not implemented yet
			return command;
		...
		private void StartBatch_DoWork_Handler(object sender,System.ComponentModel.DoWorkEventArgs e) {
			...
			int BatchSize = 50;
			int iProgress = 0;
			Helianz.Reporting.Allocators.MyAllocator1_ProviderPayment allocator = new Helianz.Reporting.Allocators.MyAllocator1_ProviderPayment();
