...
namespace Helianz.Reporting.Allocators.MyAllocator1.SupportingCode
{
...
#region Report Variables/Properties that outside can get
		...
		+ "\nWHERE "
				...
				+ "\n    AND pl.ProcStatus = " +  ((int)HelianzBusiness.ProcStat.C).ToString()
			...
			+ "\nWHERE "
					+ "\n   pt.PatNum = cp.PatNum "
					+ "\n   AND cp.WriteOff != 0 "
					+ "\n   AND cp.Status = " + ((int)HelianzBusiness.ClaimProcStatus.Received).ToString()
					+ "\n   AND pt.Guarantor = " + Guarantor
			+ "\nORDER By Date";
			return outstring;
