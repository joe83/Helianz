		...
		*/
		...
		private void LayoutChildren(Control control){
			...
			for(int i=0;i<control.Controls.Count;i++){
					...
					|| control.Controls[i] is System.Windows.Forms.ListBox)//FormSheetFieldStatic
				{
					...
					//But the scary part is that it only behaves this way in one project: Unit Tests. Additional scaling does not happen in Helianz.exe.
			...
			for(int i=listControlsDocked.Count-1;i>=0;i--){
				...
				if(control96Info.Dock==DockStyle.Top){
					...
					//if(listControlsDocked[i].GetType()==typeof(Helianz.UI.MenuOD)){
		...
		private bool LayoutSkipChildren(Control96Info control96Info,Control control){
			...
			if(control is UI.GridOD gridOD){
				gridOD.SetScaleAndZoom(_scaleMS,_zoomLocal);//grids are over in HelianzBusiness, so this is instead of a ref to LayoutManager.
			...
			if(control is SignatureBoxWrapper signatureBoxWrapper){
				signatureBoxWrapper.SetScaleAndZoom(_scaleMS,_zoomLocal);//Sig boxes are over in HelianzBusiness, so this is instead of a ref to LayoutManager.
		...
		private void PassLayoutManager(Control control){
			...
			//Because grids are in HelianzBusiness, we can't send them a ref to LayoutManager.
			...
			//	return;
			//}
			if(control is Helianz.UI.GroupBox groupBoxOD){
