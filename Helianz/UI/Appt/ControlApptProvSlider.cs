...
namespace Helianz.UI{
	...
	public partial class ControlApptProvSlider : Control{
		...
		public Helianz.UI.CheckBox FormApptEdit_CheckTimeLocked=new Helianz.UI.CheckBox();
		...
		protected override void OnPaint(PaintEventArgs pe){
			...
			using Font fontCourier=new Font(FontFamily.GenericMonospace,LayoutManager.ScaleFontODZoom(8.5f));
			for(int i=0;i<SelectedLength;i++){
			...
			using Font fontTime=new Font(FontFamily.GenericSansSerif,LayoutManager.ScaleFontODZoom(8.5f));
			float widthStr=g.MeasureString(strTime,fontTime).Width;//measures smaller unscaled string
			float heightStr=LayoutManager.ScaleMS(fontTime.Height);
			float x=Width/2f-widthStr/2;
			float y=SelectedLength*_heightRow+LayoutManager.ScaleF(2);
