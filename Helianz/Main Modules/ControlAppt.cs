...
namespace Helianz {
	...
	public partial class ControlAppt:UserControl {
		...
		private bool _waitingSortAscending=true;
		...
		private HelianzBusiness.AutoComm.Arrivals _arrivalsLoaded;
		...
		private HelianzBusiness.AutoComm.Arrivals GetArrivalsLoaded() {
			if(_arrivalsLoaded is null) {
					_arrivalsLoaded=HelianzBusiness.AutoComm.Arrivals.LoadArrivals();
				}
			return _arrivalsLoaded;
		}
		private void SetArrivalsLoaded(HelianzBusiness.AutoComm.Arrivals value) {
		...
		private void contrApptPanel_ApptMainAreaDoubleClicked(object sender,UI.ApptMainClickEventArgs e) {
			...
			if(_patient==null || frmPatientSelect.PatNumSelected!=_patient.PatNum) {//if the patient was changed
				...
				GlobalFormHelianz.PatientSelected(_patient,true,false);
			}
			...
			if(patientMerged!=null) {
				...
				GlobalFormHelianz.PatientSelected(_patient,isRefreshCurModule: true,isApptRefreshDataPat: false);
			}
		...
		private void ContrApptPanel_ApptRightClicked(object sender, UI.ApptRightClickEventArgs e){
			...
			if(HelianzBusiness.AutoComm.Byod.IsSetup(clinicNum,out string err)) {//Check-In Links feature is enabled.  (handles PrefC.HasClinicsEnabled)
				...
				menuItem.Enabled=HelianzBusiness.AutoComm.Byod.IsEnabledForConfirmed(confirmed,clinicNum,out err);//(handles PrefC.HasClinicsEnabled)
			}
		...
		private void toolBarMain_ButtonClick(object sender,Helianz.UI.ODToolBarButtonClickEventArgs e) {
			if(e.Button.Tag.GetType()==typeof(string)) {
				...
				switch(e.Button.Tag.ToString()) {
					...
					case "RapidCall":
						try {
							Helianz.Bridges.RapidCall.ShowPage();
						}
						catch(Exception ex) {
							MessageBox.Show(ex.Message);
						}
		...
		private void butClearPin_Click(object sender,EventArgs e) {
			...
			FormHelianz.S_Contr_PatientSelected(PatCur,true,false);
			}*/
		}
		...
		private void pinBoard_ApptMovedFromPinboard(object sender,UI.ApptFromPinboardEventArgs e) {
			...
			GlobalFormHelianz.PatientSelected(_patient,true,false);//especially to change name showing in title bar
		...
		private void menuItemReminderGoto_Click(object sender,EventArgs e) {
			...
			Task task=(Task)gridReminders.ListGridRows[gridReminders.GetSelectedIndex()].Tag;
			FormHelianz.S_TaskGoTo(task.ObjectType,task.KeyNum);
		}
		...
		private void menuApt_Click(object sender,System.EventArgs e) {
			...
			switch(((ToolStripMenuItem)sender).Name) {
				...
				case MenuItemNames.SendText:
					...
					GlobalFormHelianz.SendTextMessage(appointment.PatNum,"");
					break;
				case MenuItemNames.SendConfirmationText:
					...
					bool wasTextSent=GlobalFormHelianz.SendTextMessage(patient.PatNum,message);
					if(wasTextSent) {
				...
				case MenuItemNames.SendComeInText:
					...
					GlobalFormHelianz.SendTextMessage(appointment.PatNum,message);
					break;
		...
		private void menuJobs_GoToJob(object sender,System.EventArgs e) {
				FormHelianz.S_GoToJob(((long)((ToolStripMenuItem)sender).Tag));
		}
		...
		private void ButLab_Click(object sender, EventArgs e){
			...
			if(formLabCases.GoToAptNum!=0) {
				...
				Patient patient=Patients.GetPat(appointment.PatNum);
				//PatientSelectedEventArgs eArgs=new Helianz.PatientSelectedEventArgs(pat.PatNum,pat.GetNameLF(),pat.Email!="",pat.ChartNumber);
				//if(PatientSelected!=null){
				//	PatientSelected(this,eArgs);
				//}
				//Contr_PatientSelected(this,eArgs);
				RefreshModuleDataPatient(patient.PatNum);
				GlobalFormHelianz.PatientSelected(patient,false,false);
		...
		///<summary>Called from FormHelianz each time the module is selected, but it doesn't do anything after the first time.</summary>
		...
		public void ModuleSelected(long patNum,List<long> listPinApptNums=null,List<long> listOpNums=null,List<long> listProvNums=null){
			...
			if(_patient!=null && _patient.PatStatus==PatientStatus.Deleted) {
				...
				GlobalFormHelianz.PatientSelected(new Patient(),false);
		...
		///<summary>This is public so that FormHelianz can pass refreshed tasks here in order to avoid an extra query.</summary>
			...
			List<long> listOpNums=null,List<long> listProvNums=null,bool forceRefresh=false)
		{
			...
			HelianzBusiness.AutoComm.Arrivals arrivals=HelianzBusiness.AutoComm.Arrivals.LoadArrivals(
		...
		///<summary>Fills comboView with the current list of views. Triggers ModuleSelected().  Also called from FormHelianz.RefreshLocalData().</summary>
		...
		private void FillWaitingRoom() {
			...
			int selectedIdx=gridWaiting.GetSelectedIndex();
			_selectedWaitingAptNum=0;
			if(selectedIdx>=0 && selectedIdx<gridWaiting.ListGridRows.Count) {
				_selectedWaitingAptNum=(long)gridWaiting.ListGridRows[selectedIdx].Tag;
			}
			...
			if(_waitingSortColumn<0) {
				listFilteredRows.Sort((a,b) => ((DateTime)a["DateTimeArrived"]).CompareTo((DateTime)b["DateTimeArrived"]));
			}
			GridRow row;
			...
			//Restore selection to the previously selected patient (must be after EndUpdate — it clears selections).
			if(_selectedWaitingAptNum!=0) {
				for(int i=0;i<gridWaiting.ListGridRows.Count;i++) {
					if((long)gridWaiting.ListGridRows[i].Tag==_selectedWaitingAptNum) {
						gridWaiting.SetSelected(i);
						break;
					}
				}
			}
		...
		private void SyncWaitingSelectionToAppointment() {
			...
			if(idx>=0 && idx<gridWaiting.ListGridRows.Count) {
				long aptNum=(long)gridWaiting.ListGridRows[idx].Tag;
				if(aptNum==0) {
					return;
				}
				bool isNewSelection=(_selectedWaitingAptNum!=aptNum);
				_selectedWaitingAptNum=aptNum;
				contrApptPanel.SelectedAptNum=aptNum;
				//Try the fast in-memory lookup first (TableAppointments from the current view).
				DataRow dataRow=contrApptPanel.GetDataRowForSelected();
				if(dataRow!=null) {
					listConfirmed.SelectedIndex=Defs.GetOrder(DefCat.ApptConfirmed,PIn.Long(dataRow["Confirmed"].ToString()));
				}
				else if(isNewSelection) {
					//Fall back to DB lookup if the operatory isn't in the current appointment view.
					Appointment appointment=Appointments.GetOneApt(aptNum);
					if(appointment!=null) {
						listConfirmed.SelectedIndex=Defs.GetOrder(DefCat.ApptConfirmed,appointment.Confirmed);
					}
				}
		...
		///<summary>Handles click on a waiting room row — syncs the selection to the appointment module.</summary>
		private void gridWaiting_CellClick(object sender,UI.ODGridClickEventArgs e) {
			SyncWaitingSelectionToAppointment();
		}

		///<summary>Handles column header click on the waiting room grid — saves sort preference.</summary>
		private void gridWaiting_ColumnSortClick(object sender,EventArgs e) {
			int sortedCol=gridWaiting.GetSortedByColumnIdx();
			if(sortedCol<0) return;
			if(_waitingSortColumn==sortedCol) {
				_waitingSortAscending=!_waitingSortAscending;
			}
			else {
				_waitingSortColumn=sortedCol;
				_waitingSortAscending=true;
			}
		}

		///<summary>Prints the queue slip for the selected patient to the default printer.</summary>
		private void butPrintQueue_Click(object sender,EventArgs e) {
			int idx=gridWaiting.GetSelectedIndex();
			if(idx<0 || idx>=gridWaiting.ListGridRows.Count) {
				MsgBox.Show(this,"Please select a patient in the waiting room first.");
				return;
			}
			GridRow row=gridWaiting.ListGridRows[idx];
			string patientName=row.Cells[0].Text;
			string queueLabel=row.Cells.Count>2 ? row.Cells[1].Text : "";
		...
		private void PrintQueueTicket(string queueLabel,string patientName,string roomName,string clinicName) {
			...
			Printer printerForSit=Printers.GetForSit(PrintSituation.QueueTicket);
			if(printerForSit!=null && !string.IsNullOrEmpty(printerForSit.PrinterName)) {
			...
			try {
				...
				MsgBox.Show(this,"Print failed: "+ex.Message);
			}
		}
		...
		private void AutomaticCallDialingDisabledMessage() {
			...
			MsgBox.Show("Automatic dialing of patient phone numbers requires an additional service.\r\n"
				+"Contact Helianz for more information.");
			try {
				System.Diagnostics.Process.Start("http://www.helianz.com/resources/redirects/redirectdentaltekinfo.html");
			}
			catch(Exception) {
				MsgBox.Show("Could not find http://www.helianz.com/contact.html \r\n"
		...
		private void ProcessOtherDlg(OtherResult otherResult,long patNum,string strDateJumpTo,long[] aptNumArray,List<long> listApptViewNums=null) {
			...
			switch(otherResult) {
				...
				case OtherResult.GoTo:
					...
					GlobalFormHelianz.PatientSelected(_patient,true,false,true);
