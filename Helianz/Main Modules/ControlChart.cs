...
namespace Helianz {
	...
	public partial class ControlChart:UserControl	{
		...
		///<summary>This is a reference to the _pd in FormHelianz.</summary>
		...
		public ControlChart() {
			...
			this.toothChartWrapper.DrawMode = HelianzBusiness.DrawingMode.Simple2D;
		...
		private void gridPtInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(gridPtInfo.ListGridRows[e.Row].Tag==null || gridPtInfo.ListGridRows[e.Row].Tag.ToString()=="DOB") {
				...
				if(formPatientEdit.DialogResult==DialogResult.OK) {
					GlobalFormHelianz.PatientSelected(Pd.Patient,false);
				}
				ModuleSelected(Pd.PatNum);
				return;
			}
			...
			if(gridPtInfo.ListGridRows[e.Row].Tag.ToString()=="References") {
				...
				if(formReference.PatNumGoto!=0) {
					...
					GlobalFormHelianz.GoToModule(EnumModuleType.Family, patNum:formReference.PatNumGoto);
					return;
				}
		...
		private void butPin_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, listPinApptNums:listAptNums, dateSelected:DateTime.Today);
		}
		...
		private void butAddText_Click(object sender,EventArgs e) {
			if(ODBuild.IsThinfinity()) {
				MsgBox.Show(this,"The text feature is not available in Helianz Cloud.");
				return;
			}
		...
		public void InitializeOnStartup() {
			...
			if(File.Exists(Path.Combine(appDir,"Sparks3D\\Sparks3D.dll"))) {
				...
				//	ToothChartRelay.IsSparks3DPresent=true;
				//}
				//else if(HelianzHelp.ODHelp.IsEncryptedKeyValid()){//always true in debug
				//	ToothChartRelay.IsSparks3DPresent=true;
				//}
			}
		...
		/// <summary>Only use this overload when isClinicRefresh is set to true.  This is only used when calling ModuleSelected from FromHelianz. When isClinicRefresh is true the tab control tabProc is redrawn and only needs to be done when the clinic is changed or the module is selected for the first time.</summary>
		...
		///<summary>This function does not follow our usual pattern. This function is just like ModuleSelected() but also syncs any eRx data which needs to be checked frequently.  Only called from FormHelianz when the Chart module button is clicked or a new patient is selected while inside the Chart.</summary>
		...
		public void RefreshModuleScreen(bool isClinicRefresh=false) {
			...
			if(IsPatientNull()) {
				...
				//if(FormHelianz.ObjSomeEhrSuperClass!=null) {//didn't work
		...
		private void RefreshDoseSpotNotifications() {
			...
			ODThread threadRefreshDoseSpotNotifications=new ODThread((odThread) => {
				...
				ProgramProperty programPropertyDoseSpotApiVersion=HelianzBusiness.ProgramProperties.GetPropForProgByDesc(Programs.GetProgramNum(ProgramName.eRx),DoseSpotREST.PropertyDescs.DoseSpotApiVersion);
				try {
		...
		private void Tool_EHR_Click(bool onLoadShowOrders) {
			...
			if(formEHR.EhrFormResultOnClose==EhrFormResult.PatientSelect) {
				GlobalFormHelianz.PatientSelected(Patients.GetPat(formEHR.PatNum),false);
		...
		private void Tool_eRx_Click(bool isShowRefillsAndErrors=false) {
			...
			if(erxOption==ErxOption.NewCrop) {
				if(isNewCropDisabledForWeb) {
					...
					MsgBox.Show(this,"NewCrop is not available while using Helianz Cloud.");
					return;
				}
				...
				if(newCropAccountId=="") {//NewCrop has not been enabled yet.
...
#if DEBUG
					Helianz.localhost.Service1 service1Update=new Helianz.localhost.Service1();
#else
					Helianz.customerUpdates.Service1 service1Update=new Helianz.customerUpdates.Service1();
					...
					if(!HelianzBusiness.NewCrop.NewCropIsAccountIdValid()) {
				...
				if(isShowRefillsAndErrors) {
					...
					if(isEmp) {
						...
						if(listUserodsDose.Count==1) {
							...
							MsgBox.Show(this,"There are too many Helianz users associated to the selected provider.");
							return;
						}
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		private void UpdateErxAccess(string npi,string userId,long clinicNum,string clinicId,string clinicKey,ErxOption erxOption) {
			...
			if(PrefC.GetString(PrefName.UpdateServerAddress).ToLower().Contains("helianzsoft.com") || 
				PrefC.GetString(PrefName.UpdateServerAddress).ToLower().Contains("helianz.com"))
			{
