...
namespace Helianz {
	//Partial class file which houses all thread related business for FormHelianz.
	public partial class FormHelianz:FormODBase {
		...
		///<summary>Signal processing timer. Gets started in FormHelianz.FormHelianzShown(), 
		...
		///<summary>Starts or stops the local timers owned by FormHelianz.
		///Only starts signal timer if interval preference is set to non-zero value.
		///The Windows Forms timer is designed for use in a single-threaded environment which requires this method called from the main UI thread 
		///or marshal / invoke the call onto another thread.</summary>
		...
		///<summary>Either starts all possible threads owned by FormHelianz or stops a select few threads which are safe to stop.
		...
		private void BeginEhrCodeListThread() {
			...
			ODThread odThread=new ODThread(o => {
				...
				//start up sequence. Users could theoretically use Helianz for an entire day and never hit the code that utilizes the EhrCodes class.
		...
		private void ProcessHqMetricsPhones() {
			...
			{
				List<HelianzBusiness.Task> listOfficesDowns=Tasks.GetOfficeDowns();
				if(!IsDisposed) {
		...
		private void LogOffWorker() {
			...
			else {//Some Helianz dialog is active.
		...
		private void BeginODCloudDcvExtensionThread() {
			...
			///<summary>Begins a thread that monitor's the Helianz Service heartbeat and alerts the user if the service is not running.</summary>
			private void BeginODServiceMonitorThread() {
			if(IsThreadAlreadyRunning(FormODThreadNames.ODServiceMonitor)) {
				return;
			}
			ODThread threadHelianzServiceCheck=new ODThread((int)TimeSpan.FromMinutes(10).TotalMilliseconds,
				(o) => { AlertItems.CheckODServiceHeartbeat(); });
			threadHelianzServiceCheck.AddExceptionHandler(ex => ex.DoNothing());
			threadHelianzServiceCheck.GroupName=FormODThreadNames.ODServiceMonitor.GetDescription();
			threadHelianzServiceCheck.Name=FormODThreadNames.ODServiceMonitor.GetDescription();
			threadHelianzServiceCheck.Start();
		}

		#endregion
		#region ODServiceStarterThread

		///<summary>Spawns a thread that attempts to start all Helianz services.</summary>
		private void BeginODServiceStarterThread() {
			...
			ODThread odThread=new ODThread((o) => {
				if(PrefC.GetString(PrefName.WebServiceServerName)!="" && ODEnvironment.IdIsThisComputer(PrefC.GetString(PrefName.WebServiceServerName))) {
					//An InvalidOperationException can get thrown if services could not start.  E.g. current user is not running Helianz as an 
					...
					ServicesHelper.StartServices(ServicesHelper.GetAllHelianzServices());
				}
			});
			//If the thread that attempts to start all Helianz services fails for any reason, silently fail.
			odThread.AddExceptionHandler(ex => ex.DoNothing());
		...
		private void BeginODDashboardStarterThread() {
			...
			//If the thread that attempts to start Helianz dashboard fails for any reason, silently fail.
		...
		///<summary>Begins a thread that shutsdown Helianz.</summary>
		...
		private void BeginTasksThread(List<Signalod> listSignalTasks,List<long> listEditedTaskNums) {
			...
			ODThread threadTasks=new ODThread(new ODThread.WorkerDelegate((o) => {
				...
				List<HelianzBusiness.Task> listRefreshedTasks=Tasks.GetNewTasksThisUser(userNumCur,Clinics.ClinicNum,listEditedTaskNums);
		...
		///<summary>This begins the time sync thread. If Helianz is running on the same machine as the mysql server, then a thread is runs in the 
		...
		private void BeginTimeSyncThread() {
			...
			//Helianz has EHR enabled and is running on the same machine as the mysql server it is connected to.
		...
		public enum FormODThreadNames {
			...
			DataConnectionLost,
			EhrCodeList,
			///<summary>DEPRECATED. This thread now runs from Helianz Service solution</summary>
