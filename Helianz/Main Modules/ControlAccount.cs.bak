...
namespace Helianz {
	...
	public partial class ControlAccount:UserControl {
		...
		///<summary>Public so this can be checked from FormHelianz and the note can be saved.  Necessary because in some cases the leave event doesn't
			...
			if(table.Rows[e.Row]["ProcNum"].ToString()!="0") {
				...
				if(payplan.IsDynamic) {
					...
					if(formPayPlanDynamic.PatNumGoto!=0) {
						GlobalFormHelianz.PatientSelected(Patients.GetPat(formPayPlanDynamic.PatNumGoto),false);
					...
					if(formPayPlan.PatNumGoto!=0) {
						GlobalFormHelianz.PatientSelected(Patients.GetPat(formPayPlan.PatNumGoto),false);
		...
		private void gridPatInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(formPatientEdit.DialogResult==DialogResult.OK) {
				GlobalFormHelianz.PatientSelected(_patient,false);
			}
			ModuleSelected(_patient.PatNum);
		}
		...
		/// <summary>Only for use in FormHelianz.cs form deactivate event handler. Used to prevent a bug that would clear out the FamUrgFinNote when closing the program sometimes.
...
;
		...
		private void PrintStatement(Statement statement) {
			...
			//Helianz.Imaging.ImageStoreBase imageStore = Helianz.Imaging.ImageStore.GetImageStore(guar);
		...
		private void PayPlanHelper(PayPlanModes payPlanMode) {
			...
			if(patNumGoto!=0) {
				GlobalFormHelianz.PatientSelected(Patients.GetPat(patNumGoto),false);
