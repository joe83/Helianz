...
namespace Helianz{
	...
	public partial class ControlImages : UserControl{
		...
		private List<HelianzBusiness.Pearl.EnumCategoryOD> _listPearlCategoriesShown=null;
		...
		public ControlImages(){
			...
			HelianzBusiness.Bridges.Pearl.EventRefreshDisplay+=pearl_EventRefreshDisplay;
		...
		///<summary>Key down from FormHelianz is passed in to allow some keys to work here.  As long as this module is open, all key down events are sent here.</summary>
			...
			if(_listPearlCategoriesShown is null){//happens once on startup
				if(Programs.IsEnabled(ProgramName.Pearl)){
					...
					if(programProperty==null) {
						_listPearlCategoriesShown=new List<HelianzBusiness.Pearl.EnumCategoryOD>();
					}
					else{
						//Add all categories to list except tooth parts.
						_listPearlCategoriesShown=HelianzBusiness.Bridges.Pearl.GetAllCategoryODsExceptToothParts();
					}
				}
				else{
					_listPearlCategoriesShown=new List<HelianzBusiness.Pearl.EnumCategoryOD>();
		...
		private void checkPearlToothParts_Click(object sender, EventArgs e) {
			List<HelianzBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs=HelianzBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
		...
		private void imageSelector_DragDropImport(object sender,DragDropImportEventArgs e) {
			...
			for(int i=0;i<e.ListFileNames.Count;i++){
				...
				if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,filePath:e.ListFileNames[i]);
					pearl?.SendOnThread();
				}
			}
		...
		private void imageSelector_ItemDoubleClick(object sender,EventArgs e) {
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.InDatabase) {
				...
				return;//Documents must be stored in the A to Z Folder to open them outside of Helianz.  Users can use the export button for now.
			}
		...
		private void _fileSystemWatcher_FileCreated(object sender,FileSystemEventArgs e) {
			...
			if(!File.Exists(e.FullPath)) {
				...
				//The first time passing through this method, this file will exist and if it fails to save in helianz, we will get an error message stating so later in the method.
			...
			if(!IsMountShowing()){//single
				try {
					if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
						HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,filePath:e.FullPath);
						pearl?.SendOnThread();
					}
			...
			if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
				HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,bitmap,mountItem:mountItem);
				pearl?.SendOnThread();
			}
			bitmap?.Dispose();
			try {
		...
		private void pearl_EventRefreshDisplay(object sender,EventArgs eventArgs) {
			...
			try {
				...
				List<HelianzBusiness.Pearl.EnumCategoryOD> listEnumCategoryODsToothParts=HelianzBusiness.Bridges.Pearl.GetToothPartsCategoryODs();
				...
				if(sender is HelianzBusiness.Bridges.Pearl) {
					controlImageDisplay.ClearObjects();
					NodeTypeAndKey nodeTypeAndKey=controlImageDisplay.GetNodeTypeAndKey();
					SetDocumentOrMount(nodeTypeAndKey,controlImageDisplay);
					controlImageDisplay.SelectTreeNode2(nodeTypeAndKey);
		...
		private void ToolBarMoveToPatient(object sender, EventArgs e){
			if(ODBuild.IsThinfinity()) {
				MsgBox.Show(this,"Not yet implemented for Helianz Cloud.");
				return;
			}
			...
			ControlImageDisplay controlImageDisplay=GetControlImageDisplaySelected();
			GlobalFormHelianz.GoToModule(EnumModuleType.Imaging,patNum:frmPatientSelect.PatNumSelected);
		...
		private void ToolBarProgram_Click(object sender,EventArgs e){
			...
			{
				Process.Start("https://www.helianz.com/resources/redirects/redirectpearl.html");
				return;
			}
			ControlImageDisplay controlImageDisplay=GetControlImageDisplaySelected();
			if(controlImageDisplay==null) {
			...
			if(IsMountShowing()){
				...
				for(int i=0;i<listBitmaps.Count;i++) {
					...
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,listBitmaps[i],mountItem:listMountItems[i]);
					pearl?.SendOnThread();
				}
			}
			else{//one doc
				...
				HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,listBitmaps[0]);
				pearl?.SendOnThread();
			}
			if(_listPearlCategoriesShown.IsNullOrEmpty()) {//if no Pearl annotations are set to show right now
				...
				_listPearlCategoriesShown=Enum.GetValues(typeof(HelianzBusiness.Pearl.EnumCategoryOD))
					.Cast<HelianzBusiness.Pearl.EnumCategoryOD>().ToList();
			}
		}
		...
		private void ToolBarMountAcquire_Click(object sender, EventArgs e){
			...
			if(ODEnvironment.IsCloudServer) {
				...
				if(imagingDevice.DeviceType==EnumImgDeviceType.TwainRadiograph) {
					...
					GlobalFormHelianz.LockODForMountAcquire(isEnabled:false);
					ProgressWin progressWin=new ProgressWin();
					progressWin.ActionMain=() => {
						while(true) {
					...
					try{
						...
						GlobalFormHelianz.LockODForMountAcquire(isEnabled:true);
		...
		private bool PlaceAcquiredBitmapInUI(Bitmap bitmap,EnumImgDeviceType enumImgDeviceType) {
			...
			if(!IsMountShowing()){//single
				...
				SelectTreeNode1(new NodeTypeAndKey(EnumImageNodeType.Document,document.DocNum));
				if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(_patient,document,bitmap);
					pearl?.SendOnThread();
				}
				bitmap?.Dispose();
				return true;
