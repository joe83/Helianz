...
using Document=HelianzBusiness.Document;
...
///<summary></summary>
	public partial class ControlTreat : System.Windows.Forms.UserControl{
		...
		///<summary>Set to true when TP Note changes.  Public so this can be checked from FormHelianz and the note can be saved.  Necessary because in
		...
		private void gridMain_CellClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
		...
		private void gridPlans_CellClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
		...
		private void gridPlans_CellDoubleClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
			//if(e.Row==0){
		...
		private List<Document> SaveTPAsDocument(bool isSigSave,Sheet sheet=null) {
			...
			for(int i=0;i< listNumsCategories.Count;i++) {//usually only one, but do allow them to be saved once per image category.
				HelianzBusiness.Document documentSave=new Document();
		...
		private void gridPreAuth_CellDoubleClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
			if(!Security.IsAuthorized(EnumPermType.ClaimView)) {
				return;
			}
		...
		private void gridPreAuth_CellClick(object sender, Helianz.UI.ODGridClickEventArgs e) {
		...
		private void radioTreatPlanSortTooth_MouseClick(object sender,MouseEventArgs e) {
			FormHelianz.IsTreatPlanSortByTooth=radioTreatPlanSortTooth.Checked;
		...
		private bool ValidateRightClickDXC(Claim claimPreAuth) {
			if(claimPreAuth.ClaimStatus=="W") {
				...
				claimSendQueueItemsArray[0]=HelianzBusiness.Eclaims.Eclaims.GetMissingData(clearinghouseClin,claimSendQueueItemsArray[0]);
