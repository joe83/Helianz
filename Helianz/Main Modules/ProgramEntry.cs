	...
	static class ProgramEntry {
		...
		static void Main(string[] args) {
			...
			if(ODBuild.IsThinfinity()) {
				...
				Web.OnCloseHandler=FormHelianz.S_ProcessKillCommand;
				//allows helianz to run in a web browser
				Web.Start(); 
				//allows helianz to send data to the browser
				ODCloudClient.SendDataToBrowser=Helianz.Thinfinity.Browser.SendData;
				//allows helianz to get the latest cloud client version from HQ
				ODCloudClient.GetLatestCloudClientVersion=WebServiceMainHQProxy.GetLatestCloudClientVersion;
				//allows helianz to process API requests from the Cloud Client
				Helianz.Thinfinity.Browser.ProcessApiRequest=ODCloudClient.ProcessApiRequest;
			}
			try {
			...
			for(int i=0;i<processes.Length;i++) {
				...
				if(processes[i].ProcessName.StartsWith("Helianz")) {
			...
			if(!ODBuild.IsDebug() && objectVal!=null) {
				...
				if(intVal==0){
					...
					MessageBox.Show("Registry updated to EnablePerProcessSystemDPI. Please restart Helianz.");
					Application.Exit();
					return;
				}
			}
			FormHelianz formHelianz=new FormHelianz(commandLineArgs);
			...
			Action<Exception,string> actionUnhandled=new Action<Exception,string>((e,threadName) => {
				...
				try {
					...
					if(submittedException==null) {
						...
						BugSubmissions.SubmitException(e,out displayMsg,threadName,FormHelianz.PatNumCur,formHelianz.GetSelectedModuleName());
					}
				}
				catch(Exception ex) {
					ex.DoNothing();
				...
				formHelianz.ProcessKillCommand();
			});
			CodeBase.ODThread.RegisterForUnhandledExceptions(formHelianz,actionUnhandled);
			formHelianz.IsSecondInstance=isSecondInstance;
			...
			//HelianzCloud.dll references Dropbox.Api.dll which references Newtonsoft.Json.dll version 7.0.0.0. Sometimes it also says it can't find 
			//9.0.0.0.
			...
			if(commandLineArgs.Any(x => x.ToLower()=="issilentupdate=true")) {
				formHelianz.FormHelianzShown();//Form never shows. It returns out after not too long.
				//This prevents zombie process when drawing icons in Direct2D EnumContext.Graphics.
			}
			else {
				Application.Run(formHelianz);
			}
		}
	}
}
