/*=============================================================================================================
Helianz 
Copyright 2003-2023  Jordan Sparks, DMD.  http://www.helianz.com
...
//using Helianz.SmartCards;
using Helianz.UI;
using System.ServiceProcess;
using System.Linq;
using Helianz.Bridges;
using HelianzBusiness.WebServiceMainHQ;
using ServiceManager;
using System.DirectoryServices;
using WMPLib;
using HelianzImaging;
...
namespace Helianz{
	///<summary></summary>
	public partial class FormHelianz:FormODBase {
		#region Fields - Public
		///<summary>Represents if the regkey is a developer regkey.</summary>
		public static bool IsRegKeyForTesting;
		///<summary>This is used to determine how Helianz closed.  If this is set to anything but 0 then some kind of error occurred and Helianz was forced to close.  Currently only used when updating Helianz silently.</summary>
		...
		///<summary>This is the singleton instance of the FormHelianz. This allows us to have S_ methods that are public static and can be called from anywhere in the program to update FormHelianz.</summary>
		private static FormHelianz _formHelianzSingleton;
		...
		private HelianzGraph.FormDashboardEditTab _formDashboardEditTab;
		...
		#region Constructor
		///<summary></summary>
		public FormHelianz(string[] cla){
			_formHelianzSingleton=this;
			...
			GlobalFormHelianz.EventLockODForMountAcquire+=(sender,isEnabled)=>LockODForMountAcquire(isEnabled);
			GlobalFormHelianz.EventRefreshCurrentModule+=(sender,isClinicRefresh)=>RefreshCurrentModule(isClinicRefresh:isClinicRefresh);
			GlobalFormHelianz.EventModuleSelected+=(sender,e)=>GotoModule_ModuleSelected(e);
			GlobalFormHelianz.EventPatientSelected+=(sender,e)=>Contr_PatientSelected(e);
			GlobalFormHelianz.SendTextMessage=toolButTxtMsg_Click;
			GlobalFormHelianz.GoToModule=GotoModule_ModuleSelected;
			...
			//Plugins.HookAddCode(this,"FormHelianz.Constructor_end");//Can't do this because no plugins loaded.
			formSplash.Close();
		}
		...
		public static PhoneTile PhoneTile {
			get {
				return _formHelianzSingleton.phoneSmall.phoneTile;
			}
		}
		...
		public static List<string> S_Contr_TabProcPageTitles { 
			get { 
				return _formHelianzSingleton.controlChart.GetListTabProcPageTitles(); 
			} 
		}
		#endregion Properties

		#region Methods - Event Handlers - Form
		private void FormHelianz_Load(object sender,System.EventArgs e) {
		...
		private void FormHelianz_Shown(object sender,EventArgs e) {
			Logger.LogToPath("FormHelianzShown",LogPath.Startup,LogPhase.Start);
			FormHelianzShown();
			Logger.LogToPath("FormHelianzShown",LogPath.Startup,LogPhase.End);
		}

		public void FormHelianzShown() {
			...
			Version versionOd=Assembly.GetAssembly(typeof(FormHelianz)).GetName().Version;
			...
			if(versionOd!=versionObBus) {
				if(isSilentUpdate) {
					...
					MessageBox.Show("Mismatched program file versions. Please run the Helianz setup file again on this computer.");
				}
				Environment.Exit(ExitCode);
				return;
			}
			...
			if(PrefC.GetBoolSilent(PrefName.ImagesModuleUsesOld2020,false)) {
				...
				controlImages.EventKeyDown+=(sender,e)=>FormHelianz_KeyDown(sender,e);
			...
			//Spawn a thread so that attempting to start services on this computer does not hinder the loading time of Helianz.
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.LocalAtoZ) {
				...
				if(programPropertyPath==null || programPropertyAccessToken==null) {
					...
					try {
						hasConnected=HelianzCloud.Dropbox.FileExists(programPropertyAccessToken.PropertyValue,
				...
				if(programPropertyPath==null || programPropertyHost==null || programPropertyUsername==null || programPropertyPassword==null) {
					...
					try {
						hasConnected=HelianzCloud.Sftp.FileExists(programPropertyHost.PropertyValue,programPropertyUsername.PropertyValue,decryptedPassword,
							ODFileUtils.CombinePaths(programPropertyPath.PropertyValue,"A",'/'));
					}
					catch(Exception) {
						hasConnected=false;//If an error occurs, DataStorageType does not connect
				...
				HelianzCloud.Sftp.Download.CleanupStashes(false);//Cleanup stashes which are no longer in use. No need to cleanup stash for current instance yet.
			}
			...
			if(ODBuild.IsDebug()) {
				...
				//http://api.helianz.com:30222/api/v1/");
			...
			Plugins.HookAddCode(this,"FormHelianz.Load_end");
		}
		...
		private bool ActiveInstanceUnderLimit() {
			...
			string msg="You have exceeded the allowed number of concurrent Helianz Cloud sessions.\r\n"
			...
			if(MsgBox.Show(MsgBoxButtons.YesNo,Lan.g(this,msg))) {
				...
				if(ActiveInstances.GetCountCloudActiveInstances(ActiveInstances.GetActiveInstance()?.ActiveInstanceNum??0)>=PrefC.GetInt(PrefName.CloudSessionLimit)) {
					MsgBox.Show(this,"Your active sessions still exceed the allowed number of concurrent Helianz Cloud sessions. Closing this session.");
					Application.Exit();
					return false;
				}
			}
		...
		protected override void OnResize(EventArgs e){
			//FormHelianz_Resize wouldn't work because base needs to come first
			//Also, don't forget that this is not the only code that runs on resize.  See FormODBase.OnResize.
			base.OnResize(e);
			LayoutControls();
			if(Plugins.PluginsAreLoaded) {
				Plugins.HookAddCode(this,"FormHelianz.FormHelianz_Resize_end");
			}
		}
		...
		private void FormHelianz_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e) {
			...
			Plugins.HookAddCode(this,"FormHelianz_KeyDown_end",e);
		}
		...
		private bool TrySetPatient(long patNum) {
			...
			Plugins.HookAddCode(this,"FormHelianz.OnPatient_Click_end"); //historical name
			return true;
		}
		...
		private void LockODForMountAcquire(bool isEnabled){
			...
			//FormHelianz is included in the following list since it's not modal.
		...
		private void ProcessCommandLine() {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ClientDirect) {
				//Users are allowed to use eCW tight integration without command line.  They can manually launch Helianz.
		...
		private bool PrefsStartup() {
			//Default usePreviousVersions to false as this is only called after Helianz is already fully functional. No versions will have changed
		...
		private bool PrefsStartup(bool isSilentUpdate,ChooseDatabaseInfo chooseDatabaseInfo){
			...
			if(DateTime.Today<PrefC.GetDate(PrefName.TempFolderDateFirstCleaned).AddDays(7)) {
				PrefC.GetTempFolderPath();//We don't care about the return value.  Just trying to trigger the one-time cleanup and create the temp/helianz directory.
			}
		...
		private bool CheckProgramVersionsReportandReadOnly(bool isSilentUpdate) {
			...
			if((!string.IsNullOrEmpty(message))) {
				message+="Please fix this to run Helianz properly.";
				if(!isSilentUpdate) {
					MsgBox.Show(message);
				}
				return false;
		...
		private void LayoutToolBar() {
			...
			Plugins.HookAddCode(this,"FormHelianz.LayoutToolBar_end");
		...
		private void FillPatientButton(Patient patient){
			...
			if(Plugins.HookMethod(this,"FormHelianz.FillPatientButton_popups",patient,_listPopupEvents,patChanged)) {
				return;
			}
			if(!patChanged) {
				return;
		...
		private void toolButCommlog_Click() {
			if(Plugins.HookMethod(this,"FormHelianz.OnCommlog_Click",PatNumCur)) {
				return;
			}
		...
		private void SetSmsNotificationText(Signalod signalodSmsCount=null,bool doUseSignalInterval=true,int increment=0) {
			...
			try {
				...
				Plugins.HookAddCode(this,"FormHelianz.SetSmsNotificationText_end",_toolBarButtonText,menuItemTextMessagesReceived,increment);
			}
		...
		private void DataValid_BecameInvalid(Helianz.ValidEventArgs e) {
			...
			if(e.ITypes.Contains(InvalidType.Task) || e.ITypes.Contains(InvalidType.TaskPopup)) {
				Plugins.HookAddCode(this,"FormHelianz.DataValid_BecameInvalid_taskInvalidTypes");
		...
		private void FillSignalButtons(List<SigMessage> listSigMessages) {
			...
			if(lightGridHeightOld!=lightGridHeightNew) {
				lightSignalGrid1.Visible=false;//"erases" light signal grid that has been drawn on FormHelianz
		...
		private void lightSignalGrid1_ButtonClick(object sender,Helianz.UI.ODLightSignalGridClickEventArgs e) {
		...
		private void SignalsTick(bool isAllInvalidTypes=true) {
			...
			GetAndProcessSignals(
				...
				() => {
					...
					//E.g. When a user has two instances of Helianz open and they change the password in one of them.
			...
			//STOP! 
			//If you are trying to do something in FormHelianz that uses a signal, you should use FormHelianz.OnProcessSignals() instead.
		...
		public override void ProcessSignalODs(List<Signalod> listSignalods) {
			...
			#region WPF
			GlobalFormHelianz.ProcessSignalODs(listSignalods);
			#endregion WPF
			...
			Plugins.HookAddCode(this,"FormHelianz.ProcessSignals_end",listSignalods);
		}
		#endregion Signals

		#region Tasks
		///<summary>Will invoke a refresh of tasks on the only instance of FormHelianz. listRefreshedTaskNotes and listBlockedTaskLists are only used 
		...
		{
			_formHelianzSingleton.HandleRefreshedTasks(listSignalodTasks,listEditedTaskNums,listTasksRefreshed,listTaskNotesRefreshed,listUserOdPrefsBlockedTasks);
		}
		...
		private void moduleBar_ButtonClicked(object sender, Helianz.ButtonClicked_EventArgs e){
			switch(moduleBar.SelectedModule){
				case EnumModuleType.Appointments:
		...
		///When anything spawned by FormHelianz (main thread) tries to connect to the database and fails, this event will get fired.</summary>
		...
		///When anything spawned by FormHelianz (main thread) tries to connect to the MiddleTier server and fails, this event will fire.</summary>
		...
		public static void S_TaskGoTo(TaskObjectType taskObjectType,long keyNum) {
			_formHelianzSingleton.TaskGoTo(taskObjectType,keyNum);
		}
	...
	private void TaskGoTo(TaskObjectType taskObjectType,long keyNum){
			...
			if(taskObjectType==TaskObjectType.Appointment) {
				...
				GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, dateSelected:dateSelected,selectedAptNum:appointment.AptNum);
			}
		}
		...
		public static void S_SetPhoneStatusAvailable() {
			...
			_formHelianzSingleton.phoneSmall.SetAvailable(Employees.GetEmp(Security.CurUser.EmployeeNum).IsWorkingHome);
		}
		...
		private void phoneSmall_GoToChanged(object sender,EventArgs e) {
			...
			if(phoneEmpDefault!=null && phoneEmpDefault.IsTriageOperator) {
				if(Plugins.HookMethod(this,"FormHelianz.phoneSmall_GoToChanged_IsTriage",patient,phoneSmall.Extension)) {
					return;
				}
				Task task=new Task();
				task.TaskListNum=-1;//don't show it in any list yet.
				...
				if(Plugins.HookMethod(this,"FormHelianz.phoneSmall_GoToChanged_NotTriage",patient)) {
					return;
				}
				if(commlog==null) {
					commlog=new Commlog();
		...
		private void menuItemUserSettings_Click(object sender,EventArgs e) {
			FrmUserSetting frmUserSetting=new Helianz.FrmUserSetting();
		...
		private void menuItemLaboratories_Click(object sender,EventArgs e) {
			...
			if(Plugins.HookMethod(this,"FormHelianz.menuItemLaboratories_Click")) {
				return;
			}
		...
		private void menuItem_ProviderAllocatorSetup_Click(object sender,EventArgs e) {
			...
			using Reporting.Allocators.MyAllocator1.FormInstallAllocator_Provider fap = new Helianz.Reporting.Allocators.MyAllocator1.FormInstallAllocator_Provider();
			fap.ShowDialog();
		}*/
		...
		public static void S_MenuItemSecurity_Click(object sender,EventArgs e) {
			_formHelianzSingleton.menuItemSecurity_Click(sender,e);
		}
		...
		public static void S_GoToJob(long jobNum) {
			_formHelianzSingleton.GoToJob(jobNum);
		}

		///<summary>Can be called from anywhere in OD layer to load job. 
		///It is in FormHelianz because this is where the static reference to theJob Manager is.</summary>
		...
		private void menuItemLabCases_Click(object sender,EventArgs e) {
			...
			if(formLabCases.GoToAptNum!=0) {
				...
				GlobalFormHelianz.PatientSelected(patient,false);
				//OnPatientSelected(pat.PatNum,pat.GetNameLF(),pat.Email!="",pat.ChartNumber);
				GlobalFormHelianz.GoToModule(EnumModuleType.Appointments, dateSelected:appointment.AptDateTime,selectedAptNum:appointment.AptNum);
			}
		}
		...
		private void menuItemReportsGraphic_Click(object sender,EventArgs e) {
			...
			_formDashboardEditTab=new HelianzGraph.FormDashboardEditTab(Security.CurUser.ProvNum,!Security.IsAuthorized(EnumPermType.ReportProdIncAllProviders,true)) { IsEditMode=false };
		...
		private void formProcNotBilled_GoToChanged(ODEventArgs e) {
			...
			GlobalFormHelianz.PatientSelected(patient,false);
			GlobalFormHelianz.GoToModule(EnumModuleType.Account, claimNum:(long)e.Tag);
		}
		...
		private void menuItemTerminal_Click(object sender,EventArgs e) {
			...
			if(ODEnvironment.IsCloudServer) {
				//Thinfinity messes up window ordering so sometimes FormHelianz is visible in Kiosk mode.
				for(int i=0;i<Application.OpenForms.Count;i++) {
		...
		private void menuItemWiki_Click(object sender,EventArgs e) {
			if(Plugins.HookMethod(this,"FormHelianz.menuItemWiki_Click")) {
				return;
			}
		...
		private void menuItemZoom_Click(object sender,EventArgs e) {
			...
			Plugins.HookAddCode(this,"FormHelianz.menuItemZoom_Click_end",this.Menu);
		...
		public static void S_WikiLoadPage(string pageTitle) {
			if(!PrefC.GetBool(PrefName.WikiCreatePageFromLink) && !WikiPages.CheckPageNamesExist(new List<string>{ pageTitle })[0]) {
				MsgBox.Show("FormHelianz","Wiki page does not exist.");
				return;
			}
			FormWiki formWiki=new FormWiki();
			formWiki.Show();
		...
		public static void S_TaskNumLoad(long taskNum) {
			...
			if(task==null) {
				MsgBox.Show("FormHelianz","Task does not exist.");
				return;
			}
		...
		public static bool IsDashboardVisible {
			get {
				return (!_formHelianzSingleton.splitContainer.Panel2Collapsed && _formHelianzSingleton.userControlDashboard.IsInitialized);
			}
		}
		...
		private void _menuItemERouting_Click(object sender, EventArgs e) {
			if(!ClinicPrefs.IsODTouchAllowed(Clinics.ClinicNum)) {
				string site="https://www.helianz.com/site/odtouch.html";
				try{
					Process.Start(site);
				}
				catch{
		...
		///<summary>This is used to force the alert logic to run on the server in HelianzService.
		///HelianzService Alerts logic will re run on signal update interval time.
		...
		private void alertFormClosingHelper(object sender,FormClosedEventArgs e) {
			DataValid.SetInvalid(InvalidType.AlertItems);//THIS IS NOT CACHED. But is used to make server run the alert logic in HelianzService.
		}
		...
		private void menuItemRemote_Click(object sender,System.EventArgs e) {
			string site="https://www.helianz.com/site/contact.html";
		...
		private void menuItemHelpDocumentation_Click(object sender, System.EventArgs e) {
			string site="https://www.helianz.com/site/documentation.html";
			try {
				if(ODCloudClient.IsAppStream) {
					ODCloudClient.LaunchFileWithODCloudClient(site);
					return;
		...
		private void menuItemHelpSearch_Click(object sender, System.EventArgs e) {
			string site="https://www.helianz.com/site/searchsite.html";
			try {
				if(ODCloudClient.IsAppStream) {
					ODCloudClient.LaunchFileWithODCloudClient(site);
					return;
		...
		private void menuItemWebinar_Click(object sender,EventArgs e) {
			string site="https://helianz.com/site/webinars.html";
			try {
				if(ODCloudClient.IsAppStream) {
					ODCloudClient.LaunchFileWithODCloudClient(site);
					return;
				...
				#region Domain Login
					...
					try {
						...
						if(dictDomainUserNumsAndNames.Count==0) { //Log on normally if no user linked the current domain user
							...
							InputBox inputBox=new InputBox(Lan.g(this,"Select an Helianz user to log in with:"),dictDomainUserNumsAndNames.Select(x => x.Value).ToList());
							inputBox.ShowDialog();
		...
		///<summary>Opens the License Agreements for the user to sign. This prompting will only display when the user is a SecurityAdmin with a valid regkey. Closes Helianz if they refuse to sign. </summary>
		...
		///<summary>Returns a list of forms that are currently open, excluding FormHelianz, FormLogOn, and duplicates. This method is typically called in order to close any open forms except the aforementioned ones.  Therefore, the list returned is ordered with the intent that the calling method will close children first and then parents last.</summary>
		...
		private bool SaveWork(bool isForceClose) {
			...
			for(int i=0;i<listFormsOpen.Count;i++) {
				//If force closing, we HAVE to forcefully close everything related to Helianz, regardless of plugins.  Otherwise, give plugins a chance to stop the log off event.
				if(!isForceClose) {
					//This hook was moved into this method so that the form closing loop could be shared.
					//It is correctly named and was not updated to say "FormHelianz.CloseOpenForms" on purpose for backwards compatibility.
					if(Plugins.HookMethod(this,"FormHelianz.LogOffNow_loopingforms",listFormsOpen[i])) {
		...
		///Closes all open forms except FormHelianz.  Set isForceClose to true if you want to close all forms asynchronously.  Set 
		...
		private void FinishLogOff(bool isForced) {
			...
			Plugins.HookAddCode(this,"FormHelianz.FinishLogOff_start",isForced);//perform logoff 
		...
		private void InitiateShutdown(bool doShutdownAll=true) {
			...
			if(Security.CurUser==null) {
				Logger.WriteLine("Shutdown signal found while logged out. Closing Helianz.","Signals");
				ProcessKillCommand();
				return;
			}
			...
			string msg = "";
			if(!doShutdownAll) {
				msg="Your instance of Helianz ";
			}
			else if(Process.GetCurrentProcess().ProcessName=="Helianz") {
				msg+="All copies of Helianz ";
			}
			else {
		...
		public void ProcessKillCommand() {
			...
			CloseOpenForms(true);
			Application.Exit();//This will call FormHelianz's closing event which will clean up all threads that are currently running.
		}

		///<summary></summary>
		public static void S_ProcessKillCommand() {
			_formHelianzSingleton.ProcessKillCommand();
		}
		...
		private void SystemEvents_SessionSwitch(object sender,SessionSwitchEventArgs e) {
			...
			//CurUser will be null if Helianz is already in a 'logged off' state.  Check Security.IsUserLoggedIn as well because Middle Tier does not 
			//set CurUser to null when logging off.
			//Also catches the case where Helianz has NEVER connected to a database yet and checking PrefC would throw an exception (no db conn).
		...
		private void FormHelianz_Deactivate(object sender,EventArgs e) {
		...
		private void FormHelianz_FormClosing(object sender,FormClosingEventArgs e) {
		...
		private void FormHelianzClosing(object sender,FormClosingEventArgs e) {
			...
			//CloseOpenForms should have already been called with isForceClose=true if we are force closing Helianz
			//In that scenario, calling CloseOpenForms with isForceClose=false should not leave the program open.
			//However, if Helianz is closing from any other means, we want to give all forms the opportunity to stop closing.
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.SftpAtoZ) {
				HelianzCloud.Sftp.Download.CleanupStashes(true);//Cleanup current stash as well as stashes which are no longer in use.
			}
			string tempPath="";
			...
			try {
				...
				Plugins.HookAddCode(null,"FormHelianz.FormClosing_end");
				return;
			}
			...
			for(int i=listDirectories.Count-1;i>=0;i--) {//Easier than recursion.  Since the list is ordered ascending, then going backwards means we delete subdirectories before their parent directories.
				try {
					...
					//This folder will most likely get deleted next time Helianz closes.
				}
			}
			Plugins.HookAddCode(null,"FormHelianz.FormClosing_end");
		}

		private void FormHelianz_FormClosed(object sender,FormClosedEventArgs e) {
