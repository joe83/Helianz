...
namespace Helianz {
	public partial class UserControlReminderMessage:UserControl {
		...
		public List<string> ValidateTemplates() {
			...
			if(Rule.TypeCur==ApptReminderType.Arrival) {
				if(!textTemplateSms.Text.ToLower().Contains(HelianzBusiness.AutoComm.ArrivalsTagReplacer.ARRIVED_TAG.ToLower())) {
					listErrors.Add(Lan.g(this,$"Arrival texts must contain the \"{HelianzBusiness.AutoComm.ArrivalsTagReplacer.ARRIVED_TAG}\" tag."));
				}
			}
