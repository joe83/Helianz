...
namespace Helianz.User_Controls.SetupWizard {
	public partial class UserControlSetupWizPractice:SetupWizControl {
		...
		private void CheckIsDone() {
			...
			{
				//listProvider.BackColor = Helianz.SetupWizard.GetColor(ODSetupStatus.NeedsAttention);
				IsDone=false;
		...
		private void ControlDone(object sender,EventArgs e) {
				...
				|| CultureInfo.CurrentCulture.Name.EndsWith("CA")) //Canadian. en-CA or fr-CA)
			{
				...
				if(fax.Length!=0 && fax.Length!=10) {
					textFax.BackColor = Helianz.SetupWizard.GetColor(ODSetupStatus.NeedsAttention);
					IsDone=false;
				}
			}
			bool changed = false;
