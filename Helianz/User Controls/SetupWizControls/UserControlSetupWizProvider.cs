...
namespace Helianz.User_Controls.SetupWizard {
	public partial class UserControlSetupWizProvider:SetupWizControl {
		...
		private void FillGrid() {
			...
			Color needsAttnCol = Helianz.SetupWizard.GetColor(ODSetupStatus.NeedsAttention);
			...
			foreach(Provider prov in listProvs) {
				row = new GridRow();
				bool isDentist = Helianz.SetupWizard.ProvSetup.IsPrimary(prov);
		...
		private void timer1_Tick(object sender,EventArgs e) {
			if(_blink > 5) {
				...
				foreach(GridRow rowCur in gridMain.ListGridRows) {
					rowCur.ColorBackG=Helianz.SetupWizard.GetColor(ODSetupStatus.NeedsAttention);
				}
				gridMain.Invalidate();
				timer1.Stop();
				return;
			...
			foreach(GridRow rowCur in gridMain.ListGridRows) {
				rowCur.ColorBackG=rowCur.ColorBackG==Color.White?Helianz.SetupWizard.GetColor(ODSetupStatus.NeedsAttention):Color.White;
			}
			gridMain.Invalidate();
			_blink++;
		}
