...
namespace Helianz {
	public partial class UserControlReminderAgg:UserControl {
		...
		private List<string> ListTagsExludedFromArrivalResponseComeIn => new List<string> { HelianzBusiness.AutoComm.ArrivalsTagReplacer.ARRIVED_TAG };
		...
		public List<string> ValidateTemplates() {
			...
			if(Rule.TypeCur==ApptReminderType.Arrival) {
				...
				if(!textSMSAggShared.Text.ToLower().Contains(HelianzBusiness.AutoComm.ArrivalsTagReplacer.ARRIVED_TAG.ToLower())) {
					errors.Add(groupBoxSMSAggShared.Text+Lan.g(this,$" must contain the \"{HelianzBusiness.AutoComm.ArrivalsTagReplacer.ARRIVED_TAG}\" tag."));
				}
