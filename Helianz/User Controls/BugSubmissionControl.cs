	...
	public partial class BugSubmissionControl:UserControl {
		...
		private void FillOfficeInfoGrid(BugSubmission sub){
			...
			if(sub.IsMobileSubmission) {
				...
				gridOfficeInfo.ListGridRows.Add(new GridRow("ODBdllVersion",sub.Info.HelianzBusinessVersion?.ToString()));
				gridOfficeInfo.ListGridRows.Add(new GridRow("ODBdllMTVersion",sub.Info.HelianzBusinessMiddleTierVersion?.ToString()??"INVALID"));
		...
		private void butGoToAccount_Click(object sender,EventArgs e) {
			...
			GlobalFormHelianz.GoToModule(EnumModuleType.Account,patNum:_patCur.PatNum);
		}
