...
*/
		...
		private List<HelianzBusiness.Pearl.EnumCategoryOD> _listPearlCategoriesShown=new List<HelianzBusiness.Pearl.EnumCategoryOD>();
		...
		public void SetShowDrawings(bool showOD,bool showPearl,List<HelianzBusiness.Pearl.EnumCategoryOD> listEnumCategoryODs){
		...
		public void ToolBarPaste_Click(object sender,EventArgs e){
			...
			if(IsMountShowing()){
				if(bitmapPaste!=null){
					...
					if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(doc.DocCategory)) {
						HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,doc,bitmapPaste,mountItem:_listMountItems[_idxSelectedInMount]);
					...
					for(int i=0;i<stringArrayfileNames.Length;i++) {
						...
						if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(doc.DocCategory)) {
							HelianzBusiness.Bridges.Pearl pearl = HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,doc,filePath:stringArrayfileNames[i],mountItem:listAvail[i]);
			...
			if(bitmapPaste!=null){
				...
				if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,bitmapPaste);
				...
				for(int i=0;i<stringArrayfileNames.Length;i++) {
					...
					if(stringArrayfileNames.Length>1){
						...
						if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
							HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayfileNames[i]);
		...
		private void panelMain_DragDrop(object sender,DragEventArgs e) {
			...
			for(int i=0;i<stringArrayFiles.Length;i++) {
				...
				Documents.Update(document,documentOld);
				if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayFiles[i],mountItem:listMountItemsAvail[i]);
					pearl?.SendOnThread();
				}
			}//for
			EventSelectTreeNode?.Invoke(this,null);
		...
		private void DrawDrawings(float rotateText,Graphics g=null,XGraphics xg=null){
			...
			for(int i=0;i<_listImageDraws.Count;i++){
				if(!_showDrawingsOD && _listImageDraws[i].ImageAnnotVendor==EnumImageAnnotVendor.Helianz){
					continue;
				}
		...
		private int HitTestHover(PointF pointfBitmap){
			...
			for(int i=0;i<_listImageDraws.Count;i++){
				...
				if(_listImageDraws[i].PearlLayer==HelianzBusiness.Pearl.EnumCategoryOD.Measurements
		...
		private void ToolBarImportMount(){
			...
			for(int i=0;i<stringArrayFileNames.Length;i++) {
				...
				Documents.Update(document,documentOld);
				if(HelianzBusiness.Bridges.Pearl.DoAutoUploadForImageCategory(document.DocCategory)) {
					HelianzBusiness.Bridges.Pearl pearl=HelianzBusiness.Bridges.Pearl.SetupPearlForSendingSingle(PatientCur,document,filePath:stringArrayFileNames[i],mountItem:listMountItemsAvail[i]);
					pearl?.SendOnThread();
				}
