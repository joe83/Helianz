...
<#
.SYNOPSIS
	Builds the OpenDental main client application and moves binaries to a dedicated output folder.

.PARAMETER Configuration
	Build configuration: Debug or Release (default: Release)

.PARAMETER OutputDir
	Destination folder for built binaries (default: .\Output\OpenDental)

.PARAMETER MsBuildPath
	Full path to MSBuild.exe. Auto-detected from Visual Studio if not provided.

.EXAMPLE
	.\Build-OpenDental.ps1
	.\Build-OpenDental.ps1 -Configuration Debug -OutputDir "C:\Deploy\OpenDental"
#>
param(
	[ValidateSet('Debug','Release')]
	[string]$Configuration = 'Release',

	[string]$OutputDir = "$PSScriptRoot\Output\OpenDental",
...
Write-Host "  OpenDental Client Build Script" -ForegroundColor Cyan
...
$projectFile = Join-Path $PSScriptRoot "OpenDental\OpenDental.csproj"
if (-not (Test-Path $projectFile)) {
	throw "Project not found: $projectFile"
}

$binDir = Join-Path $PSScriptRoot "OpenDental\bin\$Configuration"
...
# ---------------------------------------------------------------------------
Write-Host ""
Write-Host "[STEP 2/3] Building OpenDental ($Configuration)..." -ForegroundColor Yellow
...
$buildArgs = @(
	...
	"/verbosity:minimal",
	"/fl",                   # file logger
	"/flp:LogFile=`"$PSScriptRoot\build-opendental.log`";Verbosity=normal"
)

& $MsBuildPath @buildArgs
if ($LASTEXITCODE -ne 0) {
	Write-Host ""
	Write-Error "Build FAILED. Check build-opendental.log for details."
	exit 1
}
