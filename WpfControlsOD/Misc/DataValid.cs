...
namespace Helianz{
	///<summary>Handles a global event to keep local data synchronized. Tied to Signalod table in the database. In other words, it's used to tell other computers in the office about a change. Mostly for cache, but also for some non-cached types. Also see CodeBase.ODEvent, which is being deprecated. Also, see GlobalFormHelianz, which is used for user actions for things like switching modules or refreshing screen data. If you have a window that needs to process incoming signals from the db, see FormODBase.ProcessSignalODs and FormFrame.ProcessSignalODs.</summary>
	public class DataValid{
		///<summary>FormHelianz subscribes to this event. Frms fire this event to cause event handler to run.</summary>
		...
		public static void SetInvalid(params InvalidType[] arrayITypes) {
			...
			//FormHelianz.S_DataValid_BecomeInvalid();
		}
