	...
	static class Program {
		...
		static void Main(string[] args) {
			if(ODBuild.IsDebug()) {
				...
				Application.Run(new FormDebug("HelianzHL7"));
			}
			else {
				EventLog.WriteEntry("HelianzHL7.Main", DateTime.Now.ToLongTimeString() +" - Service main method starting...");
				ServiceHL7 serviceHL7=new ServiceHL7();
				serviceHL7.ServiceName="HelianzHL7";//default
				//Get the executing assembly location directory (location of this HelianzHL7.exe)
				...
				//Get all installed services that have names that start with "Helianz"
				for(int i=0;i<serviceControllersAll.Length;i++) {
					if(serviceControllersAll[i].ServiceName.StartsWith("Helianz")) {
				...
				EventLog.WriteEntry("HelianzHL7.Main",DateTime.Now.ToLongTimeString() +" - Service main method exiting...");
			}
		}
	}
}
