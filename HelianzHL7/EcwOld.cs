		...
		private void EcwOldProcessMessage(string fullPath) {
			...
			try {
				...
				if(msg.MsgType==MessageTypeHL7.ADT) {
					if(IsVerboseLogging) {
						EventLog.WriteEntry("HelianzHL7","Processed ADT message",EventLogEntryType.Information);
					}
					...
					if(IsVerboseLogging) {
						EventLog.WriteEntry("HelianzHL7","Processed SUI message",EventLogEntryType.Information);
					}
