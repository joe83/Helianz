...
namespace HelianzBusiness {
	...
	public class ImageStore {
			...
			if(CloudStorage.IsCloudStorage) {
				...
				try {
					if(string.IsNullOrEmpty(AtoZpath)) {
						...
						//CreateDirectory treats this as a relative path and the full path would be "C:\Program Files (x86)\Helianz\P\PatientAustin1".
						throw new ApplicationException("AtoZpath was null or empty");
					}
					if(!Directory.Exists(retVal)) {
						Directory.CreateDirectory(retVal);
		...
		///<summary>Surround with try/catch.  Typically returns something similar to \\SERVER\HelianzImages\EmailImages.
		...
		public static string GetHashString(Document doc,string patFolder,bool includeFileInHash=true) {
			...
			if(PrefC.AtoZfolderUsed==DataStorageType.InDatabase){
				patFolder=ODFileUtils.CombinePaths(Path.GetTempPath(),"helianz");
		...
		public static Document ImportForm(string form,long docCategory,Patient pat) {
			...
			try {
				if(CloudStorage.IsCloudStorage) {
					//byte[] bytes=HelianzBusiness.FileIO.FileAtoZ.ReadAllBytes(pathSourceFile);
					//HelianzBusiness.FileIO.FileAtoZ.WriteAllBytes(CloudStorage.PathTidy(ODFileUtils.CombinePaths(patFolder,doc.FileName)),bytes);
		...
		///<summary> Save a Document to another location on the disk (outside of Helianz). </summary>
		...
		///<summary> Save an Eob to another location on the disk (outside of Helianz). </summary>
		...
		///<summary> Save an EHR amendment to another location on the disk (outside of Helianz). </summary>
