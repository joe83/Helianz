	...
	public class EcwSIU {
		public static void ProcessMessage(MessageHL7 message,bool isVerboseLogging) {
			...
			if(apt.PatNum != pat.PatNum) {
				EventLog.WriteEntry("HelianzHL7","Appointment does not match patient: "+pat.FName+" "+pat.LName
			...
			if(pat.FName=="" || pat.LName=="") {
				EventLog.WriteEntry("HelianzHL7","Appointment not processed due to missing patient first or last name. PatNum:"+pat.PatNum.ToString()
					,EventLogEntryType.Information);
				return;//this will also skip the appt insert.
			}
			if(isNewPat) {
				if(isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted patient: "+pat.FName+" "+pat.LName+", PatNum:"+pat.PatNum.ToString()
						,EventLogEntryType.Information);
				}
				...
				if(isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated patient: "+pat.FName+" "+pat.LName,EventLogEntryType.Information);
				}
			...
			if(isNewApt) {
				if(isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted appointment for: "+pat.FName+" "+pat.LName,EventLogEntryType.Information);
				}
				...
				if(isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated appointment for: "+pat.FName+" "+pat.LName,EventLogEntryType.Information);
				}
