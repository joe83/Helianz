...
namespace HelianzBusiness {
	public class BillingL {
		...
		public static void RunFromODService(LogWriter logWriter,Action<SendStatementsIO> actionMock) {
			...
			sendStatementsIO.Source="HelianzService";
			...
			sendStatementsIO.FuncChooseSaveFile=(initialSaveDirectory) => { throw new Exception("HelianzService should not be prompted to select an XML file."); };
			sendStatementsIO.ActionProgressEvent=(odEventArgs) => { };
			sendStatementsIO.FuncGetBillingDataTable=() => { throw new Exception("HelianzService should not be allowed to pause/resume."); };
		...
		///Changes to this method will also need to be made to HelianzService.ProcessStatements.SendEmails().</summary>
		private static bool PrintBatch(SendStatementsIO sendStatementsIO) {
			...
			for(int i=0; i<sendStatementsIO.CurStatementBatch.ListStatements.Count; i++) {
				...
				//imageStore = Helianz.Imaging.ImageStore.GetImageStore(pat);
		...
		///<summary>Sends text messages to the current batch of statements. Changes to this method will also need to be made to HelianzService.ScheduledProcessThread.SendTextMessages().
		...
		///This method closely mimics Helianz.FormBillingOptions.CreateHelper(). Changes to this method will also need to be considered there.</summary>
	...
	public class SendStatementsIO {
		...
		///This had previously been stored as a list of dictionaries, making it the most hideous use of dictionary ever recorded in Helianz lore.
	...
	public class ClinicBillingOptions {
		...
		///<summary>Sets the all of the billing preferences for the clinic passed in. The preferences used for this method are the same ones used in Helianz.FormBillingOptions.cs. Changes to the preference list will also need to be modified in Helianz.FormBillingOptions.cs.</summary>
