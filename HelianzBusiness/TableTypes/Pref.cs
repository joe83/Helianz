...
namespace HelianzBusiness {
	...
	public enum PrefName {
		...
		AccountingLockDate,
		///<summary>Enum:AccountingSoftware 0=Helianz, 1=QuickBooks, 2=QuickBooksOnline</summary>
		AccountingSoftware,
		...
		ADPRunIID,
		///<summary>Stored as DateTime, but cleared when aging finishes. Used only in HelianzService for automatic aging. The DateTime will be used as a flag to signal other
		...
		AgingServiceTimeDue,
		///<summary>How often to check for new alerts in Helianz. Defaults to 180 (3 minutes).</summary>
		...
		///<summary>No UI, can only be manually enabled by a programmer.  Only used for debugging electronic statements, because it will bloat the HelianzImages folder.  Originally created to help with the "missing brackets bug" for EHG billing.</summary>
		...
		///<summary>Boolean, 0 by default. When true, Helianz Service will receive claim reports instead of the specified computer spawning a thread
		///in FormHelianz.</summary>
		...
		///Stores the time of day that the HelianzService should create a claimsnapshot.</summary>
		...
		///<summary>When a number of sessions of Helianz Cloud is within the maximum minus this number an Alert will be created.</summary>
		...
		///<summary>Local preference. The maximum number of sessions of Helianz Cloud allowed to be running simultaneously.</summary>
		CloudSessionLimit,
		///<summary>Hidden preference. False for cloud users using Thinfinity. Controls how Helianz communicates with the ODCloudClient. Set to False to use the original HTTP system with Thinfinity. If altered a restart is required as this value is set to a static variable on startup</summary>
		CloudIsAppStream,
		...
		CorruptedDatabase,
		///<summary>This is the default encounter code used for automatically generating encounters when specific actions are performed in Helianz.  The code is displayed/set in FormEhrSettings.  We will set it and give the user a list of 9 suggested codes to use such that the encounters generated will cause the pateint to be considered part of the initial patient population in the 9 clinical quality measures tracked by OD.  CQMDefaultEncounterCodeSystem will identify the code system this code is from and the code value will be a FK to that code system.</summary>
		...
		///<summary>Used by OD HQ.  Not added to db convert script.  Will be passed to HelianzEConnector when service initialized.  Specifies the time (in minutes) between each time that the listener service will upload it's current heartbeat to HQ.  Initially 360</summary>
		CustListenerHeartbeatFrequencyMinutes,
		///<summary>Used by HelianzEConnector.  String specifies which port the HelianzWebService should look for on the customer's server in order to create a socket connection.  Initially 25255</summary>
		CustListenerPort,
		///<summary>Used by OD HQ.  Not added to db convert script.  Will be passed to HelianzEConnector when service initialized.  Specifies the read/write socket timeout.  Initially 3000</summary>
		...
		DoLimitTaskSignals,
		/// <summary>Boolean. Determine whether or not to allow users to bypass HelianzLogin using ActiveDirectory. Default is false.</summary>
		DomainLoginEnabled,
		...
		///<summary>Deprecated.  Not used anywhere. Email messages are retreived from Helianz Service.</summary>
		[Obsolete("This preference has been deprecated. Do not use.",true)]
		...
		///<summary>Boolean, false by default.  When true, allows automated family balancer to run from HelianzService.</summary>
		[PrefName(ValueType=PrefValueType.BOOL)]
		...
		HasVerboseLogging,
		///<summary>Encrypted.  Has no UI to change it, but status can be viewed in FormSupportStatus.  Used to validate whether customer is on support and should have access to Help, Sparks3D, Imaging, etc.  See the HelianzHelp project.</summary>
		HelpKey,
		HL7FolderOut,
		HL7FolderIn,
		///<summary>Deprecated.  Use SiteLink.EmployeeNum instead.  Used by HQ. Projected onto wall displayed on top of FormMapHQ</summary>
		HQTriageCoordinator,
		///<summary>Only used at Helianz HQ. Not added with script.</summary>
		HQVideoToken,
		...
		///<summary>Enum:InsBlueBookAnonShareEnable 0 by default. 0=Off, 1=OffNoPrompt, 2=On When on, insurance payment data will be anonymously shared with Helianz for the purpose of making improvements to the Blue Book feature.</summary>
		[PrefName(ValueType=PrefValueType.ENUM)]
		...
		///Never directly ask the cache for this preference.  Instead, use the Introspection class within the Misc folder of the HelianzBusiness project in order to access it.</summary>
		IntrospectionItems,
		///<summary>Preference to show writeoffs in the StatementInvoicePayment grid.</summary>
		InvoicePaymentsGridShowNetProd,
		///<summary>True if there is a row in the ehrprovkey table.  The HelianzService will check this preference and if it is false it will not query
		...
		///<summary>Records the name of the computer running HelianzService.</summary>
		HelianzServiceComputerName,
		///<summary>Records a heartbeat for HelianzService. If the heartbeat is older than 6 minutes, an alert will notify the user.</summary>
		HelianzServiceHeartbeat,
		HelianzVendor,
		OracleInsertId,
		...
		///<summary>Bool, false by default. If true, then HelianzService will run recurring charges on a set schedule.</summary>
		...
		///that recurring charges have started and prevents HelianzService from running repeating charges.</summary>
		...
		SmsMonthlyLimit,
		/// <summary>Name of this Software.  Defaults to 'Helianz Software'.</summary>
		SoftwareName,
		SolidBlockouts,
		SpellCheckIsEnabled,
		...
		///<summary>Boolean, false by default. When enabled, main title of FormHelianz uses clinic abbr instead of description</summary>
		...
		TpUnearnedType,
		///<summary>The date and time when the HelianzService last sent update messages for account debits and credits.</summary>
		...
		///<summary>The time of day for the HelianzService to update Transoworld (TSI) with all payments and other debits and credits for families
		///where the guarantor has been sent to TSI for collection.</summary>
		TransworldServiceTimeDue,
		///<summary>FK to definition.DefNum.  Billing type the HelianzService will change guarantors to once an account is paid in full.</summary>
		...
		TreatPlanShowIns,
		///<summary>This preference merely defines what FormHelianz.IsTreatPlanSortByTooth is on startup.
	...
	///<summary>Used by pref "AccountingSoftware".  0=Helianz, 1=QuickBooks, 2=QuickBooksOnline</summary>
	...
	public enum DatabaseModeEnum {
		...
		Normal,
		///<summary>Helianz is hosting the database.</summary>
		Cloud,
	}
