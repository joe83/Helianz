...
namespace HelianzBusiness{
	...
	public class MedicationPat:TableBase {
		...
		///<summary>If eRx is used to prescribe a medication, a medication order is imported automatically into Helianz.  If a provider is logged in, then this is CPOE (Computerized Provider Order Entry), and this will be true.   Or, if a provider is logged in and Rx entered through OD, it's also CPOE.  If a staff person is logged in, and enters an Rx through NewCrop or OD, then this is non-CPOE, so false.</summary>
		public bool IsCpoe;
