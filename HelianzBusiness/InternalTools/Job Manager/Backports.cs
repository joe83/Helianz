...
namespace HelianzBusiness {
	public class Backports {
		...
		//(e.g. helianz18.1 will be helianz). Add the project to BackportProjects and add it to the list in that class. Be sure to include the
		...
		public static class BackportProjects {
			...
			public static BackportProject Helianz=new BackportProject(ProjectName.helianz,MajorMinorPattern.MajorDotMinor,HeadPattern.head);
			public static BackportProject HelianzHelp=new BackportProject(ProjectName.HelianzHelp,MajorMinorPattern._Major_Minor,HeadPattern.head);
			public static BackportProject HelianzService=new BackportProject(ProjectName.HelianzService,MajorMinorPattern._Major_Minor,
				HeadPattern.head);
			public static BackportProject HelianzWebApps=new BackportProject(ProjectName.HelianzWebApps,MajorMinorPattern._Major_Minor,
				HeadPattern.head);
			...
			public static List<BackportProject> ListProjects=new List<BackportProject> { Bridges,CDT,EHR,ODCrypt,Helianz,HelianzHelp,
				HelianzService,HelianzWebApps,PhoneTrackingServer,ODXam,ODCloudAdmin,Unknown };
		}
			...
			public void GetFilePaths(List<string> listVersions,BackportProject currentProject,string headString) {
				if(currentProject.PatternMajor==MajorMinorPattern.MajorDotMinor) {
					...
					foreach(string version in listVersions) {
						...
						else if(currentProject.Name==ProjectName.HelianzWebApps) {
							//Related to the ODXam replacement. ODXam business also has folder name changes. This allows backporting to work.
							versionPath=versionPath.Replace("HelianzBusiness.ODXam",$"HelianzBusiness.ODXam_{v.Major}_{v.Minor}");
						}
