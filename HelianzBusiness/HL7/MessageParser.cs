	...
	public class MessageParser {
		...
		public static void Process(MessageHL7 msg,bool isVerboseLogging) {
			...
			if(_isEcwHL7Def &&  (string.IsNullOrEmpty(patLName) || string.IsNullOrEmpty(patFName))) {
				EventLog.WriteEntry("HelianzHL7","Message not processed due to missing first or last name. PatNum:"+patNum.ToString()
			...
			if(def.InternalType!=HL7InternalType.eCWFull && def.InternalType!=HL7InternalType.eCWTight && pat==null && chartNum!=null) {
				...
				if(pat!=null) {
					...
					if(_isVerboseLogging) {
						EventLog.WriteEntry("HelianzHL7","Updated patient "+pat.GetNameFLnoPref()+" to include ChartNumber.",EventLogEntryType.Information);
					}
				}
			}
			...
			if(_isNewPat) {
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted patient with PatNum="+pat.PatNum,EventLogEntryType.Information);
				}
				patOld=pat.Copy();
				if(pat.Guarantor==0) {
					...
					Patients.Update(pat,patOld);
					if(_isVerboseLogging) {
						EventLog.WriteEntry("HelianzHL7","Updated patient with PatNum="+pat.PatNum+", updated Guarantor from 0 to "+pat.Guarantor+".",
							EventLogEntryType.Information);
					}
					patOld=pat.Copy();
				}
			...
			for(int i=0;i<hl7defmsg.ListHL7DefSegments.Count;i++) {
				try {
					...
					for(int j=0;j<listSegments.Count;j++) {//normally only 1 or 0 in the list, but if it is a repeatable segment the list may contain multiple segments to process
						if(_isVerboseLogging) {
							EventLog.WriteEntry("HelianzHL7","Process segment "+hl7defmsg.ListHL7DefSegments[i].SegmentName.ToString(),EventLogEntryType.Information);
						}
			...
			if(pat.FName=="" || pat.LName=="") {
				EventLog.WriteEntry("HelianzHL7","Patient demographics not processed due to missing first or last name. PatNum:"+pat.PatNum.ToString()
					,EventLogEntryType.Information);
				...
				Patients.Update(pat,patOld);
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated patient "+pat.GetNameFLnoPref(),EventLogEntryType.Information);
				}
			...
			if(!_isEcwHL7Def && msg.MsgType==MessageTypeHL7.SRM) {
				if(msg.AckEvent=="S04") {
					...
					if(_isVerboseLogging) {
						EventLog.WriteEntry("HelianzHL7","Appointment broken due to inbound SRM message with event type S04 for patient "+pat.GetNameFLnoPref(),EventLogEntryType.Information);
					}
				}
		...
		public static void ProcessAIGorAIP(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg) {
			...
			_aptProcessed=apt;
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated appointment for patient "+pat.GetNameFLnoPref()+" due to an incoming AIG or AIP segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessAIL(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg) {
			...
			_aptProcessed=apt;
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated appointment for patient "+pat.GetNameFLnoPref()+" due to an incoming AIL segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessAL1(Patient pat,HL7DefSegment segDef,SegmentHL7 seg) {
				...
				switch(segDef.hl7DefFields[i].FieldName) {
					...
					case "allergenRxNorm":
						if(seg.GetFieldComponent(intItemOrder,2).ToLower()!="rxnorm") {
							...
							EventLog.WriteEntry("HelianzHL7","A drug allergy was not added for patient "+pat.GetNameFLnoPref()
			...
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Inserted a new allergy for patient "+pat.GetNameFLnoPref()+" due to an incoming AL1 segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessARQ(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			if(externAptId!="" && externRoot!="" && OIDExternals.GetByRootAndExtension(externRoot,externAptId)==null) {
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Added an external appointment ID to the oidexternals table due to an incoming ARQ segment.\r\n"
			...
			HL7Msgs.Update(_hl7MsgCur);
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated hl7msg to include AptNum for "+pat.GetNameFLnoPref()+" due to an incoming SCH segment.",EventLogEntryType.Information);
			}
			_aptProcessed=apt;
			return;
		}
		...
		public static void ProcessGT1(Patient pat,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			if(guarPatNumOrdinal==-1 && guarChartNumOrdinal==-1 && guarIdsOrdinal==-1) {
				...
				HL7Msgs.Update(_hl7MsgCur);
				EventLog.WriteEntry("HelianzHL7","Guarantor not processed.  guar.PatNum, guar.ChartNumber, or guarIdList must be included in the GT1 definition.  "
			...
			if(guarNameOrdinal==-1) {
				...
				HL7Msgs.Update(_hl7MsgCur);
				EventLog.WriteEntry("HelianzHL7","Guarantor not processed due to guar.nameLFM not included in the GT1 definition.  Patnum of patient: "
			...
			if(seg.GetFieldComponent(guarNameOrdinal,0)=="" || seg.GetFieldComponent(guarNameOrdinal,1)=="") {
				...
				HL7Msgs.Update(_hl7MsgCur);
				EventLog.WriteEntry("HelianzHL7","Guarantor not processed due to missing first or last name.  PatNum of patient: "
					+pat.PatNum.ToString(),EventLogEntryType.Information);
				return;
			}
			...
			if(guarPatNum==0 && guarChartNum=="" && guarPatNumFromIds==0 && listOids.Count==0) {//because we have an example where they sent us this (position 2 (guarPatNumOrder or guarChartNumOrder for eCW) is empty): GT1|1||^^||^^^^||||||||
				...
				HL7Msgs.Update(_hl7MsgCur);
				EventLog.WriteEntry("HelianzHL7","Guarantor not processed due to missing guar.PatNum, guar.ChartNumber, and guarIdList.  "
			...
			if(isNewGuar) {
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted patient "+guar.GetNameFLnoPref()+" when processing a GT1 segment.",EventLogEntryType.Information);
				}
			}
			else {
				Patients.Update(guar,guarOld);
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated patient "+guar.GetNameFLnoPref()+" when processing a GT1 segment.",EventLogEntryType.Information);
				}
			}
			...
			if(_isVerboseLogging && strVerboseMsg.Length>0) {
				EventLog.WriteEntry("HelianzHL7","Added the following external patient ID(s) to the oidexternals table due to an incoming GT1 segment for PatNum: "+guar.PatNum+"."
		...
		public static void ProcessNTE(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			_aptProcessed=apt;
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated appointment for patient "+pat.GetNameFLnoPref()+" due to an incoming NTE segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessOBX(Patient pat,HL7DefSegment segDef,SegmentHL7 seg) {
			...
			for(int i=0;i<segDef.hl7DefFields.Count;i++) {
				...
				switch(segDef.hl7DefFields[i].FieldName) {
					case "medicationRxNorm":
						if(seg.GetFieldComponent(intItemOrder,2).ToLower()!="rxnorm") {
							//if not an RXNORM code, do nothing.  Only RXNORM codes are currently supported
							EventLog.WriteEntry("HelianzHL7","A medication was not added for patient "+pat.GetNameFLnoPref()
			...
			if(rxnorm==0) {
				EventLog.WriteEntry("HelianzHL7","A medication was not added for patient "
			...
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Inserted a new medication for patient "+pat.GetNameFLnoPref()+" due to an incoming OBX segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessPID(Patient pat,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			for(int i=0;i<segDef.hl7DefFields.Count;i++) {
				...
				switch(segDef.hl7DefFields[i].FieldName) {
					...
					case "patientIds"://Not used by eCW
						...
						if(strPatOIDRoot=="") {
							//if they have not set their internal OID root, we cannot identify which repetition in this field contains the OD PatNum
							EventLog.WriteEntry("HelianzHL7","Could not process the patientIdList field due to the internal OID for the patient object missing.",EventLogEntryType.Information);
							continue;
						}
						string strPatIdCur="";
						...
						if(_isVerboseLogging && verboseMsg.Length>0) {
							EventLog.WriteEntry("HelianzHL7","Added the following external patient ID(s) to the oidexternals table due to an incoming PID segment for PatNum: "
			...
			Patients.Update(pat,patOld);
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated patient "+pat.GetNameFLnoPref()+" due to an incoming PID segment.",EventLogEntryType.Information);
			}
			return;
		}
		...
		public static void ProcessPR1(Patient pat,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
				...
				switch(segDef.hl7DefFields[i].FieldName) {
					case "proccode.ProcCode":
						...
						if(seg.GetFieldComponent(itemOrder,2).ToLower()!="cd2" || seg.GetFieldComponent(itemOrder,6)!="2014") {
							EventLog.WriteEntry("HelianzHL7","A procedure was not added for patient "+pat.GetNameFLnoPref()
			...
			switch(procCode.TreatArea) {
				case TreatmentArea.Arch:
					if(strSurf!="L" && strSurf!="U") {
						EventLog.WriteEntry("HelianzHL7","A procedure was not added for patient "+pat.GetNameFLnoPref()+".  The treatment area for the code "
				...
				case TreatmentArea.ToothRange:
					...
					for(int i=0;i<listToothNums.Length;i++) {
						...
						if(Tooth.IsPrimary(Tooth.Parse(listToothNums[i]))) {
							EventLog.WriteEntry("HelianzHL7","A procedure was not added for patient "+pat.GetNameFLnoPref()+".  The treatment area for the code "+procCode.ProcCode
			...
			if(procCur.ProcStatus==ProcStat.C && dateProc.Date > DateTime.Today.Date && !PrefC.GetBool(PrefName.FutureTransDatesAllowed)) {
				EventLog.WriteEntry("HelianzHL7", "A procedure was not added for patient "+pat.GetNameFLnoPref()+". Procedures are not allowed for future dates.",EventLogEntryType.Information);
				return;
			}
			...
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Inserted a new procedure for patient "+pat.GetNameFLnoPref()+" due to an incoming PR1 segment.",EventLogEntryType.Information);
			}
			...
			if(strOidExt!="" && strOidExtRoot!="") {
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Added an external procedure ID to the oidexternals table due to an incoming PR1 segment.\r\nProcNum: "
		...
		public static void ProcessPRB(Patient pat,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			if(probActionOrder<0 || probCodeOrder<0 || probUniqueIdOrder<0) {
				EventLog.WriteEntry("HelianzHL7","The PRB segment was not processed.  The segment must have an action, code, and unique ID defined.",EventLogEntryType.Information);
				return;
			}
			...
			if(seg.GetFieldComponent(probActionOrder).ToLower()!="ad" && seg.GetFieldComponent(probActionOrder).ToLower()!="up") {
				EventLog.WriteEntry("HelianzHL7","The PRB segment was not processed.  The action codes supported are 'AD' for add or 'UP' for update.",EventLogEntryType.Information);
				return;
			}
			...
			if(seg.GetFieldComponent(probCodeOrder,2).ToLower()!="snm" || probDefNum==0) {
				EventLog.WriteEntry("HelianzHL7","The PRB segment was not processed.  "
			...
			if(_isVerboseLogging && markedInactiveCount>0) {
				EventLog.WriteEntry("HelianzHL7","Updated "+markedInactiveCount.ToString()+" problems to a status of inactive due to an incoming PRB segment.",EventLogEntryType.Information);
			}
			...
			if(probCur.DiseaseNum==0) {//new problem
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted a new problem for patient "+pat.GetNameFLnoPref()+" due to an incoming PRB segment.",EventLogEntryType.Information);
				}
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Added an external problem ID to the oidexternals table due to an incoming PRB segment.\r\nDiseaesNum: "
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated an existing problem for patient "+pat.GetNameFLnoPref()+" due to an incoming PRB segment.",EventLogEntryType.Information);
				}
			}
			return;
		}
		...
		public static void ProcessPV1(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg) {
			...
			if(_isVerboseLogging) {
				EventLog.WriteEntry("HelianzHL7","Updated patient "+pat.GetNameFLnoPref()+" due to an incoming PV1 segment.",EventLogEntryType.Information);
			}
			if(apt!=null) {
				Appointments.Update(apt,aptOld);
				_aptProcessed=apt;
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated appointment for patient "+pat.GetNameFLnoPref()+" due to an incoming PV1 segment.",
						EventLogEntryType.Information);
				}
			}
			return;
		...
		public static long ProcessSCH(Patient pat,Appointment apt,HL7DefSegment segDef,SegmentHL7 seg,MessageHL7 msg) {
			...
			if(isNewApt) {
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Inserted appointment "+(isBreakApt?"with broken status ":"")+"for "+pat.GetNameFLnoPref()+" due to an incoming SCH segment.",EventLogEntryType.Information);
				}
				...
				if(_isVerboseLogging) {
					EventLog.WriteEntry("HelianzHL7","Updated appointment for "+pat.GetNameFLnoPref()+" due to an incoming SCH segment.",EventLogEntryType.Information);
				}
				Appointments.Update(apt,aptOld);
