	...
	public class CentralConnections {
			...
			,bool noShowOnStartup=false,List<string> listAdminCompNames=null,bool isCommandLineArgs=false,bool useDynamicMode=false,bool allowAutoLogin=true) 
		{
			try {
				...
				xmlWriter.WriteEndElement();
				if(HelianzBusiness.WebTypes.Shared.XWeb.XWebs.UseXWebTestGateway) {
		...
		public static ChooseDatabaseInfo GetChooseDatabaseConnectionSettings() 
		{
			...
			//1. /home/username/.helianz/config.xml (or corresponding user path in Windows)
			//2. /etc/helianz/config.xml (or corresponding machine path in Windows) (should be default for new installs) 
			...
			try {
				...
				if(xPathNavigator2!=null) {
					HelianzBusiness.WebTypes.Shared.XWeb.XWebs.UseXWebTestGateway=xPathNavigator2.Value.ToLower()=="true";
				}
				...
				/* example:
				<ServerConnection>
					<URI>http://server/HelianzServer/ServiceMain.asmx</URI>
				...
				*/
				...
				if(ODBuild.IsTrial()) {
					...
					centralConnection.DatabaseName="helianz";
				}
			...
			//IsAccessibleFromMainMenu is set in FormHelianz
