...
namespace HelianzBusiness {
	...
	public class BugSubmissions {
		...
		public static long Insert(BugSubmission bugSubmission) {
			...
			long retVal=0;
			//Always use the connection store config file because creating BugSubmissions should always happen via HelianzWebServiceHQ.
			DataAction.RunBugsHQ(() => { 
		...
		public static BugSubmissionResult SubmitException(Exception ex,out string displayMsg,string threadName="",long patNumCur=-1,string moduleName="") {
			...
			if(bugSubmission.RegKey=="7E57-1NPR-0DUC-710N") {
				...
				programVersion=bugSubmission.Info.HelianzBusinessVersion;
				webServiceHqURL="https://www.patientviewer.com:49997/HelianzWebServiceHQ/WebServiceMainHQ.asmx";
			}
