...
namespace HelianzBusiness{
	...
	public class AlertItems{
		...
		///<summary>Checks to see if the heartbeat for Helianz Service was within the last six minutes. If not, an alert will be sent telling 
		///users Helianz Service is down.</summary>
		public static void CheckODServiceHeartbeat() {
			...
			if(!IsODServiceRunning()) {//If the heartbeat is over 6 minutes old, send the alert if it does not already exist
				...
				List<AlertItem> listAlertItemsOld=RefreshForType(AlertType.HelianzServiceDown);
				if(listAlertItemsOld.Count==0) {//an alert does not already exist
					...
					alertItem.Description=Lans.g("Alerts","No instance of Helianz Service is running.");
					alertItem.Type=AlertType.HelianzServiceDown;
		...
		public static bool IsODServiceRunning() {
			...
			string command="SELECT ValueString,NOW() FROM preference WHERE PrefName='HelianzServiceHeartbeat'";
