	...
	public class InsVerifies {
		...
		private static List<InsVerifyGridObject> TableToListInsVerifyGridObjects(DataTable table,bool isForMedicaid) {
			...
			for(int i=0;i<table.Rows.Count;i++) {
				...
				insVerify.ClinicNum=PIn.Long(row["ClinicNum"].ToString());//Non DB column. Used in HelianzService.
				insVerify.InsSubNum=PIn.Long(row["InsSubNum"].ToString());//Non DB column. Used in HelianzService.
				insVerify.CarrierNum=PIn.Long(row["CarrierNum"].ToString());//Non DB column. Used in HelianzService.
		...
		///<summary>Called in HelianzService.  Attempts to verify patient benefits for same list in FormInsVerificaitonList.cs.
		...
		///<summary>This method determines whether an insurance adjustment needs to be made given the current benefits in Helianz and the EB271 segments from the 271 response.
		///An insurance adjustment will be made if there is an individual general deductible or annual max specified in Helianz and the 
