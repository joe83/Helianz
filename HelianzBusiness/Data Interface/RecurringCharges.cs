	...
	public class RecurringChargerator {
			...
			List<long> listClinicNumsBadCredentials,out StringBuilder strBuilderResultText,out double amount,out StringBuilder receipt) 
		{
			...
			if(ODEnvironment.IsCloudServer) {
				MarkFailed(chargeData,Lans.g(_lanThis,"XCharge is not available while using Helianz Cloud."),LogLevel.Error);
				return false;
			}
			long clinicNumCur=0;
		...
		/// <summary>Charges the card using version 1 of PayConnect. This method is equivalent to the SendPayConnect() method in prior versions of Helianz.</summary>
