...
namespace HelianzBusiness {
	...
	public class Statements{
		...
		public static double GetFamilyBalance(long patNum) {
			...
			List<Patient> listPatients=HelianzBusiness.Patients.GetFamily(patNum).ListPats.ToList();
			string command="SELECT * FROM statement "
										+"WHERE PatNum IN("+string.Join(",",listPatients.Select(x => POut.Long(x.PatNum)).ToList())+") "
										+"AND IsSent=1 "
										+"ORDER BY StatementNum DESC LIMIT 1";
			Statement statement=HelianzBusiness.Crud.StatementCrud.SelectOne(command);
			if(statement==null) {
				return 0;
			}
