...
namespace HelianzBusiness{
	...
	public class Appointments {
		...
		public static List<AppointmentWithServerDT> GetAppointmentsForApi(int limit,int offset,DateTime dateTStart,DateTime dateTEnd,DateTime dateTStamp,long clinicNum,long patNum,int aptStatus,long operatoryNum){
			...
			DateTime dateTimeServer=PIn.DateT(HelianzBusiness.Db.GetScalar(commandDatetime));//run before appts for rigorous inclusion of appts
		...
		public static DataTable GetWebSchedAppointmentsForApi(int limit,int offset,DateTime dateTStart,DateTime dateTEnd,DateTime dateTStamp,long clinicNum,string dateTimeFormatString) {
			...
			for(int i=0;i<tableAppointments.Rows.Count;i++) {//Paging handled in query.
				...
				dataRow["confirmed"]=HelianzBusiness.Defs.GetName(HelianzBusiness.DefCat.ApptConfirmed,PIn.Long(tableAppointments.Rows[i]["Confirmed"].ToString()));
				...
				dataRow["unschedStatus"]=HelianzBusiness.Defs.GetName(HelianzBusiness.DefCat.RecallUnschedStatus,PIn.Long(tableAppointments.Rows[i]["UnschedStatus"].ToString()));
		...
		///Used by multiple applications so be very careful when changing this method.  E.g. Helianz and Web Sched.</summary>
		...
		public static void ValidateApptForWeb(Appointment appointment) {
			...
			try {
				...
				switch((ApptSchedEnforceSpecialty)odex.ErrorCode) {
					case ApptSchedEnforceSpecialty.Warn:
						throw new ODException(odex.Message+"\r\nTo schedule appointment anyway, please use the desktop application of Helianz.");
					case ApptSchedEnforceSpecialty.Block:
						throw new ODException(odex.Message);
				}
			}
			...
			,PrefName prefNameEmailType,bool logErrors=true) 
		{
			try {
				...
				string emailSubj=HelianzBusiness.EmailMessages.SubjectTidy(ClinicPrefs.GetPrefValue(prefNameEmailSubj,appointment.ClinicNum));
				...
				string emailBody=HelianzBusiness.EmailMessages.BodyTidy(ClinicPrefs.GetPrefValue(prefNameEmailBody,appointment.ClinicNum));
