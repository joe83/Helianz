...
namespace HelianzBusiness {
	...
	public class Patients {
		...
		///<summary>Used by the HelianzService Transworld thread to sync accounts sent to collection.</summary>
		...
		public static List<Family> GetFamilies(List<long> listPatNums) {
			...
			if(listPatNums==null || listPatNums.Count < 1) {
				return new List<HelianzBusiness.Family>();
			}
			...
			DateTime birthdate,int patStatus,long clinicNum,DateTime dateTStamp,long priProv,int gender,int position,long guarantor,long superFamily,long employerNum)
		{
			...
			DateTime dateTimeServer=PIn.DateT(HelianzBusiness.Db.GetScalar(commandDatetime));//run before patients for rigorous inclusion of patient
			List<Patient> listPatients=Crud.PatientCrud.SelectMany(command);
		...
		///<summary>Used only by the HelianzService Transworld thread to sync accounts sent for collection.  Gets a list of PatAgings for the guars
		...
		public static bool MergeTwoPatientPointOfNoReturn(long patTo,long patFrom,List<PatField> patFieldsToDelete,List<PatField> patFieldsToUpdate) {
			...
			//in the merge tool after an update to Helianz. This will allow our customers to remerge the missing
