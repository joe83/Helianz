...
namespace HelianzBusiness {
	...
	public class Userods {
		...
		public static void LoadDatabaseInfoFromFile(string configFilePath){
			...
			XPathNavigator xPathNavigatorConfigFileNode=xPathNavigatorConn.SelectSingleNode("ApplicationName");//usually /HelianzServer
			if(xPathNavigatorConfigFileNode==null) {//when first updating, this node will not exist in the xml file, so just add it.
				try {
					//AppendChild does not affect the position of the XPathNavigator; adds <ApplicationName>/HelianzServer<ApplicationName/> to the xml
				...
				try {
					xPathNavigatorConfigFileNode.SetValue(HostingEnvironment.ApplicationVirtualPath);//sets value to /HelianzServer or whatever they named their app
					xmlDocument.Save(configFilePath);
				}
				catch { }//do nothing, unable to write to the XML file, move on anyway
			}
		...
		///<summary>DEPRICATED DO NOT USE.  Use HelianzBusiness.Authentication class instead.  For middle tier backward-compatability only.</summary>
		...
		public static void Validate(bool isNew,Userod userod,bool excludeHiddenUsers,List<long> listUserGroupNums) {
			...
			if(!userod.IsHidden) {//if the user is now not hidden
				//CEMT users will not be visible from within Helianz.  Therefore, make a different check so that we can know if the name
