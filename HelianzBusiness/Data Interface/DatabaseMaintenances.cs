...
namespace HelianzBusiness {
	public class DatabaseMaintenances {
		...
		public static string MySQLServerOptionsValidate(bool verbose,DbmMode dbmMode) {
			...
			string warningMessage=Lans.g("FormDatabaseMaintenance","There are some global MySQL variables that are not set correctly. A manual fix is required to prevent data loss. For more information please check the web manual at")+" https://helianz.com/manual/myini.html ";
			string issueLog="";
			switch(dbmMode) {
		...
		public static string SpecialCharactersInNotes(bool verbose,DbmMode dbmMode) {
			...
			List<Patient> listPatients=HelianzBusiness.Crud.PatientCrud.SelectMany(command);
			intC=0;
			...
			switch(dbmMode) {
				...
				case DbmMode.Fix:
					if(numFound==0) {
						...
						long ebillNum=HelianzBusiness.Crud.EbillCrud.Insert(ebill);
						if(ebillNum>0) {
		...
		//		log+="Error: Unable to load types from HelianzBusiness. Try running the method on a different computer.\r\n";
		//		if(verbose) {
		...
		//		if(type.Namespace!="HelianzBusiness" || !type.IsClass) {
		//			continue;
		//		}
		...
		//			//This error log is explicitly for Helianz engineers and is purposefully not translated.
...
HAVING cnt>1";
		...
		public static string OrthoChartDeleteDuplicates() {
			...
			//The group by clause must use the keyword BINARY because the ortho chart within Helianz is case sensitive.
			string command=@"
		...
		///This also includes going into the HelianzImages folder where the attachments are stored and delete the files there as well.</summary>
