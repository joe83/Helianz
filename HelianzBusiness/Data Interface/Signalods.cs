...
namespace HelianzBusiness {
	...
	public class Signalods {
		...
		public static List<SignalodForApi> GetSignalOdsForApi(int limit,int offset,DateTime dateTSince,List<InvalidType> listInvalidTypes=null){
			...
			DateTime dateTimeServer=PIn.DateT(HelianzBusiness.Db.GetScalar(commandDatetime));//run before signals for rigorous inclusion of signals
			try {
				listSignalods=Crud.SignalodCrud.SelectMany(command);
			} 
			catch {
		...
		///<summary>Process all Signals and Acks Since a given DateTime.  Only to be used by HelianzWebService.
		...
		public static List<Signalod> GetInvalidSignalsForWeb() {
			try {
				...
				ODException.SwallowAnyException(() => dateTimeLastRefreshed=HelianzBusiness.MiscData.GetNowDateTime());
