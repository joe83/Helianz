...
namespace HelianzBusiness{
	...
	public class Clinics {
		...
		///<summary>Currently active clinic within Helianz.  Reflects FormHelianz.ClinicNum</summary>
		...
		///<summary>The currently selected clinic's ClinicNum.  Unlike the one stored in FormHelianz, this is accessible from the business layer.</summary>
		...
		///<summary>Called when logging user off or closing helianz.</summary>
		...
		public static void Delete(Clinic clinic) {
			...
			if(table.Rows.Count>0) {
				...
				throw new Exception(Lans.g("Clinics","Cannot delete clinic because the following Helianz users are using it:")+userNames);
			}
			#endregion
			#region AlertSub
			...
			if(table.Rows.Count>0) {
				...
				throw new Exception(Lans.g("Clinics","Cannot delete clinic because the following Helianz users are subscribed to it:")+"\r"+String.Join("\r",listUsers.OrderBy(x => x).ToList()));
			}
			#endregion
			#region UserClinics
		...
		///<summary>Provide the currently selected clinic num (FormHelianz.ClinicNum).  If clinics are not enabled, this will return true if the pref
		///PracticeIsMedicalOnly is true.  If clinics are enabled, this will return true if either the headquarters 'clinic' is selected
		///(FormHelianz.ClinicNum=0) and the pref PracticeIsMedicalOnly is true OR if the currently selected clinic's IsMedicalOnly flag is true.
		...
		public static void ReplaceOffice(StringBuilder stringBuilder,Clinic clinic,bool isHtmlEmail=false,bool replaceDisclaimer=false) {
			...
			if(replaceDisclaimer) {
				ReplaceTags.ReplaceOneTag(stringBuilder,"[EmailDisclaimer]",HelianzBusiness.EmailMessages.GetEmailDisclaimer(clinic?.ClinicNum??0),isHtmlEmail);
			}
		}
		...
		public static long GetClinicNumForApi(string strClinicNum) {
			...
			Clinic clinic=HelianzBusiness.Clinics.GetClinicNoCache(clinicNum);
			if(clinic is null) {
				return -1;
			}
			return clinic.ClinicNum;
