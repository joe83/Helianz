...
using HelianzBusiness.SheetFramework;
...
namespace HelianzBusiness {
	public class Procedures {
		...
		///an ehrprovkey.  Only used by the HelianzService AlertRadiologyProceduresThread.</summary>
		...
		public static List<ProcedureForApi> GetProcsOneAptForApi(long aptNum,DateTime dateTStamp) {
			...
			DateTime dateTimeServer=PIn.DateT(HelianzBusiness.Db.GetScalar(commandDatetime));//run before procedures for rigorous inclusion of procedures
	...
	public class ProcExtended {
		...
		public double InsPayTotal {
			get { return ClaimProcs.Where(x => HelianzBusiness.ClaimProcs.GetInsPaidStatuses().Contains(x.Status)).Sum(x => x.InsPayAmt); }
		}

		public double WriteOffTotal {
			get { return ClaimProcs.Where(x => HelianzBusiness.ClaimProcs.GetInsPaidStatuses().Contains(x.Status)).Sum(x => x.WriteOff); }
		}

		public double WriteOffEstTotal {
			get {
				return ClaimProcs.Where(x => HelianzBusiness.ClaimProcs.GetEstimatedStatuses().Contains(x.Status))
