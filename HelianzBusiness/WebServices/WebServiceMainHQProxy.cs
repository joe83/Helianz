...
namespace HelianzBusiness {
	...
	public static class WebServiceMainHQProxy {
		...
		///Currently, HelianzREST and WebHostSynch hard-code the URL.</summary>
		public static IWebServiceMainHQ GetWebServiceMainHQInstance(string webServiceHqUrl="") {
			...
			//Right click HelianzBusiness project, Properties, Settings, HelianzBusiness_WebServiceMainHq_Mock = True.
			//Set this application setting to False if you want to debug using a locally hosted WebServiceMainHQ.
			if(ODBuild.IsDebug() && Properties.Settings.Default.HelianzBusiness_WebServiceMainHq_Mock) {
			...
			if(webServiceHqUrl.IsNullOrEmpty() && ODBuild.IsDebug()) {
				...
				if(!ODCloudClient.IsAppStream){
					service.Url="http://localhost/HelianzWebServiceHQ/WebServiceMainHQ.asmx";//localhost
				}
				//service.Url="https://patientviewer.com:49997/HelianzWebServiceHQ/WebServiceMainHQ.asmx";//Live server
				if(Environment.MachineName.ToLower()=="jordanhome"){
					service.Url="https://patientviewer.com:49997/HelianzWebServiceHQ/WebServiceMainHQ.asmx";
				}
			}
		...
		///<summary>Takes a list of preferences from HQ and, if a corresponding HelianzBusiness.Pref exists, updates Pref.ValueString.</summary>
      ...
      PhoneNumber: {phone.PhoneNumber}
         ...
         InactiveCode: {phone.InactiveCode}";
			...
			long eServiceSignalNum=HelianzBusiness.EServiceSignals.Insert(new EServiceSignal() { 
		...
		public class EServiceSetup {
			...
			public class SignupOut {
				...
				///<summary>Used to push values to HelianzBusiness.Pref entries at the dental office.</summary>
		...
		/// <summary> Takes in an helianz address. Packs it up and sends it off to USPS to validate. This service is available to all customers on support. </summary>
		...
		///<summary>Proxy class that interfaces with HelianzWebCore.EClipboard2FactorAuthFP</summary>
		[Serializable]
