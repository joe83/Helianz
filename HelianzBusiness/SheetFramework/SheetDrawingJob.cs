...
namespace HelianzBusiness {
	public class SheetDrawingJob {
			...
			foreach(SheetField field in sheet.SheetFields) {
				...
				switch(field.FieldType) {
					...
					case SheetFieldType.Special:
						HelianzBusiness.SheetPrinting.DrawFieldSpecial(sheet,field,null,gx,yPos);
						break;
			...
			bool isPrinting=false,long eraClaimSegmentIdx=-1,DataTable table=null,Patient pat=null,Patient patGuar=null,float scaleMS=1) 
		{
			...
			Sheets.SetPageMargin(sheet,_printMargin);
			Helianz.UI.GridOD odGrid=new Helianz.UI.GridOD();//Only used for measurements, also contains printing/drawing logic.
			odGrid.BeginUpdate();
		...
		public void DrawFieldImage(SheetField field,Graphics g,XGraphics gx,Bitmap image=null) {
			...
			if(image!=null) {
				...
				if(field.FieldValue.StartsWith("MountNum:")){
					...
					bmpOriginal=HelianzBusiness.Properties.Resources.Patient_Info;
		...
		private static void DrawMedLabFooter(Sheet sheet,Graphics g,XGraphics gx,int pagesPrinted,int yPosPrint, MedLab medLab) {
			...
			fieldCur.XPos=50;
			int pageCount;
			fieldCur.YPos=HelianzBusiness.SheetPrinting.BottomCurPage(yPosPrint+_printMargin.Bottom+_printMargin.Top+1,sheet,out pageCount)+1;
			fieldCur.Width=625;
			fieldCur.Height=20;
