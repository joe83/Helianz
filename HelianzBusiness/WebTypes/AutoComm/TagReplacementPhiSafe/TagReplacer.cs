...
namespace HelianzBusiness.AutoComm {
	public class TagReplacer {
		...
		protected void ReplaceOneTag(StringBuilder template,string tagToReplace,string replaceValue,bool isEmailBody) {
			HelianzBusiness.ReplaceTags.ReplaceOneTag(template,tagToReplace,replaceValue,isEmailBody);
		}
		...
		public virtual string ReplaceTags(string template,AutoCommObj autoCommObj,Clinic clinic,bool isEmailBody) {
			...
			ReplaceOneTag(retVal,"[ProvName]",HelianzBusiness.Providers.GetFormalName(autoCommObj.ProvNum),isEmailBody);
			ReplaceOneTag(retVal,"[ProvAbbr]",HelianzBusiness.Providers.GetAbbr(autoCommObj.ProvNum),isEmailBody);
			long clinicNum=0;
			if(clinic!=null) {
				clinicNum=clinic.ClinicNum;
			}
			ReplaceOneTag(retVal,"[EmailDisclaimer]",HelianzBusiness.EmailMessages.GetEmailDisclaimer(clinicNum),isEmailBody);
