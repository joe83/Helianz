	...
	public class EConnectorStatistics : WebBase {
		...
		public static void UpdateEConnectorStats() {
			...
			if(PrefC.HasClinicsEnabled) {
				eConnStats.CountActiveClinics=HelianzBusiness.Clinics.GetCount();
				eConnStats.CountInactiveClinics=HelianzBusiness.Clinics.GetCount()-eConnStats.CountActiveClinics;
			}
			else {
				eConnStats.CountActiveClinics=0;
				eConnStats.CountInactiveClinics=HelianzBusiness.Clinics.GetCount();
			}
			if(DateTime.Now.Hour==0) { //These are heavy queries so only run them once a day around midnight.
				eConnStats.CountActivePatients=HelianzBusiness.Procedures.GetCountPatsComplete(DateTime.Today.AddYears(-2),DateTime.Today);
				eConnStats.CountNonactivePatients=HelianzBusiness.Patients.GetPatCountAll()-eConnStats.CountActivePatients;
				eConnStats.ListEServiceSignals=HelianzBusiness.EServiceSignals.GetServiceHistory(eServiceCode.ListenerService,DateTime.Today.AddDays(-30),
					DateTime.Today,30);
			}
			...
			HelianzBusiness.Prefs.UpdateString(PrefName.EConnectorStatistics,statsStr);
