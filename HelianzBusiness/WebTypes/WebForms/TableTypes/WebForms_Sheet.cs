...
namespace HelianzBusiness.WebTypes.WebForms {
	[Serializable]
	[CrudTable(CrudLocationOverride=@"..\..\..\HelianzBusiness\WebTypes\WebForms\Crud",NamespaceOverride="HelianzBusiness.WebTypes.WebForms.Crud",CrudExcludePrefC=true)]
	public class WebForms_Sheet:TableBase {
		...
		///<summary>Enum:SheetTypeEnum  The type of sheet.  Only PatientForm, MedicalHistory, and Consent sheet types are supported in Web Forms. !!!NOTE!!! The SheetType field was originally created with the wrong enumeration type (SheetFieldType). The enumeration was corrected but this class is used by older versions of Helianz which will continue to use SheetFieldType values. Therefore, always treat the value passed in as if it were a SheetFieldType first. If the value is not a SheetFieldType, then it will be the correct SheetTypeEnum type. E.g. Older versions are expecting the value in the XML payload to be a string representation of SheetFieldType values (SigBox instead of PatientForm).</summary>
		[XmlIgnore]
    ...
    public WebForms_Sheet Copy(){
		...
		///<summary>Used only for serialization purposes. The SheetType field was originally created with the wrong enumeration type (SheetFieldType). The enumeration was corrected but this class is used by older versions of Helianz which will continue to use SheetFieldType values. Therefore, always treat the value passed in as if it were a SheetFieldType first. If the value is not a SheetFieldType, then it will be the correct SheetTypeEnum type.</summary>
		...
		public string SheetFieldTypeXml {
			get {
				...
				//Older versions of Helianz are expecting the string representation of the SheetFieldType enum value.
