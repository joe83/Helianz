...
namespace HelianzBusiness.WebTypes.Shared.XWeb {
	public class XWebs {
		...
		internal abstract class XWebInputAbs {
			...
			public XWebResponse GenerateOutput(string email="",string logGuid="",bool doThrowWhenOnlinePaymentsDisabled=true) {
				Patient pat=HelianzBusiness.Patients.GetPat(_patNum);
				if(pat==null) {
				...
				if(!HelianzBusiness.PrefC.HasClinicsEnabled) { //Patient.ClinicNum is unreliable if clinics have been turned off.
					_clinicNum=0;
				}
				HelianzBusiness.WebTypes.Shared.XWeb.WebPaymentProperties xwebProperties;
		...
		private class XWebInputDTGPaymentSale:XWebInputDTG {
			...
			protected override void PostProcessOutput(XWebResponse response) {
				...
				if(InsertPaymentOnApproval) { //Insert Payment, PaySplit, and set FK.					
					response.PaymentNum=HelianzBusiness.Payments.InsertFromXWeb(
