...
namespace HelianzBusiness {
	public class DtoProcessor {
		...
		///<summary>The string representation of the AssemblyVersion for HelianzBusiness.dll.</summary>
		private static string _strHelianzBusinessVersion=null;
		...
		///Set serverMapPath to the root directory of the HelianzServerConfig.xml.  Typically Server.MapPath(".") from a web service.
		...
		public static string ProcessDto(string dtoString,string serverMapPath="") {
			...
			try {
				...
				if(!_isMiddleTierInitialized) {
					...
					//if we later call CDT.Class1.Decrypt inside a thread, we won't we able to find HelianzServer.dll in the call stack.
					ODException.SwallowAnyException(() => CDT.Class1.Decrypt("odv2e$fakeciphertext",out _));
					_strHelianzBusinessVersion=Assembly.GetAssembly(typeof(Db)).GetName().Version.ToString();
				...
				if(string.IsNullOrEmpty(DataConnection.GetServerName()) && string.IsNullOrEmpty(DataConnection.GetConnectionString())) {
					...
					//the application virtual path is usually /HelianzServer, but may be different if hosting multiple databases on one IIS server
					...
					{
						//either configFilePath not set or file doesn't exist, default to HelianzServerConfig.xml
						configFilePath=ODFileUtils.CombinePaths(serverMapPath,"HelianzServerConfig.xml");
					}
				...
				if(_strHelianzBusinessVersion!=_strProgramVersion && !ODBuild.IsDebug()) {
					...
					if(_strHelianzBusinessVersion!=_strProgramVersion) {
						//The version mismatch issue is still present.
						throw new ODException($"Version mismatch.  Middle Tier Server: '{_strHelianzBusinessVersion}'  Database: '{_strProgramVersion}'");
					}
				}
				#endregion
				...
				#region DtoGetObject
					...
					if(dtoInformation.ClassName=="Security" && dtoInformation.MethodName=="LogInWeb") {
						dtoInformation.Parameters[2]=new DtoObject(serverMapPath,typeof(string));//because we can't access this variable from within HelianzBusiness.
						RemotingClient.MiddleTierRole=MiddleTierRole.ServerMT;
		...
		private class DtoInformation {
			...
			public DtoInformation(DataTransferObject dto) {
				...
				AssemblyName=FullNameComponents[0];//HelianzBusiness or else a plugin name
