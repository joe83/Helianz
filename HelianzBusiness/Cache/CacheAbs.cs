	...
	public abstract class CacheAbs<T> where T : TableBase {
		...
		public DataTable GetTableFromCache(bool doRefreshCache) {
			...
			if(IsCacheNull()) {
				...
				//Helianz has custom controls that ask the preference cache questions in event handlers. E.g. ContentsResized() for spell checking within our custom text box control.
			...
			if(doRefreshCache) {
				...
				//Fall back to filling this cache from the local database connection since cache is so important for Helianz to function properly.
				FillCache(FillCacheSource.Database,null);
			}
			return CacheToTable();
		}
