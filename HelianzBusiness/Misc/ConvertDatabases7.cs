...
using static HelianzBusiness.LargeTableHelper;//so you don't have to type the class name each time.
...
namespace HelianzBusiness {
	public partial class ConvertDatabases {
		...
		private static void To21_2_1() {
			...
			try {
				if(IsUsingReplication()) {
					string replicationMonitorMsg="Monitoring the slave status is now monitored by the HelianzReplicationService. "
					+"Each replication server will need the new HelianzReplicationService installed. "
					+"Please visit https://helianz.com and search for 'Slave Monitor' for more information.";
		...
		private static void To21_2_25() {
			...
			Db.NonQ(command);
			command="INSERT INTO preference(PrefName,ValueString) VALUES('EmailHostingEndpoint','https://emailhosting.helianz.cloud:8200/')";
			Db.NonQ(command);
