	...
	public class ItransNCpl {
		...
		public static void UpdateCarriersFromJsonList(ItransNCpl iTransNCpl,bool isAutomatic=true,ItransImportFields fieldsToImport=ItransImportFields.None) {
			...
			foreach(ItransNCpl.Carrier jsonCarrier in iTransNCpl.ListCarriers) {//Update carriers.
				...
				List<HelianzBusiness.Carrier> listDbCarriers=Carriers.GetAllByElectId(jsonCarrier.Bin).FindAll(x => x.IsCDA);
					...
					|| x.CarrierName.ToLower()==jsonCarrier.Name.Fr.ToLower())))
				{
					...
					HelianzBusiness.Carrier carrierNew=new HelianzBusiness.Carrier();
		...
		private static ItransNCpl.Carrier GetJsonCarrierForUpdate(ItransNCpl iTransNCpl,HelianzBusiness.Carrier carrierOD) {
		...
		///<summary></summary>
		private static void UpdateCarrierInDb(HelianzBusiness.Carrier odCarrier,Carrier jsonCarrier,List<CanadianNetwork> listCanadianNetworks
			,ItransImportFields fieldsToImport,string jsonCarrierPhone,bool isAutomatic)
		{
			HelianzBusiness.Carrier odCarrierOld=odCarrier.Copy();
		...
		private static void TrySetCanadianNetworkNum(Carrier jsonCarrier,HelianzBusiness.Carrier odCarrier,List<CanadianNetwork> listCanadianNetworks) {
