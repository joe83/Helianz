	...
	public partial class ConvertDatabases{
		...
		private static void To3_7_4() {
			if(FromVersion < new Version("3.7.4.0")) {
				...
				string command="INSERT INTO program (ProgName,ProgDesc,Enabled,Path,CommandLine,Note"
					...
					+"'"+POut.String("\""+@"C:\Program Files\EasyNotesPro\DefaultDentalToolbar.etb"+"\""+" Helianz false")+"', "
		...
		private static void To3_9_5() {
			if(FromVersion < new Version("3.9.5.0")) {
				...
				string command="INSERT INTO preference VALUES ('BackupRestoreAtoZToPath', '"+POut.String(@"C:\HelianzData\")+"')";
				Db.NonQ32(command);
		...
		private static void To5_4_0() {
			if(FromVersion<new Version("5.4.0.0")) {
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					...
					Db.NonQ32(command);
					command="INSERT INTO preference VALUES('ImageStore', 'Helianz.Imaging.FileStore')";
					Db.NonQ32(command);
					//PT Dental Bridge---------------------------------------------------------------------------
					command="INSERT INTO program (ProgName,ProgDesc,Enabled,Path,CommandLine,Note"
						+") VALUES("
