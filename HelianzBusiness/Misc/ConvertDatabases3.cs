...
#region Create Code Systems Tables
		...
		private static void To14_2_1() {
			if(FromVersion<new Version("14.2.1.0")) {
				...
				//Add new SoftwareName preference for EHR report printing. Defaults to Helianz Software.
				if(DataConnection.DBtype==DatabaseType.MySql) {
					command="INSERT INTO preference(PrefName,ValueString) VALUES('SoftwareName','Helianz Software')";
					Db.NonQ(command);
				}
				else {//oracle
					command="INSERT INTO preference(PrefNum,PrefName,ValueString) VALUES((SELECT MAX(PrefNum)+1 FROM preference),'SoftwareName','Helianz Software')";
					Db.NonQ(command);
				}
		...
		private static void To14_2_21() {
			if(FromVersion<new Version("14.2.21.0")) {
				...
				//Come to find out it was due to a redirect issue.  We're going to update the Uri to point to helianz.com instead of helianz.com so that this doesn't happen again.
				command=@"UPDATE preference SET ValueString='http://www.helianz.com/updates/' WHERE PrefName='UpdateWebsitePath' AND ValueString='http://www.helianz.com/updates/'";
				Db.NonQ(command);
		...
		private static void To14_2_23() {
			if(FromVersion<new Version("14.2.23.0")) {
				...
				string command;
				//We fixed the (403) Forbidden error by getting rid of the redirect, changing the A record for helianz.com and pointing it to HQ.
				//Therefore, we now want users to be pointing to helianz instead of helianz.  This simply undoes what happened at the end of the 14.2.21 method.
				command=@"UPDATE preference SET ValueString='http://www.helianz.com/updates/' WHERE PrefName='UpdateWebsitePath' AND ValueString='http://www.helianz.com/updates/'";
				Db.NonQ(command);
		...
		private static void To15_3_1() {
			if(FromVersion<new Version("15.3.1.0")) {
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					command="INSERT INTO preference(PrefName,ValueString) VALUES ('WebServiceHQServerURL','http://www.patientviewer.com:49999/HelianzWebServiceHQ/WebServiceMainHQ.asmx')";
					Db.NonQ(command);
				}
				else {//oracle
					command="INSERT INTO preference(PrefNum,PrefName,ValueString) VALUES((SELECT MAX(PrefNum)+1 FROM preference),'WebServiceHQServerURL','http://www.patientviewer.com:49999/HelianzWebServiceHQ/WebServiceMainHQ.asmx')";
					Db.NonQ(command);
				}
				if(DataConnection.DBtype==DatabaseType.MySql) {
        ...
        string podiumProgramNum=Db.GetScalar(command);
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					command="INSERT INTO programproperty (ProgramNum,PropertyDesc,PropertyValue"
							...
							+"'Enter 0 to use Helianz for sending review invitations, or 1 to use eConnector', "
							+"'0')";
					Db.NonQ(command);
				}
				else {//oracle
