		...
		private static void CanadaDeleteDuplicatePreauthClaimprocs() {
			...
			try {
				...
				for(int i=1;i<table.Rows.Count;i++) {
					ClaimProc claimProcPrev=new HelianzBusiness.ClaimProc() {
					...
					ClaimProc claimProcCur=new HelianzBusiness.ClaimProc() {
...
No Action Required in many cases, check your new patient Web Sched on your web site to see behavior. See online manual for details.";
		...
		private static void To17_4_22() {
			...
			//Add TransworldPaidInFullBillingType preference for use by the HelianzService. No default, requires user interaction.
			if(DataConnection.DBtype==DatabaseType.MySql) {
		...
		private static void To17_4_23() {
			...
			if(DataConnection.DBtype==DatabaseType.MySql) {
				command="ALTER TABLE providererx ADD AccountId varchar(25) NOT NULL";//The account ids are generated by Helianz and dont need to be 255 long
				Db.NonQ(command);
			}
			else {//oracle
				command="ALTER TABLE providererx ADD AccountId varchar2(25)";//The account ids are generated by Helianz and dont need to be 255 long
				Db.NonQ(command);
			}
			if(DataConnection.DBtype==DatabaseType.MySql) {
				command="ALTER TABLE clinicerx ADD AccountId varchar(25) NOT NULL";//The account ids are generated by Helianz and dont need to be 255 long
				Db.NonQ(command);
			}
			else {//oracle
				command="ALTER TABLE clinicerx ADD AccountId varchar2(25)";//The account ids are generated by Helianz and dont need to be 255 long
				Db.NonQ(command);
			}
		}
		...
		private static void To18_1_32() {
			string command="SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'helianzarchive'";
			DataTable table=Db.GetTable(command);
			if(table.Rows.Count==1) {
				...
				if(dateArchive.Year > 1880) {
					//This is most likely the database that had its data archived into the helianzarchive database.
					//Rename the helianzarchive database to '[currentdatabasename]_archive' so that multiple databases on the same server can archive.
					LargeTableHelper.RenameDatabase("helianzarchive",(Db.GetScalar("SELECT DATABASE()")+"_archive"));
				}
			}
		}
		...
		private static void To18_2_0() {
			...
			command=@"UPDATE preference SET ValueString='https://helianzsoft.com/WebHostSynch/SheetsSynch.asmx'
				WHERE PrefName='WebHostSynchServerURL' 
				AND ValueString='https://helianzsoft.com/WebHostSynch/Sheets.asmx'";
			Db.NonQ(command);
			if(DataConnection.DBtype==DatabaseType.MySql) {
