	...
	public class PopupCrud {
		...
		public static List<Popup> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				popup.PopupLevel      = (HelianzBusiness.EnumPopupLevel)PIn.Int(row["PopupLevel"].ToString());
				...
				popup.DateTimeDisabled= PIn.DateT (row["DateTimeDisabled"].ToString());
				retVal.Add(popup);
			}
			return retVal;
		}
		...
		public static DataTable ListToTable(List<Popup> listPopups,string tableName="") {
			...
			foreach(Popup popup in listPopups) {
				table.Rows.Add(new object[] {
					...
					POut.DateT (popup.DateTimeDisabled,false),
				});
			}
			return table;
		}
		...
		public static long Insert(Popup popup,bool useExistingPK) {
			...
			command+=
				...
				+    POut.Bool  (popup.IsArchived)+","
				+    POut.Long  (popup.PopupNumArchive)+","
				+    POut.DateT (popup.DateTimeDisabled)+")";
		...
		public static void Update(Popup popup) {
			string command="UPDATE popup SET "
				...
				+"DateTimeDisabled=  "+POut.DateT (popup.DateTimeDisabled)+" "
		...
		public static bool Update(Popup popup,Popup oldPopup) {
			...
			if(popup.DateTimeDisabled != oldPopup.DateTimeDisabled) {
				if(command!="") { command+=",";}
				command+="DateTimeDisabled = "+POut.DateT(popup.DateTimeDisabled)+"";
			}
			if(command=="") {
				return false;
			}
