	...
	public class PatientCrud {
		...
		public static List<Patient> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				patient.PatStatus                = (HelianzBusiness.PatientStatus)PIn.Int(row["PatStatus"].ToString());
				patient.Gender                   = (HelianzBusiness.PatientGender)PIn.Int(row["Gender"].ToString());
				patient.Position                 = (HelianzBusiness.PatientPosition)PIn.Int(row["Position"].ToString());
				patient.Birthdate                = PIn.Date  (row["Birthdate"].ToString());
				patient.SSN                      = PIn.String(row["SSN"].ToString());
				patient.Nik                      = PIn.String(row["Nik"].ToString());
				...
				patient.GradeLevel               = (HelianzBusiness.PatientGrade)PIn.Int(row["GradeLevel"].ToString());
				patient.Urgency                  = (HelianzBusiness.TreatmentUrgency)PIn.Int(row["Urgency"].ToString());
				...
				patient.PreferConfirmMethod      = (HelianzBusiness.ContactMethod)PIn.Int(row["PreferConfirmMethod"].ToString());
				patient.PreferContactMethod      = (HelianzBusiness.ContactMethod)PIn.Int(row["PreferContactMethod"].ToString());
				patient.PreferRecallMethod       = (HelianzBusiness.ContactMethod)PIn.Int(row["PreferRecallMethod"].ToString());
				...
				patient.PreferContactConfidential= (HelianzBusiness.ContactMethod)PIn.Int(row["PreferContactConfidential"].ToString());
				patient.SuperFamily              = PIn.Long  (row["SuperFamily"].ToString());
				patient.TxtMsgOk                 = (HelianzBusiness.YN)PIn.Int(row["TxtMsgOk"].ToString());
				...
				patient.ShortCodeOptIn           = (HelianzBusiness.YN)PIn.Int(row["ShortCodeOptIn"].ToString());
				patient.SecurityHash             = PIn.String(row["SecurityHash"].ToString());
				retVal.Add(patient);
			}
			return retVal;
		}
		...
		public static long Insert(Patient patient,bool useExistingPK) {
			...
			command+=
				...
				+    POut.Bool  (patient.HasSignedTil)+","
				+    POut.Int   ((int)patient.ShortCodeOptIn)+","
				+"'"+POut.String(patient.SecurityHash)+"')";
			...
			if(!useExistingPK && PrefC.RandomKeys) {
				...
				while(index < listPatients.Count) {
					...
					if(sbCommands==null) {
						...
						sbCommands.Append("LName,FName,MiddleI,Preferred,PatStatus,Gender,Position,Birthdate,SSN,Nik,Address,Address2,City,State,Zip,HmPhone,WkPhone,WirelessPhone,Guarantor,CreditType,Email,Salutation,EstBalance,PriProv,SecProv,FeeSched,BillingType,ImageFolder,AddrNote,FamFinUrgNote,MedUrgNote,ApptModNote,StudentStatus,SchoolName,ChartNumber,MedicaidID,Bal_0_30,Bal_31_60,Bal_61_90,BalOver90,InsEst,BalTotal,EmployerNum,EmploymentNote,County,GradeLevel,Urgency,DateFirstVisit,ClinicNum,HasIns,TrophyFolder,PlannedIsDone,Premed,Ward,PreferConfirmMethod,PreferContactMethod,PreferRecallMethod,SchedBeforeTime,SchedAfterTime,SchedDayOfWeek,Language,AdmitDate,Title,PayPlanDue,SiteNum,ResponsParty,CanadianEligibilityCode,AskToArriveEarly,PreferContactConfidential,SuperFamily,TxtMsgOk,SmokingSnoMed,Country,DateTimeDeceased,BillingCycleDay,SecUserNumEntry,SecDateEntry,HasSuperBilling,PatNumCloneFrom,DiscountPlanNum,HasSignedTil,ShortCodeOptIn,SecurityHash) VALUES ");
						countRows=0;
					}
					else {
						hasComma=true;
					...
					sbRow.Append("'"+POut.String(patient.Nik)+"'"); sbRow.Append(",");
					...
					sbRow.Append("'"+POut.String(patient.SecurityHash)+"'"); sbRow.Append(")");
		...
		public static void Update(Patient patient) {
			string command="UPDATE patient SET "
				...
				+"SecurityHash             = '"+POut.String(patient.SecurityHash)+"' "
		...
		public static bool Update(Patient patient,Patient oldPatient) {
			...
			if(patient.Nik != oldPatient.Nik) {
				if(command!="") { command+=",";}
				command+="Nik = '"+POut.String(patient.Nik)+"'";
			}
			...
			if(patient.SecurityHash != oldPatient.SecurityHash) {
				if(command!="") { command+=",";}
				command+="SecurityHash = '"+POut.String(patient.SecurityHash)+"'";
			}
			if(command=="") {
				return false;
			}
