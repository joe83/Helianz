	...
	public class SignalodCrud {
		...
		public static List<Signalod> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				if(fKeyType=="") {
					signalod.FKeyType =(HelianzBusiness.KeyType)0;
				}
				else try{
					signalod.FKeyType =(HelianzBusiness.KeyType)Enum.Parse(typeof(HelianzBusiness.KeyType),fKeyType);
				}
				catch{
					signalod.FKeyType =(HelianzBusiness.KeyType)0;
				}
				signalod.IType      = (HelianzBusiness.InvalidType)PIn.Int(row["IType"].ToString());
				signalod.RemoteRole = (HelianzBusiness.MiddleTierRole)PIn.Int(row["RemoteRole"].ToString());
