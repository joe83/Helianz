	...
	public class MountDefCrud {
		...
		public static List<MountDef> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				mountDef.DefaultCat        = PIn.Long  (row["DefaultCat"].ToString());
				mountDef.FlipOnAcquire     = PIn.Bool  (row["FlipOnAcquire"].ToString());
				mountDef.AdjModeAfterSeries= PIn.Bool  (row["AdjModeAfterSeries"].ToString());
				retVal.Add(mountDef);
			}
			return retVal;
		}
			...
			foreach(MountDef mountDef in listMountDefs) {
				table.Rows.Add(new object[] {
					...
					POut.Int   (mountDef.ColorTextBack.ToArgb()),
					            mountDef.ScaleValue,
					POut.Long  (mountDef.DefaultCat),
					POut.Bool  (mountDef.FlipOnAcquire),
					POut.Bool  (mountDef.AdjModeAfterSeries),
				});
			}
			return table;
		}
		...
		public static void Update(MountDef mountDef) {
			string command="UPDATE mountdef SET "
				...
				+"DefaultCat        =  "+POut.Long  (mountDef.DefaultCat)+", "
				+"FlipOnAcquire     =  "+POut.Bool  (mountDef.FlipOnAcquire)+", "
				+"AdjModeAfterSeries=  "+POut.Bool  (mountDef.AdjModeAfterSeries)+" "
		...
		public static bool Update(MountDef mountDef,MountDef oldMountDef) {
			...
			if(mountDef.DefaultCat != oldMountDef.DefaultCat) {
				if(command!="") { command+=",";}
				command+="DefaultCat = "+POut.Long(mountDef.DefaultCat)+"";
			}
			if(mountDef.FlipOnAcquire != oldMountDef.FlipOnAcquire) {
				if(command!="") { command+=",";}
				command+="FlipOnAcquire = "+POut.Bool(mountDef.FlipOnAcquire)+"";
			}
			if(mountDef.AdjModeAfterSeries != oldMountDef.AdjModeAfterSeries) {
				if(command!="") { command+=",";}
				command+="AdjModeAfterSeries = "+POut.Bool(mountDef.AdjModeAfterSeries)+"";
			}
			if(command=="") {
				return false;
			}
