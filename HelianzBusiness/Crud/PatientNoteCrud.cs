	...
	public class PatientNoteCrud {
		...
		public static List<PatientNote> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				patientNote.Consent                   = (HelianzBusiness.PatConsentFlags)PIn.Int(row["Consent"].ToString());
				patientNote.UserNumOrthoLocked        = PIn.Long  (row["UserNumOrthoLocked"].ToString());
				patientNote.Pronoun                   = (HelianzBusiness.PronounPreferred)PIn.Int(row["Pronoun"].ToString());
		...
		public static DataTable ListToTable(List<PatientNote> listPatientNotes,string tableName="") {
			...
			foreach(PatientNote patientNote in listPatientNotes) {
				table.Rows.Add(new object[] {
					...
					POut.Long  (patientNote.UserNumOrthoLocked),
					POut.Int   ((int)patientNote.Pronoun),
				});
			}
			return table;
		}
		...
		public static void Update(PatientNote patientNote) {
			string command="UPDATE patientnote SET "
				...
				+"UserNumOrthoLocked        =  "+POut.Long  (patientNote.UserNumOrthoLocked)+", "
				+"Pronoun                   =  "+POut.Int   ((int)patientNote.Pronoun)+" "
		...
		public static bool Update(PatientNote patientNote,PatientNote oldPatientNote) {
			...
			if(patientNote.UserNumOrthoLocked != oldPatientNote.UserNumOrthoLocked) {
				if(command!="") { command+=",";}
				command+="UserNumOrthoLocked = "+POut.Long(patientNote.UserNumOrthoLocked)+"";
			}
			if(patientNote.Pronoun != oldPatientNote.Pronoun) {
				if(command!="") { command+=",";}
				command+="Pronoun = "+POut.Int   ((int)patientNote.Pronoun)+"";
			}
			if(command=="") {
				return false;
			}
