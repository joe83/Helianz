	...
	public class PromotionLogCrud {
		...
		public static List<PromotionLog> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				promotionLog.PromotionStatus    = (HelianzBusiness.PromotionLogStatus)PIn.Int(row["PromotionStatus"].ToString());
				promotionLog.PatNum             = PIn.Long  (row["PatNum"].ToString());
				promotionLog.ClinicNum          = PIn.Long  (row["ClinicNum"].ToString());
				promotionLog.SendStatus         = (HelianzBusiness.AutoCommStatus)PIn.Int(row["SendStatus"].ToString());
				promotionLog.MessageType        = (HelianzBusiness.CommType)PIn.Int(row["MessageType"].ToString());
				promotionLog.MessageFk          = PIn.Long  (row["MessageFk"].ToString());
				promotionLog.DateTimeEntry      = PIn.DateT (row["DateTimeEntry"].ToString());
				promotionLog.DateTimeSent       = PIn.DateT (row["DateTimeSent"].ToString());
				promotionLog.ResponseDescript   = PIn.String(row["ResponseDescript"].ToString());
				promotionLog.ApptReminderRuleNum= PIn.Long  (row["ApptReminderRuleNum"].ToString());
		...
		public static DataTable ListToTable(List<PromotionLog> listPromotionLogs,string tableName="") {
			...
			foreach(PromotionLog promotionLog in listPromotionLogs) {
				table.Rows.Add(new object[] {
					...
					POut.Long  (promotionLog.ClinicNum),
					POut.Int   ((int)promotionLog.SendStatus),
					POut.Int   ((int)promotionLog.MessageType),
					POut.Long  (promotionLog.MessageFk),
					POut.DateT (promotionLog.DateTimeEntry,false),
					POut.DateT (promotionLog.DateTimeSent,false),
					            promotionLog.ResponseDescript,
					POut.Long  (promotionLog.ApptReminderRuleNum),
				});
			}
			return table;
		}
		...
		public static long Insert(PromotionLog promotionLog,bool useExistingPK) {
			...
			command+=
				...
				+    POut.Long  (promotionLog.ApptReminderRuleNum)+")";
			if(promotionLog.ResponseDescript==null) {
				promotionLog.ResponseDescript="";
			}
			OdSqlParameter paramResponseDescript=new OdSqlParameter("paramResponseDescript",OdDbType.Text,POut.StringParam(promotionLog.ResponseDescript));
			if(useExistingPK || PrefC.RandomKeys) {
		...
		public static void InsertMany(List<PromotionLog> listPromotionLogs,bool useExistingPK) {
			if(!useExistingPK && PrefC.RandomKeys) {
				...
				while(index < listPromotionLogs.Count) {
					...
					sbRow.Append(POut.Long(promotionLog.ClinicNum)); sbRow.Append(",");
					sbRow.Append(POut.Int((int)promotionLog.SendStatus)); sbRow.Append(",");
					sbRow.Append(POut.Int((int)promotionLog.MessageType)); sbRow.Append(",");
					sbRow.Append(POut.Long(promotionLog.MessageFk)); sbRow.Append(",");
					sbRow.Append(DbHelper.Now()); sbRow.Append(",");
					sbRow.Append(POut.DateT(promotionLog.DateTimeSent)); sbRow.Append(",");
					sbRow.Append("'"+POut.String(promotionLog.ResponseDescript)+"'"); sbRow.Append(",");
					sbRow.Append(POut.Long(promotionLog.ApptReminderRuleNum)); sbRow.Append(")");
		...
		public static void Update(PromotionLog promotionLog) {
			string command="UPDATE promotionlog SET "
				...
				+"ClinicNum          =  "+POut.Long  (promotionLog.ClinicNum)+", "
				+"SendStatus         =  "+POut.Int   ((int)promotionLog.SendStatus)+", "
				+"MessageType        =  "+POut.Int   ((int)promotionLog.MessageType)+", "
				+"MessageFk          =  "+POut.Long  (promotionLog.MessageFk)+", "
				//DateTimeEntry not allowed to change
				+"DateTimeSent       =  "+POut.DateT (promotionLog.DateTimeSent)+", "
				+"ResponseDescript   =  "+DbHelper.ParamChar+"paramResponseDescript, "
				+"ApptReminderRuleNum=  "+POut.Long  (promotionLog.ApptReminderRuleNum)+" "
		...
		public static bool Update(PromotionLog promotionLog,PromotionLog oldPromotionLog) {
			...
			if(promotionLog.ClinicNum != oldPromotionLog.ClinicNum) {
				if(command!="") { command+=",";}
				command+="ClinicNum = "+POut.Long(promotionLog.ClinicNum)+"";
			}
			if(promotionLog.SendStatus != oldPromotionLog.SendStatus) {
				if(command!="") { command+=",";}
				command+="SendStatus = "+POut.Int   ((int)promotionLog.SendStatus)+"";
			}
			if(promotionLog.MessageType != oldPromotionLog.MessageType) {
				if(command!="") { command+=",";}
				command+="MessageType = "+POut.Int   ((int)promotionLog.MessageType)+"";
			}
			if(promotionLog.MessageFk != oldPromotionLog.MessageFk) {
				if(command!="") { command+=",";}
				command+="MessageFk = "+POut.Long(promotionLog.MessageFk)+"";
			}
			//DateTimeEntry not allowed to change
			...
			if(promotionLog.ApptReminderRuleNum != oldPromotionLog.ApptReminderRuleNum) {
				if(command!="") { command+=",";}
				command+="ApptReminderRuleNum = "+POut.Long(promotionLog.ApptReminderRuleNum)+"";
			}
			if(command=="") {
				return false;
			}
