...
namespace HelianzBusiness.Bridges{
	/// <summary></summary>
...
*/
		...
		private HelianzBusiness.Pearl.Result SimulateGetResultsForOneImage(string requestId,Bitmap bitmap) {
			HelianzBusiness.Pearl.Result result=new HelianzBusiness.Pearl.Result();
			...
			for(int i=0;i<listEnumConditions.Count;i++) {
				...
				toothPart.color=new HelianzBusiness.Pearl.Color();
		...
		private bool PollRequest() {
			HelianzBusiness.Pearl.Result result=null;
			if(PearlRequests.IsRequestHandled(_pearlRequest)) {
				return true;
			}
			if(ODBuild.IsDebug()&&!ODBuild.IsUnitTest) {
		...
		public void ProcessResultsForOneImage(HelianzBusiness.Pearl.Result result,long docNum,MountItem mountItem,Bitmap bitmap){
