	...
	public class GridOD:System.Windows.Forms.UserControl {
		...
		private void ContextMenuAddPopupLinks(object sender) {
			...
			if(_mouseButtonLastPressed==MouseButtons.Right && rowClick<=this.ListGridRows.Count) {
				...
				_listMenuItemsLinks.AddRange(HelianzBusiness.UI.PopupHelper.GetContextMenuItemLinks(stringBuilder.ToString(),DoShowRightClickLinks));
...
//LostFocus happens before click.  That's where textEdit is cleared.
		...
		private int SortAmountParse(GridRow row1,GridRow row2) {
			decimal amt1=ParseAmountFromText(row1.Cells[_sortedByColumnIdx].Text);
			decimal amt2=ParseAmountFromText(row2.Cells[_sortedByColumnIdx].Text);
