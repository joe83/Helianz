...
namespace HelianzBusiness {
	///<summary>These global methods can be called from anywhere. FormHelianz subscribes to the resulting events so that it can do things like refresh a module or select a patient. Even some classes in HelianzBusiness take advantage of this, although those sections really don't belong in the business layer if they are telling FormHelianz what to do. These will replace the public static methods in FormHelianz that use the singleton pattern. The main reason for switching to this event pattern is that the WPF windows don't have access to FormHelianz. Also see CodeBased.ODEvent which is being deprecated. Also, see DataValid.EventInvalid, which handles all Signalod events.</summary>
	public class GlobalFormHelianz {
