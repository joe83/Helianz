...
namespace HelianzBusiness {
	...
	public class Podium {
		...
		///<summary>The main logic that sends Podium invitations.  Set isService true only when the calling method is the Helianz Service.</summary>
		...
		public static void ShowPage() {
			try {
				if(Programs.IsEnabled(ProgramName.Podium)) {
					Process.Start("http://www.helianz.com/resources/redirects/redirectpodiumdashboard.html");
				}
				else {
					Process.Start("http://www.helianz.com/resources/redirects/redirectpodiumod.html");
				}
			}
			catch {
		...
		///200-299 indicates a successful response, >=300 indicates a Podium error response, negative status codes indicate an Helianz error.</summary>
		...
		public class PropertyDescs {
			...
			public static string UseService="Enter 0 to use Helianz for sending review invitations, or 1 to use the Service";
