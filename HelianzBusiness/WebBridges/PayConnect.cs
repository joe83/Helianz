...
namespace HelianzBusiness {
	public class PayConnect {
		...
		private static PayConnectService.Credentials GetCredentials(Program prog,long clinicNum){
			...
			cred.Username=HelianzBusiness.ProgramProperties.GetPropVal(prog.ProgramNum,"Username",clinicNum);
			cred.Password=CDT.Class1.TryDecrypt(HelianzBusiness.ProgramProperties.GetPropVal(prog.ProgramNum,"Password",clinicNum));
			cred.Client="Helianz2";
			...
			Action<string> showError) 
		{
			try {
				...
				showError(Lans.g("PayConnect","Signature capture failed")+". \r\n"+Lans.g("PayConnect","Error message from")+" Helianz: \""+ex.Message+"\"");
			}
			return null;
		}
		...
		///<summary>Only deletes the credit card from Helianz.  We currently do not delete PayConnect tokens from PayConnect anywhere like we do with XCharge/XWeb/PaySimple.
	...
	public class PayConnectREST {
		...
		///<summary>These headers use the credentials PayConnect needs to get an accountToken from Helianz as a merchant.</summary>
