...
namespace HelianzBusiness {
	...
	public class DoseSpotV2 {
		...
		public static string GetDoseSpotRoot() {
			...
			if(oIDExternal==null) {
				...
				oIDExternal.rootExternal=OIDInternals.HelianzOID+"."+_doseSpotPatNum+"."+OIDInternals.CustomerPatNum;
		...
		public static bool SyncPrescriptionsFromDoseSpot(string clinicID,string clinicKey,string userID,long patNum,Action<List<RxPat>> onRxAdd = null) {
			...
			foreach(DoseSpotMedicationWrapper doseSpotMedicationWrapper in listDoseSpotMedicationWrappers) {
				...
				if(doseSpotMedicationWrapper.IsSelfReported) {
					...
					rxPatOld=RxPats.GetErxByIdForPat(Erx.HelianzErxPrefix+doseSpotMedicationWrapper.MedicationId.ToString(),patNum);
					if(rxPatOld==null) {
				...
				if(doseSpotMedicationWrapper.IsSelfReported) {//Self Reported medications won't have a prescriber number
					if(rxPatOld==null) {//Rx doesn't exist in the database.  This probably originated from DoseSpot
						MedicationPat medicationPat=MedicationPats.GetMedicationOrderByErxIdAndPat(Erx.HelianzErxPrefix+doseSpotMedicationWrapper.MedicationId.ToString(),patNum);
					...
					if(userOdPref==null) {//The Dose Spot User ID from this medication is not present in Helianz.
			...
			for(int i = 0;i<listMedicationPats.Count;i++) {
				...
				if(listMedicationPats[i].DateStop.Year>1880) {
					continue;//The medication is already discontinued in Helianz.
				}
				//The medication was discontinued inside the eRx interface.
				DoseSpotMedicationWrapper doseSpotMedicationWrapper=listDoseSpotMedicationWrappers.FirstOrDefault(x => Erx.HelianzErxPrefix+x.MedicationId.ToString()==eRxGuidCur);
		...
		public static void SyncPrescriptionsToDoseSpot(string clinicID,string clinicKey,string userID,long patNum) {
			...
			foreach(MedicationPat medicationPat in listMedicationPats) {
				...
				if(doseSpotSelfReported.SelfReportedMedicationId>0) {
					...
					medicationPat.ErxGuid=Erx.HelianzErxPrefix+DoseSpotRESTV2.PostSelfReportedMedications(token,oIDExternal.IDExternal,doseSpotSelfReported);
					if(rxPat!=null) {
...
#if DEBUG
			HelianzBusiness.localhost.Service1 service1=new HelianzBusiness.localhost.Service1();

#else
			HelianzBusiness.customerUpdates.Service1 service1=new HelianzBusiness.customerUpdates.Service1();
		...
		private static DoseSpotService.Clinic MakeDoseSpotClinic(Clinic clinic) {
			...
			//This is a required field but there is no way to set this value in Helianz.
	...
	public class DoseSpotRESTV2 {
		...
		public static void MigrateDoseSpotIfNeeded() {
			...
			ProgramProperty programPropertyDoseSpotMigrationRequest=HelianzBusiness.ProgramProperties.GetPropForProgByDesc(Programs.GetProgramNum(ProgramName.eRx),DoseSpotREST.PropertyDescs.DoseSpotApiMigrationRequested);
		...
		public static DoseSpotSelfReported MedicationPatToDoseSpotSelfReport(MedicationPat medicationPat) {
			...
			if(!medicationPat.ErxGuid.IsNullOrEmpty()) {
				...
				if(!guid.StartsWith(Erx.UnsentPrefix)) {
					...
					guid=Regex.Replace(guid,Erx.HelianzErxPrefix,"");//Remove OD prefix.
			...
			if(medicationPat.DateStop.Year<1880 || medicationPat.DateStop>=DateTime.Today) {
				...
				if(doseSpotSelfReported.Comment.IsNullOrEmpty()) {
					...
					doseSpotSelfReported.Comment="Discontinued in Helianz";
				}
			}
