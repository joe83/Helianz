...
namespace HelianzBusiness {
	public class PayPlanEdit {
		...
		///<summary>Issues charges for all active dynamic payment plans in a database. Called by the HelianzService.PaymentPlanThread.</summary>
		public static void IssueChargesDueForDynamicPaymentPlans(List<PayPlan> listDynamicPayPlans,LogWriter log,bool isHelianzService=false) {
			Meth.NoCheckMiddleTierRole();
			...
			try {
				...
				if(isHelianzService){
					//This instance of Helianz service is no longer running the dynamic payment plan logic.  Update the database to indicate this fact.
		...
		///<summary>Run by HelianzService.PaymentThread. Takes any hiddenUnearnedTotal from hidden unearned, and applies it to listPayPlanChargesToday.</summary>
