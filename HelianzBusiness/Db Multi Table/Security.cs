	...
	public class Security{
		...
		public static Userod LogInWeb(string oduser,string odpass,string configPath,string clientVersionStr,bool usingEcw) {
			...
			if(RemotingClient.MiddleTierRole==MiddleTierRole.ServerMT) {
				...
				dto.MethodName="HelianzBusiness.Security.LogInWeb";
		...
		private static bool IsValidEServicePermission(EnumPermType perm) {
			...
			switch(CurUser.EServiceType) {
				...
				case EServiceTypes.HelianzService:
					return IsPermAllowedHelianzService(perm);
		...
		///<summary>Returns true if the HelianzService should be allowed to run methods with the passed in permission.</summary>
		private static bool IsPermAllowedHelianzService(EnumPermType perm) {
