...
namespace HelianzBusiness {
	...
	public class MiscData {
		...
		///<summary>Gets the assembly version from the HelianzBusiness assembly (technically the assembly of the MiscData Type) for the Middle Tier.
		...
		///<summary>Gets the assembly version from the HelianzBusiness assembly (technically the assembly of the MiscData Type).</summary>
		...
		///E.g. if the current database that is connected is called 'helianz182' then the archive name will be 'helianz182_archive'</summary>
		...
		public static void SetSqlMode() {
			...
			if(table.Rows.Count<1 || (table.Rows[0]["Value"].ToString()!="" && table.Rows[0]["Value"].ToString().ToUpper()!="NO_AUTO_CREATE_USER")) {
				command="SET GLOBAL sql_mode=''";//in case user did not use our my.ini file.  http://www.helianz.com/manual/mysqlservervariables.html
				Db.NonQ(command);
			}
		}
