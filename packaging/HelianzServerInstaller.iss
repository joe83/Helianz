; =============================================================================
;  Helianz Server - Inno Setup 6 Installer
;
;  Prerequisites:
;    1. Run Build-HelianzServer.ps1 first -> populates Output\HelianzServer\
;    2. Install Inno Setup 6 (https://jrsoftware.org/isinfo.php)
;    3. Compile this script: iscc HelianzServerInstaller.iss
;       Output: ..\Output\Installer\HelianzServerSetup-24.3.exe
;
...
;    - Sets MySQL root password to "helianz" and creates the helianz DB
...
;    - Creates the HelianzServerPool application pool (.NET 4, 32-bit)
;    - Registers /HelianzServer under "Default Web Site"
;    - Writes HelianzServerConfig.xml with the default credentials
; =============================================================================

#define MyAppName      "Helianz Server"
#define MyAppVersion   "24.3"
#define MyAppPublisher "Helianz Software"
#define MyAppURL       "https://www.helianz.com"
...
DefaultDirName={commonpf64}\HelianzServer
DefaultGroupName={#MyAppName}
AllowNoIcons=yes
; Uncomment if you have a LICENSE file at repo root:
; LicenseFile=..\LICENSE
OutputDir=..\Output\Installer
OutputBaseFilename=HelianzServerSetup-{#MyAppVersion}
...
[Files]
; --- Pre-built HelianzServer web application ---
; Run Build-HelianzServer.ps1 before compiling this installer.
Source: "..\Output\HelianzServer\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs

; --- Baseline database schema (loaded into MySQL on first install) ---
Source: "..\packaging\helianzdata\mysql.sql"; DestDir: "{tmp}"
...
Filename: "powershell.exe"; Parameters: "-ExecutionPolicy Bypass -NonInteractive -Command ""Import-Module WebAdministration -ErrorAction SilentlyContinue; Remove-WebApplication -Name 'HelianzServer' -Site 'Default Web Site' -ErrorAction SilentlyContinue; Remove-WebAppPool -Name 'HelianzServerPool' -ErrorAction SilentlyContinue; Remove-NetFirewallRule -DisplayName 'Block MySQL 3306 (Helianz)' -ErrorAction SilentlyContinue"""; Flags: runhidden waituntilterminated; RunOnceId: "CleanupIIS"

[Messages]
; Shown at the end of the wizard
FinishedLabel=Setup has finished installing [name] on your computer.%n%nThe middle-tier service is now registered at:%n%n    http://localhost/HelianzServer/ServiceMain.asmx%n%nPoint your Helianz clients to this URL.%n%nIf any step failed, review the log at:%n    %%TEMP%%\HelianzServer-install.log
...
begin
  ...
  FailedSteps := '';
  LogPath     := ExpandConstant('{%TEMP%}\HelianzServer-install.log');

  Page := CreateOutputProgressPage(
    'Configuring HelianzServer',
  ...
  try
    ...
    // -----------------------------------------------------------------------
    Page.SetText('Step 5 of 6 — Registering HelianzServer in IIS...', '');
  ...
  if FailedSteps <> '' then
    ...
    MsgBox(
      'HelianzServer installed and configured!' + #13#10 + #13#10 +
      'Endpoint:' + #13#10 +
      '  http://localhost/HelianzServer/ServiceMain.asmx' + #13#10 + #13#10 +
      'Point your Helianz clients to this address.',
      mbInformation, MB_OK);
end;
