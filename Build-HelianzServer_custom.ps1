...
<#
.SYNOPSIS
	Builds the HelianzServer (middle-tier) ASP.NET Web Service, publishes it to a
...
.DESCRIPTION
	Steps performed:
	  ...
	  3. Build & publish the HelianzServer project
...
.PARAMETER OutputDir
	...
	Default: .\Output\HelianzServer

.PARAMETER IISSiteName
	Name of the IIS site that will host HelianzServer.
	Default: "Default Web Site"

.PARAMETER IISAppName
	Virtual path / application name under the IIS site.
	Default: HelianzServer

.PARAMETER IISPort
	TCP port for a *new* dedicated IIS site (only used when -CreateDedicatedSite is set).
	Default: 9391 (for temp/branch servers). Use 80 for main servers.

.PARAMETER AppPoolName
	IIS Application Pool name. Default: HelianzServerPool
...
.PARAMETER ConfigureServer
	...
	connection and middle-tier port, then writes HelianzServerConfig.xml to the
...
.PARAMETER MySqlDatabase
	MySQL database name. Used as the default in the dialog. Default: helianz.
...
.PARAMETER ServerPort
	Middle-tier ServerPort written into HelianzServerConfig.xml. Default: 9390.
...
.EXAMPLE
	# Build only
	.\Build-HelianzServer.ps1

	# Build + register in IIS (run as Administrator)
	.\Build-HelianzServer.ps1 -RegisterIIS

	# Build + register in IIS + configure DB connection interactively (run as Administrator)
	.\Build-HelianzServer.ps1 -RegisterIIS -ConfigureServer

	# Build + configure DB + backup existing output dir before overwriting
	.\Build-HelianzServer.ps1 -ConfigureServer -BackupOutputDir

	# Build + create a dedicated IIS site on port 9391 for a branch/clinic temp server
	.\Build-HelianzServer.ps1 -RegisterIIS -CreateDedicatedSite -IISPort 9391 -IISAppName HelianzServerTemp -AppPoolName HelianzServerTempPool -OutputDir "C:\HelianzServerTemp" -ConfigureServer
#>
param(
	...
	[string]$IISAppName = 'HelianzServer',
...
function Get-HelianzServerConfigSourcePath {
	$preferredPaths = @(
		(Join-Path $PSScriptRoot 'HelianzServerConfig.xml'),
		(Join-Path $PSScriptRoot 'HelianzServerConfig.xml')
	)
...
# ---------------------------------------------------------------------------
# Helper: Parse an existing HelianzServerConfig.xml into a PSCustomObject
# ---------------------------------------------------------------------------
function Read-ExistingServerConfig {
	...
	try {
		...
		return [PSCustomObject]@{
			...
			Database    = if ($conn -and $conn.Database)      { $conn.Database.Trim() }      else { 'helianz' }
...
function Invoke-ServerConfigDialog {
	...
	Write-Host "  HelianzServer Configuration Setup" -ForegroundColor Cyan
	Write-Host "  --------------------------------------------------------" -ForegroundColor Cyan
	...
	$resolvedDb   = p -Label 'Database Name'  -Param $ParamDatabase  -Existing $(if ($existing) { $existing.Database } else { '' }) -Fallback 'helianz'
...
# ---------------------------------------------------------------------------
# Helper: Write HelianzServerConfig.xml from a config object
# ---------------------------------------------------------------------------
function Write-ServerConfigXml {
	...
	$lines.Add("		<DatabaseType>MySql</DatabaseType>")
	$lines.Add("		<ApplicationName>/HelianzServer</ApplicationName>")
	$lines.Add("		<VerboseLogging>false</VerboseLogging>")
...
function Invoke-OutputDirBackup {
	...
	if (-not (Test-Path $BackupDir)) { New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null }
	$ts   = Get-Date -Format 'yyyyMMdd_HHmmss'
	$dest = Join-Path $BackupDir "HelianzServer_$ts"
...
Write-Host ""
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host "  HelianzServer (Middle-Tier) Build Script" -ForegroundColor Cyan
...
if ($RegisterIIS -or $Remove) { Assert-Administrator }

# ---------------------------------------------------------------------------
# Remove mode — tear down the dedicated site
# ---------------------------------------------------------------------------
if ($Remove) {
	Write-Host ""
	Write-Host "==================================================================" -ForegroundColor Magenta
	Write-Host "  HelianzServer TEARDOWN" -ForegroundColor Magenta
	Write-Host "  Site    : $IISAppName" -ForegroundColor Magenta
	Write-Host "  Port    : $IISPort" -ForegroundColor Magenta
	Write-Host "==================================================================" -ForegroundColor Magenta
	...
	if ($site) {
		...
		Remove-Website -Name $IISAppName -ErrorAction Stop
		Write-Host "  Site '$IISAppName' removed." -ForegroundColor Green
	} else {
		Write-Host "  Site '$IISAppName' not found." -ForegroundColor DarkGray
	}
	...
	if ($pool) {
		...
		Write-Host "  App Pool '$AppPoolName' not found." -ForegroundColor DarkGray
	}

	Write-Host ""
	Write-Host "  Teardown complete. Output folder preserved at:" -ForegroundColor DarkYellow
	Write-Host "    $OutputDir" -ForegroundColor DarkYellow
	Write-Host ""
	exit 0
}
...
$fallbackPublishDir = Join-Path $PSScriptRoot "_publish_HelianzServer_fallback"
...
# ---------------------------------------------------------------------------
Write-Host ""
Write-Host "[STEP 2/4] Building and publishing HelianzServer ($Configuration)..." -ForegroundColor Yellow
...
$buildArgs = @(
	...
	"/verbosity:minimal",
	"/fl",
	"/flp:LogFile=`"$PSScriptRoot\build-helianzserver.log`";Verbosity=normal"
)

& $MsBuildPath @buildArgs
if ($LASTEXITCODE -ne 0) {
	Write-Host ""
	Write-Error "Build FAILED. Check build-helianzserver.log for details."
	exit 1
}
...
$configSourcePath = Get-HelianzServerConfigSourcePath
$preservedConfigPath = $null

if (-not $configSourcePath -and (Test-Path (Join-Path $OutputDir 'HelianzServerConfig.xml'))) {
	$preservedConfigPath = Join-Path $PSScriptRoot '_publish_HelianzServer_config.xml'
	Copy-Item -Path (Join-Path $OutputDir 'HelianzServerConfig.xml') -Destination $preservedConfigPath -Force
}
...
if (Test-Path "$publishDir\Web.config") {
	...
	Write-Host "[INFO] Using PackageTmp output: $packageTmpDir2" -ForegroundColor DarkCyan
	$sourceDir = $packageTmpDir2
} else {
	# Last resort: assemble a deployable web root from project content + bin output.
	$binFallback = Join-Path $PSScriptRoot "HelianzServer\bin"
	if (Test-Path "$binFallback\HelianzServer.dll") {
		...
		Copy-Item -Path (Join-Path $PSScriptRoot 'HelianzServer\ServiceMain.asmx') -Destination $fallbackPublishDir -Force
		Copy-Item -Path (Join-Path $PSScriptRoot 'HelianzServer\Web.config') -Destination $fallbackPublishDir -Force
		$appDataDir = Join-Path $PSScriptRoot 'HelianzServer\App_Data'
...
if (-not $sourceDir) {
	Write-Error "Could not locate published output. Check build-helianzserver.log for details."
	exit 1
}
...
# Copy this script into the output folder so it's self-contained for server deployment
if ($PSCommandPath) {
	Copy-Item -Path $PSCommandPath -Destination $OutputDir -Force
	Write-Host "[INFO] Script copied to output folder (self-contained)." -ForegroundColor DarkGray
}

$targetConfigPath = if ($CreateDedicatedSite) {
	$cfgDir = Join-Path $OutputDir "HelianzServer"
	if (-not (Test-Path $cfgDir)) { New-Item -ItemType Directory -Path $cfgDir -Force | Out-Null }
	Join-Path $cfgDir "HelianzServerConfig.xml"
} else {
	Join-Path $OutputDir 'HelianzServerConfig.xml'
}
...
if ($ConfigureServer) {
	...
	Write-Host "[INFO] HelianzServerConfig.xml written with configured settings." -ForegroundColor Green
	...
	Write-Host "[INFO] Preserved existing HelianzServerConfig.xml" -ForegroundColor DarkCyan
} else {
	Write-Host "[WARN] No HelianzServerConfig.xml source found. The middle tier will not connect to MySQL until this file is added." -ForegroundColor DarkYellow
...
if ($RegisterIIS) {
	Write-Host ""
	Write-Host "[STEP 4/4] Registering HelianzServer in IIS..." -ForegroundColor Yellow
	$url = ''
	...
	$pool.enable32BitAppOnWin64 = ($Platform -eq 'x86') -or $CreateDedicatedSite
	...
	if ($CreateDedicatedSite) {
		# --- Create a brand-new IIS site with /HelianzServer application ---
		#     Site root: OutputDir (empty parent, scripts live here)
		#     App /HelianzServer: OutputDir\HelianzServer (actual server files)
		#     URL: http://HOST:PORT/HelianzServer/ServiceMain.asmx
		$appSubDir = Join-Path $OutputDir "HelianzServer"

		# Move server files into HelianzServer\ subfolder if not already there
		if (-not (Test-Path (Join-Path $appSubDir "Web.config"))) {
			...
			Get-ChildItem -Path $OutputDir -Force | Where-Object {
				$_.Name -ne "HelianzServer" -and $_.PSIsContainer -eq $false
			} | Move-Item -Destination $appSubDir -Force -ErrorAction SilentlyContinue
			# Also move bin folder
			if (Test-Path (Join-Path $OutputDir "bin")) {
				Move-Item -Path (Join-Path $OutputDir "bin") -Destination $appSubDir -Force -ErrorAction SilentlyContinue
			}
		...
		if (Get-Website -Name $IISAppName -ErrorAction SilentlyContinue) {
			Write-Host "  IIS site '$IISAppName' already exists - updating." -ForegroundColor DarkGray
		...
		# Add /HelianzServer application so clients reach the right path
		# Delete and recreate for idempotency (avoids folder-vs-app confusion)
		Remove-WebApplication -Name "HelianzServer" -Site $IISAppName -ErrorAction SilentlyContinue
		New-WebApplication -Name "HelianzServer" -Site $IISAppName -PhysicalPath $appSubDir -ApplicationPool $AppPoolName | Out-Null
		Write-Host "  /HelianzServer application created." -ForegroundColor DarkCyan

		$url = "http://localhost:$IISPort/HelianzServer/ServiceMain.asmx"
	...
	if (-not $url) {
		if ($CreateDedicatedSite) {
			$url = "$(Get-SiteBaseUrl -SiteName $IISAppName)/HelianzServer/ServiceMain.asmx"
		} else {
