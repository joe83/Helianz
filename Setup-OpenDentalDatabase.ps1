...
<#
.SYNOPSIS
	Installs MySQL Community Server (if needed) and creates a fresh OpenDental
	database ready for the community 24.3.x application.

.DESCRIPTION
	Steps performed:
	  1. Parse connection settings from OpenDentServerConfig.xml
	...
	# 7. Load baseline schema (packaging/opendentaldata/mysql.sql -> v3.4.17.0)
...
After the database is created, this script loads the baseline schema from
	'packaging/opendentaldata/mysql.sql' (seeds the DB at version 3.4.17.0).
	The OpenDental application then automatically upgrades from 3.4.17.0 to
	24.3.45.0 via the ConvertDatabases chain on first startup.

.PARAMETER ConfigPath
	Path to OpenDentServerConfig.xml. Defaults to the repo-root copy.
...
.EXAMPLE
	...
	.\Setup-OpenDentalDatabase.ps1

	# Drop and recreate without prompting
	.\Setup-OpenDentalDatabase.ps1 -Force

	# MySQL already installed with blank root password
	.\Setup-OpenDentalDatabase.ps1 -MySqlRootPassword '' -Force

	# Run elevated automatically (or use _setup_db.ps1 wrapper)
	Start-Process powershell -Verb RunAs -ArgumentList '-File "Setup-OpenDentalDatabase.ps1" -Force' -Wait
#>

param(
	[string]$ConfigPath        = "$PSScriptRoot\OpenDentServerConfig.xml",
...
# ---------------------------------------------------------------------------
# Helper: Parse first DatabaseConnection from OpenDentServerConfig.xml
# ---------------------------------------------------------------------------
...
Write-Host "  OpenDental Database Setup" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
Write-Host ""
...
# ---------------------------------------------------------------------------
$baselineSql = Join-Path $PSScriptRoot 'packaging\opendentaldata\mysql.sql'
Write-Host ""
...
if (-not (Test-Path $baselineSql)) {
	...
	Write-Host "  The database was created empty. OpenDental cannot auto-initialize from empty." -ForegroundColor DarkYellow
	Write-Host "  You must manually load the baseline schema before starting OpenDental." -ForegroundColor DarkYellow
} else {
	...
	Write-Host "  OpenDental will upgrade from 3.4.17.0 to 24.3.45.0 automatically on first startup." -ForegroundColor DarkGray
}
...
if ($appConnect) {
	...
	Write-Host "  [WARN] App user could not connect. Check the credentials in OpenDentServerConfig.xml." -ForegroundColor DarkYellow
}
...
Write-Host "  1. Deploy OpenDentalServer to IIS (run _iis_run.ps1 if not done)." -ForegroundColor DarkGray
Write-Host "  2. Connect the OpenDental 24.3.x client to this server." -ForegroundColor DarkGray
