...
namespace HelianzCloud {
	...
	public static class Sftp {
		...
		public class Download : TaskStateDownload {
			private static string _stashRootFolderPath=ODFileUtils.CombinePaths(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),"Helianz","Stash");
				...
				try {
					...
					foreach(string stashInstanceFolderPath in arrayStashInstanceFolderPaths) {
						...
						try {
							...
							if(process!=null && process.ProcessName.StartsWith("Helianz")) {//The process for this stash instance folder is still running.
								isRunning=true;
							}
						}
						catch(Exception ex) {
